 ...
 * @(#)WritingShape.java
 *
 * $Date: 2014-11-27 06:02:59 +0100 (Do, 27 Nov 2014) $
 *
 ...
 * <p>This is used to give the impression of painting strokes over time.
 */
public class WritingShape {
	/** The key that relates to the bounds of this shape.
	 * Note these are not the literal bounds; this is the expected frame. Strokes do
	 * not need to fill these bounds, and they can liberally step outside of these
	 * bounds for occasional flourishes.
	 */
	public final static Key<Rectangle2D> BOUNDS_KEY = new Key<Rectangle2D>("bounds", Rectangle2D.class);
	...
	/** The properties this shape manages. Currently this only manages the BOUNDS_KEY.
	 */
	 ...
	 * 
	 * @param source the shape to clone.
	 */
	 ...
	 * @param s previous output from <code>WritingShape.toString()</code> 
	 */
	public WritingShape(String s) {
		assign(s);
	}
	
	/** Reset this WritingShape to have no strokes and a unit rectangle as bounds. */
	public void reset() {
		setBounds(new Rectangle2D.Float(0,0,1,1));
		strokes.clear();
	}
	
	/** Invoke <code>reset()</code> and reformat this shape from the string provided.
	 * 
	 * @param s previous output from WritingShape.toString()
	 */
	...
	/** Serialize this shape as a String. This mimics the standard shape
	 * output (m, l, q, c, z), except it also includes the instruction "b" (4 arguments
	 * for the bounds), and "w" (1 argument for the pause beat). The "w" instruction
	 * happens to separate each unique WritingStroke.
	 */
	@Override
	...
	/** Add a PropertyChangeListener that will be notified when the bounds of this
	 * WritingShape change.
	 * @param pcl a new listener to receive notifications when the bounds change.
	 */
	 ...
	 * <p>Note these are not the literal bounds; this is the expected frame. Strokes do
	 * not need to fill these bounds, and they can liberally step outside of these
	 * bounds for occasional flourishes.
	 * @param r the new bounds for this shape.
	 */
	 ...
	 * @param bounds the bounds of this shape to draw to. The Graphics2D will be
	 * transformed so every stroke will fit in this argument as if it were
	 * <code>WritingShape.getBounds()</code>.
	 * @param time the time (in seconds), where t=0 is the beginning of rendering this
	 * shape.
	 * @param beatsPerSecond the number of beats per second.
	 * @param pixelsPerSecond the number of pixels per second.
	 * @return whether the shape is completely rendered. This should be equivalent
	 * to <code>(t &lt; getDuration( beatsPerSecond, pixelsPerSecond)</code> .
	 */
	public boolean paint(Graphics g,Rectangle2D bounds,float time,float beatsPerSecond,float pixelsPerSecond) {
		if(beatsPerSecond<=0) throw new IllegalArgumentException("beatsPerSecond must be greater than zero");
		if(pixelsPerSecond<=0) throw new IllegalArgumentException("pixelsPerSecond must be greater than zero");
		
		Graphics2D g2 = (Graphics2D)g.create();
		try {
			AffineTransform tx = TransformUtils.createAffineTransform(getBounds(), bounds);
			// if this WritingShape is based on a unit rectangle, the caller probably configured a
