...
/** This implements several methods to get <code>Graphics2D</code>'s
 * off the ground.  Methods that are implemented here may not be
...
public abstract class AbstractGraphics2D extends Graphics2D {
	 ...
	 * This will be an Area, a Rectangle, or a Rectangle2D.
	 */
	protected Shape clipping = null;
	/** The optional name.  Used for debugging, or naming groups in vector graphics. */
	...
	public void reset() {
		...
		font = UIManager.getFont("Label.font")==null ?
	...
	/** Returns the name of this Graphics2D.
	...
	public Color getColor() {
		...
		throw new ClassCastException("The current paint is a "+paint.getClass().getName());
	}
	...
	/** This changes the transform of this <code>Graphics2D</code>,
	 * renderings the image, and restores the transform of this image.
	 */
	@Override
	public boolean drawImage(Image i, AffineTransform t,
			ImageObserver obs) {
	...
	/** This does nothing, because setXORMode is unsupported.
	 * Therefore we're always in paint mode.
	 */
	@Override
	...
	/** This throws an <code>UnsupportedOperationException</code>
	...
	/** This creates a new image from this operation, renders that image, and flushes it. */
	@Override
	...
	/** If <code>i</code> is not a <code>BufferedImage</code>, then
	 * a new <code>BufferedImage</code> is created just to render this image.
	 */
	@Override
	public void drawRenderedImage(RenderedImage i, AffineTransform t) {
		if(i instanceof BufferedImage) {
			...
			BufferedImage bi = new BufferedImage(cm,r,cm.isAlphaPremultiplied(),properties);
			drawImage(bi,t,null);
			bi.flush();
		}
	}
	...
	/** Creates a <code>GlyphVector</code> and calls <code>drawGlyphVector()</code>.
	 */
	@Override
	 ...
	 * <P>This way if the AttributedCharacterIterator represents several
	 * different fills, they'll be handled correctly.
	 */ 
	@Override
	 ...
	 */
	@Override
	public void drawRoundRect(int x, int y, int w, int h, int arcW,
			int arcH) {
	 ...
	 */
	@Override
	public void fillRoundRect(int x, int y, int w, int h, int arcW,
			int arcH) {
		RoundRectangle2D r = new RoundRectangle2D.Float(x,y,w,h,arcW,arcH);
		fill(r);
	}
	...
	static synchronized void loadImage(Image i) {
		if(i instanceof BufferedImage) return;
	...
	public boolean drawImage(Image i, int x, int y, ImageObserver obs) {
		if(i instanceof BufferedImage) {
			BufferedImage bi = (BufferedImage)i;
			return drawImage(bi,x,y,x+bi.getWidth(),y+bi.getHeight(),obs);
		}
		loadImage(i);
		return drawImage(i,x,y,x+i.getWidth(null),y+i.getHeight(null),obs);
	}

	/** Calls another <code>drawImage</code> method */
	@Override
	public boolean drawImage(Image i, int x, int y, int w,
			int h, ImageObserver obs) {
		if(i instanceof BufferedImage) {
			BufferedImage bi = (BufferedImage)i;
			return drawImage(bi,x,y,w,h,0,0,bi.getWidth(),bi.getHeight(),obs);
		}
		loadImage(i);
		return drawImage(i,x,y,w,h,0,0,i.getWidth(null),i.getHeight(null),obs);
	}
	...
	public boolean drawImage(Image i, int x, int y, Color bgColor,ImageObserver obs) {
		if(i instanceof BufferedImage) {
			BufferedImage bi = (BufferedImage)i;
			return drawImage(bi,x,y,x+bi.getWidth(),y+bi.getHeight(),bgColor,obs);
		}
		loadImage(i);
		return drawImage(i,x,y,x+i.getWidth(null),y+i.getHeight(null),bgColor,obs);
	}

	/** Calls another <code>drawImage</code> method */
	@Override
	public boolean drawImage(Image i, int x, int y, int w,
			int h, Color bgColor, ImageObserver obs) {
		if(i instanceof BufferedImage) {
			BufferedImage bi = (BufferedImage)i;
			return drawImage(bi,x,y,w,h,0,0,bi.getWidth(),bi.getHeight(),bgColor,obs);
		}
		loadImage(i);
		return drawImage(i,x,y,w,y,0,0,i.getWidth(null),i.getHeight(null),bgColor,obs);
	}


	/** Calls <code>drawImage(img, dx1, dy1, dy1, dy2, sx1, sy1, sx2, sy2, null, observer)</code>. */
	@Override
	...
	static private final BufferedImage tinyScratch = new BufferedImage(1,1,BufferedImage.TYPE_INT_ARGB);
