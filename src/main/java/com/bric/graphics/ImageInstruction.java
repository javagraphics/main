 ...
 *
 * This instruction should be equivalent to calling this code:
 * <br><code>g = (Graphics2D)g.create();</code>
 * <br><code>g.clip(clipping);</code>
 * <br><code>g.setTransform(transform);</code>
 * <br><code>g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,opacity));</code>
 * <br><code>g.drawImage(bi, destRect.x, destRect.y, </code>
 * <br><code> &nbsp; destRect.x+destRect.width, destRect.y+destRect.height, </code>
 * <br><code> &nbsp; sourceRect.x, sourceRect.y, </code>
 * <br><code> &nbsp; sourceRect.x+sourceRect.width, sourceRect.y+sourceRect.height,</code>
 * <br><code> &nbsp; null);</code>
 * 
 */
public class ImageInstruction implements GraphicInstruction, Serializable {
	 ...
	 * @param t the transform to draw with.
	 * @param dest the destination rectangle.
	 * @param src the source rectangle.
	 * @param clip the optional clipping.
	 * @param opacity the opacity.
	 */
	public ImageInstruction(Image i,AffineTransform t,Rectangle dest,Rectangle src,Shape clip,float opacity) {
		...
		if(sourceRect.x+sourceRect.width>bi.getWidth())
			throw new IllegalArgumentException("the source width ("+(sourceRect.x+sourceRect.width)+") exceeds the image width ("+bi.getWidth()+").");
		if(sourceRect.y+sourceRect.height>bi.getHeight())
			throw new IllegalArgumentException("the source height ("+(sourceRect.y+sourceRect.height)+") exceeds the image height ("+bi.getHeight()+").");
		destRect = new Rectangle(dest);
		if(clip instanceof Rectangle) {
			clipping = new Rectangle( (Rectangle)clip );
		} else if(clip instanceof Rectangle2D) {
			Rectangle2D r = (Rectangle2D)clip ;
			clipping = new Rectangle2D.Double( r.getX(), r.getY(), r.getWidth(), r.getHeight() );
		} else if(clip!=null) {
	...
	/** The source rectangle.  This is anchored at (0,0) and should not
	 * exceed the width/height of the original image.
	 */
	...
	/** Returns a BufferedImage of the argument.
	 * If the argument is already a BufferedImage, it is immediately
	 * returned.
	 * Otherwise a new BufferedImage is created and the argument
	 * is rendered in it.
	 * @param i
	 * @return
	 */
	...
	public void paint(Graphics2D g)
	{
		...
		g.drawImage(bi, destRect.x, destRect.y, 
	 ...
	 * This may return an empty area if the clipping and destination rect don't intersect.
	 */
	 ...
	 * render the entire buffered image.  That is: the entire buffered
	 * image should be stretched so (0,0) and (w,h) fit within
	 * the rectangle returned by this method.  This eliminates the need for
	 * the sourceRect data, because it is now implied that the sourceRect
	 * is (0,0,w,h).
	 */
	public Rectangle2D getResizedDestRect() {
		...
		double ls = (sourceRect.getX()+sourceRect.getWidth())/(sourceRect.getWidth());
		double ts = (h-sourceRect.getY())/(sourceRect.getHeight());
		double bs = (sourceRect.getY()+sourceRect.getHeight())/(sourceRect.getHeight());
		double x2 = destRect.getX()+destRect.getWidth();
		double y2 = destRect.getY()+destRect.getHeight();
		Rectangle2D r = new Rectangle2D.Double( 
				x2-destRect.getWidth()*ls,
				y2-destRect.getHeight()*ts,
				destRect.getWidth()*rs,
	...
	private void writeObject(java.io.ObjectOutputStream out) throws IOException {
		...
		if(bi.getType()==BufferedImage.TYPE_INT_ARGB) {
			...
			argbImage = new BufferedImage(bi.getWidth(),bi.getHeight(),BufferedImage.TYPE_INT_ARGB);
			Graphics2D g = argbImage.createGraphics();
			g.drawImage(bi,0,0,null);
			g.dispose();
		}
		int[] data = new int[bi.getWidth()*bi.getHeight()];
		argbImage.getRaster().getDataElements(0,0,bi.getWidth(),bi.getHeight(),data);
