 ...
 * @(#)GraphicsWriter.java
 *
 * $Date: 2014-10-29 06:15:04 +0100 (Mi, 29 Okt 2014) $
 *
 ...
 * <P>As you render objects to this writer, several {@link GraphicInstruction} objects
 * are collected in a stack.  An exception is thrown if you attempt to use any
 * composite that is not a <code>AlphaComposite.SRC_OVER</code>, because this assumes
 * each element is rendered "on top of" all existing elements.
 * <P>The concept here is that simple vector art can be stored and translated.  This
 * object (and all instructions) should be fully serializable.  Also you can pass this
 * <code>Graphics2D</code> to any existing <code>paint()</code> method and then sift
 * through the resulting instructions to export the data to another format.  (For
 * example, this model was originally designed to help export graphics to PPTX files.)
 * 
 */
public class GraphicsWriter extends AbstractGraphics2D 
	...
	/** If this is true, then GraphicInstruction.getSource() will return
	 * part of the stack trace of the call that created an instruction.
	 */
	...
	Vector<GraphicInstruction> elements;
	...
	/**
	 * 
	 * @param createMakesNewGroup if this is true, then when
	 * Graphics2D.create() is called for this object, a new writer
	 * will be created that contains its own distinct list of
	 * instructions.  If this is false, then the same instruction list
	 * is shared between this object and its clone.
	 * <P>This can be used to imply the existence of groups in vector
	 * graphics.
	 */
	 ...
	 * <P>This can be used to imply the existence of groups in vector
	 * graphics.
	 * @param instructionLimit the maximum number of instructions this writer will
	 * store.
	 */
	public GraphicsWriter(boolean createMakesNewGroup,int instructionLimit) {
		elements = new Vector<GraphicInstruction>();
	...
	public void draw(Shape s)
	{
		if(root.instructionCount>=root.instructionLimit)
			return;
		DrawInstruction drawInstr = new BasicDrawInstruction(s,transform,clipping,paint,stroke,opacity);
		FillInstruction fillInstr = null;
		int lastIndex = elements.size()-1;
		if(elements.size()>0 && elements.get(lastIndex) instanceof FillInstruction)
			fillInstr = (FillInstruction)elements.get(lastIndex);
		BasicShapeInstruction mergedInstruction = BasicShapeInstruction.merge(fillInstr, drawInstr);
	 ...
	 * <P>Depending on how this writer was constructed, this may contain
	 * other <code>GraphicsWriter</code> objects (suggesting nested groups).
	 * 
	 * @param simplify this removes empty groups/instructions from the instructions.
	 * @return an array of instructions that recreate this graphics.
	 */
	public GraphicInstruction[] getInstructions(boolean simplify) {
		if(simplify && elements.size()==1 && (elements.get(0) instanceof GraphicsWriter)) {
		...
		return simplifiedList.toArray(new GraphicInstruction[simplifiedList.size()]);
	}
	...
	public boolean drawImage(Image img, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2,
		...
		add(new ImageInstruction(img,getTransform(),destRect,srcRect,clipping,opacity));
		return true;
	}
	...
	public void setComposite(Composite comp) {
		if(!(comp instanceof AlphaComposite))
			throw new IllegalArgumentException("Only AlphaComposite of type SRC_OVER are supported.");
		
		AlphaComposite ac = (AlphaComposite)comp;
		int type = ac.getRule();
		if(type==AlphaComposite.CLEAR && isEmpty()) {
			//TODO: keep track of parents, and see if entire graphics tree is empty or not
			setOpacity(0);
	...
	/** The AlphaComposite class doesn't have a useful
	* toString() method
	**/
	public static String toString(AlphaComposite c) {
		return "AlphaComposite[rule="+Reflection.nameStaticField(AlphaComposite.class,new Integer(c.getRule()))+", alpha="+c.getAlpha()+"]";
	}
	
	public void setOpacity(float f) {
		if(opacity<0 || opacity>1)
			throw new IllegalArgumentException("The opacity ("+f+") must between [0,1].");
		opacity = f;
		super.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, f));
	}
	...
	/** Returns the bounds that would be affected if <code>paint()</code> were called. */
	...
	private void readObject(java.io.ObjectInputStream in) throws IOException, ClassNotFoundException {
		elements = (Vector<GraphicInstruction>)in.readObject();
	...
	public void drawString(AttributedCharacterIterator aci, float x, float y) {
		...
		r.setFrame(r.getX()-1+x,r.getY()-1+y,r.getWidth()+2,r.getHeight()+2);
	...
	void drawTextBox(Rectangle2D box,Color background,Color frame,float frameThickness,AttributedCharacterIterator text,Insets insets) {
		if(root.instructionCount>=root.instructionLimit)
			return;
		add(new TextBoxInstruction(box,background,frame,frameThickness,text,insets,transform,clipping,Float.MAX_VALUE,opacity));
	}
	...
	/** Returns the current stack trace, minus all lines that involve "com.bric.graphics" objects. */
