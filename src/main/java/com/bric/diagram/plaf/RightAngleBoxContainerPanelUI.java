/*
 * @(#)RightAngleBoxContainerPanelUI.java
 *
 * $Date: 2015-09-28 23:54:10 +0200 (Mo, 28 Sep 2015) $
 *
...
import com.bric.diagram.BinaryRelationship;
import com.bric.diagram.Box;
import com.bric.diagram.BoxContainer;
import com.bric.diagram.Connector;
...
import com.bric.util.CombinationIterator;
...
{	
	public static final Key<Point> KEY_JOIN1_CONTROL_POINT = new Key<>("join-control-1", Point.class);
	public static final Key<Point> KEY_JOIN2_CONTROL_POINT = new Key<>("join-control-2", Point.class);
	public static final Key<Boolean> KEY_STAGING_DIRTY = new Key<>("staging-dirty", Boolean.class);
	...
	private static final Comparator<Join> JOIN_COMPARATOR = new Comparator<Join>() {
		...
		public int compare(Join o1, Join o2)
		{
			if(!o1.boxEdge.equals(o2.boxEdge))
				throw new RuntimeException("this comparator requires joins share the same box edge");
	...
	static class BoxEdge {
			...
			if(anchors==null || forceRegeneration) {
				...
				if(side.isLeftOrRight) {
					...
					for(int i = 0; i<orderedJoins.length; i++) {
						int y = rect.y + (i+1)*span/(orderedJoins.length+1);
					...
					for(int i = 0; i<orderedJoins.length; i++) {
						int x = rect.x + (i+1)*span/(orderedJoins.length+1);
						anchors[i] = new Point(x, y);
					}
				}
			}
		...
		public String toString() {
			return side.toString()+" of "+box.getBounds()+" ("+box.getID()+")";
		}
	...
	public void paint(Graphics g0, JComponent c)
	{
			...
			while(iter.hasNext()) {
				...
				if(oldScenario!=scenario) {
					...
					if(opacity<0) {
						...
						g.setStroke(new BasicStroke(3*(1-opacity*opacity*opacity*opacity)));
		...
		if(staging.debugging) {
			...
			g.drawString(""+i+" / "+staging.allScenarios.size()+" ("+scenarioIndex+")", 5, 20);
			...
			for(BoxEdge edge : scenario.allEdges) {
				...
				for(int a = 0; a<joins.length; a++) {
					g.drawString(joins[a].boxEdge.box.getID()+": "+joins[a].boxEdge.side, 5, y);
					y+=14;
				}
			}
			g.dispose();
	...
	static class Join {
		...
		/**
		 * @param boxEdge the side of the box this join originates from.
		 * @param connector
		 * @param first if true this refers to box1 of a Connector, if false it refers to box2
		 */
		...
		@Override
		public String toString() {
			return "Join[ edge="+boxEdge+", first="+first+", connector="+connector+"]";
		}
	}

	static final Key<ExecutorService> KEY_EXECUTOR_SERVICE = new Key<>("executorService", ExecutorService.class);
	...
	static class Staging implements Runnable {
			...
			public void run() {
				synchronized(changeListeners) {
					...
					for(ChangeListener changeListener : changeListeners) {
						changeListener.stateChanged(new ChangeEvent(Staging.this));
					}
				}
			}
		};
		
		public Staging(BoxContainerPanelUI ui,PlafContext context,BoxContainerPanel bcp,boolean debugging) {
			this.debugging = debugging;
			this.boxContainerPanel = bcp;
			this.ui = ui;
			boxContainer = bcp.getBoxContainer();
			if(boxContainer!=null) {
				ObservableList<Connector> connectors = boxContainer.getConnectors();
				
				List<Join[]> possibleJoins = new ArrayList<>();
				for(Connector connector : connectors) {
					boolean b1 = boxContainer.getBoxes().contains(connector.getBox1());
					boolean b2 = boxContainer.getBoxes().contains(connector.getBox2());
					if(b1 && b2) {
		...
		public void run() {
			try {
				...
				while(true) {
					...
					for(Join join : joins) {
						SortedSet<Join> allJoinsForEdge = edgesToJoins.get(join.boxEdge);
					...
					for(BoxEdge edge : edgesToJoins.keySet()) {
						...
						Join[] joinArray = localJoins.toArray(new Join[localJoins.size()]);
					...
					Scenario thisScenario = new Scenario(ui, boxContainer.getBoxes(), boxEdges);
		...
		private void submitScenario(Scenario newScenario) {
			...
			synchronized(this) {
				if(bestScenario==null || newScenario.compareTo(bestScenario)<0) {
	...
	private static final Key<Staging> KEY_STAGING = new Key<>("staging", Staging.class);
	...
	protected void restage(BoxContainerPanel bcp)
	{
		try {
			synchronized(bcp) {
				...
				Staging newStaging = new Staging(this, context, bcp, debugScenarios);
				newStaging.addChangeListener(stagingChangeListener);
				context.set(KEY_STAGING, newStaging);
				bcp.repaint();

				/*bcp.setLayout(new GridBagLayout());
	...
	static class Scenario implements Comparable<Scenario> {
		...
		/** The first criteria: how many connectors overlap boxes? (measured in pixels) */
		static final int SCORE_INDEX_PIXELS_CONNECTORS_OVERLAPPING_BOXES = 0;
		
		/** The second criteria: how many connectors overlap other connectors? (measured in pixels) */
		...
		Scenario(BoxContainerPanelUI ui,List<Box> allBoxes,List<BoxEdge> allEdges) {
				...
				case SCORE_INDEX_PIXELS_CONNECTORS_OVERLAPPING_BOXES:
					for(int a = 0; a<allEdges.size(); a++) {
						...
						for(int i = 0; i<joins.length; i++) {
							Point p1 = anchors[i];
							Point p3 = joins[i].connector.getControlPoint(scratch1);
							Point p2 = scratch2;
					...
					for(int a = 0; a<allEdges.size(); a++) {
						...
						for(int i = 0; i<joins.length; i++) {
							...
							for(int b = a; b<allEdges.size(); b++) {
								...
								Point[] otherAnchors = otherEdge.getAnchors(false);
								for(int j = 0; j<otherJoins.length; j++) {
									if(otherJoins[j].connector!=joins[i].connector) {
										Point p1b = otherAnchors[j];
										Point p3b = otherJoins[j].connector.getControlPoint(scratch3);
										Point p2b = scratch4;
										...
										newScore.value += getOverlap(p1, p2, p1b, p2b);
										newScore.value += getOverlap(p1, p2, p2b, p3b);
										newScore.value += getOverlap(p2, p3, p1b, p2b);
										newScore.value += getOverlap(p2, p3, p2b, p3b);
									}
								}
							}
						}
							...
							for(int b = a; b<allEdges.size(); b++) {
								...
								for(int j = 0; j<otherJoins.length; j++) {
									if(otherJoins[j].connector!=joins[i].connector) {
										...
										newScore.value += isIntersection(p1, p2, p1b, p2b) ? 1 : 0;
										newScore.value += isIntersection(p1, p2, p2b, p3b) ? 1 : 0;
										newScore.value += isIntersection(p2, p3, p1b, p2b) ? 1 : 0;
										newScore.value += isIntersection(p2, p3, p2b, p3b) ? 1 : 0;
									}
								}
							}
						}
		...
		private static boolean isIntersection(Point p1, Point p2, Point p1b, Point p2b)
		{
			...
			if(minX<=otherX && otherX<=maxX && minY<=otherY && otherY<=maxY)
				return true;
			return false;
		}
		...
		/** Calculate how many pixels overlap between the Rectangle and line segment.
		 * 
		 */
		...
		public Collection<? extends Shape> getShapes(boolean forceRegeneration) {
			if(connectorShapes.size()==0 || forceRegeneration) {
				...
				for(BoxEdge edge : allEdges) {
					...
					for(int a = 0; a<joins.length; a++) {
						Point center = joins[a].connector.getControlPoint(false);
						Point joinPoint;
						if(!edge.side.isLeftOrRight()) {
						...
						if(points==null) {
							...
							//make sure they're ordered so we get the decoration correct
							if(!joins[a].first)
				...
				for(Connector connector : pointMap.keySet()) {
					...
					for(int a = 1; a<points.size(); a++) {
						...
						if(a!=points.size()-1) {
							...
							Point2D p1 = midway(prev, current, prev.distance(current)-myRadius1 );
							Point2D p1c = midway(prev, current, prev.distance(current)-myRadius1*1/4 );
							Point2D p2c = midway(current, next, myRadius2*1/4 );
							Point2D p2 = midway(current, next, myRadius2 );
							path.lineTo(p1.getX(), p1.getY());
							path.curveTo( p1c.getX(), p1c.getY(), p2c.getX(), p2c.getY(), p2.getX(), p2.getY());
						} else {
							path.lineTo(current.x, current.y );
						}
					}
					
					BinaryRelationship relationship = connector.getRelationship();
					if(ui!=null) {
						ui.addConnectorDecoration(path, relationship.relationshipSideA, points.get(0), points.get(1));
						ui.addConnectorDecoration(path, relationship.relationshipSideB, points.get(points.size()-1), points.get(points.size()-2));
					} else {
						relationship.relationshipSideA.appendDecoration(path, points.get(0), points.get(1));
						relationship.relationshipSideB.appendDecoration(path, points.get(points.size()-1), points.get(points.size()-2));
					}
		...
		private Point2D midway(Point2D p1,Point2D p2,double distance) {
			...
			return new Point2D.Double( p1.getX()+distance*Math.cos(theta), p1.getY()+distance*Math.sin(theta) );
		}
	}
	 ...
	 * @return
	 */
	private static Join[] calculatePossibleJoins(Connector connector,boolean first) {
		...
		if(r.contains(p)) {
			...
			double topDistance = (new Point2D.Double(r.getCenterX(), r.getMinY())).distance(p);
			double bottomDistance = (new Point2D.Double(r.getCenterX(), r.getMaxY())).distance(p);
			double leftDistance = (new Point2D.Double(r.getMinX(), r.getCenterY())).distance(p);
			double rightDistance = (new Point2D.Double(r.getMaxX(), r.getCenterY())).distance(p);
			double min = Math.min( Math.min(topDistance, bottomDistance), Math.min(leftDistance, rightDistance));
			if(min==topDistance) {
	...
	@Override
	protected void paintHandles(Graphics2D g,BoxContainerPanel bcp, List<Connector> connectors, List<Box> boxes)
	{
		g = (Graphics2D)g.create();
		super.paintHandles(g, bcp, connectors, boxes);
		for(Connector connector : connectors) {
			Point p1 = connector.get(KEY_JOIN1_CONTROL_POINT);
			if(p1!=null)
				paintEllipseHandle(g, p1);
			Point p2 = connector.get(KEY_JOIN2_CONTROL_POINT);
			if(p2!=null)
				paintEllipseHandle(g, p2);
		}
	}
