...
import com.bric.window.WindowDragger;
...
public class PreferencePanel extends JPanel {
		...
		public void actionPerformed(ActionEvent e) {
			...
			for(int a = 0; a<rows.size(); a++) {
				...
				for(int b = 0; b<row.buttons.length; b++) {
					...
					row.buttons[b].setSelected( rows.size()==1 && row.buttons[b]==button );
				}
			}
		}
	};
	...
	public PreferencePanel(String defaultTitle) {
		...
		c.insets = new Insets(2,2,2,0); c.fill = GridBagConstraints.VERTICAL;
		header.add(prevButton,c);
		c.gridx++;
		c.insets = new Insets(2,0,2,2);
		header.add(nextButton,c);
		c.gridx++; c.insets = new Insets(2,2,2,2);
		header.add(homeButton,c);
		c.gridx++;
		JPanel fluff = new JPanel();
		fluff.setOpaque(false); c.weightx = 1;
		header.add(fluff,c);
		repack();
		
		if(isMac) {
			homeButton.putClientProperty("JButton.buttonType", "segmentedTextured");
		} else {
			homeButton.setUI(new TexturedButtonUI());
		}
	...
	private void layoutHomePanel() {
		...
		for(int a = 0; a<rows.size(); a++) {
			...
			c.gridwidth = GridBagConstraints.REMAINDER; c.gridheight = 2;
		...
		maxContentSize.height = Math.max(prefSize.height, maxContentSize.height);
	}
	...
	private void repack() {
		...
		if(w!=null) {
			//TODO: this feature never really got off the ground, nor was it essential.
			//if(animateResizing) {
	...
	Runnable layoutRunnable = new Runnable() {
		public void run() {
			...
			if(rows.size()==1) {
				/** We're supposed to show 1 row, and that row always
				 * stays visible.
				 */
				...
				if(firstRow.getParent()==PreferencePanel.this) {
					...
					c.gridx = 0; c.gridy = 0;
					c.gridwidth = GridBagConstraints.REMAINDER;
					c.fill = GridBagConstraints.BOTH; c.weightx = 1; c.weighty = 0;
					add(header,c);
					c.gridy++;
					add(separator,c);
					c.gridy++; c.weighty = 1;
	...
	/** Define the insets used to pad the component buttons.
	 * <BR>(This property is only used when more than 1
	 * row is visible.  If only 1 row is used, then the
	 * buttons are wrapped in a JToolBar, and the default
	...
	/** Define the insets used to pad the row labels.
	 * <BR>(This property is only used when labels are visible,
	 * which only occurs when there is more than 1 row.)
	 */
	...
	/** Returns the insets used to pad the row labels.
	 * <BR>(This property is only used when labels are visible,
	 * which only occurs when there is more than 1 row.)
	 */
	...
	/** Returns the insets used to pad the component buttons.
	 * <BR>(This property is only used when more than 1
	 * row is visible.  If only 1 row is used, then the
	 * buttons are wrapped in a JToolBar, and the default
	 * look-and-feel behavior for that toolbar is used.)
	...
	class Row extends JPanel {
		...
		public Row(AbstractButton[] inButtons,JComponent[] inComponents,String inTitle) 
		{
			...
			for(int a = 0; a<buttons.length; a++) {
				...
				if(components[a]==null)
					throw new NullPointerException("components["+a+"] = null");
				...
				maxContentSize.width = Math.max( maxContentSize.width, prefSize.width);
				maxContentSize.height = Math.max( maxContentSize.height, prefSize.height);
			}
	...
	/** Indicates whether this panel should stay at a constant
	 * size, regardless of which preference is selected.
	 */
	public boolean isFixedSize() {
		return fixedSize;
	}
	
	/** Controls whether this panel is always sized to fit the largest
	 * possible component.  If this is true, then no matter how
	 * the panel is adjusted it should not change its preferred size.
	 * If this is false, then depending on which control is visible the
	 * preferred size of this panel may change.
	 */
	...
	public JDialog createDialog(Frame parent,String name) { 
		...
		InputMap inputMap = d.getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW);
