...
import com.bric.plaf.ThumbnailLabelUI;
...
public class IOLocationTileList extends JList {
	...
	private PropertyChangeListener graphicListener = new PropertyChangeListener() {
		public void propertyChange(PropertyChangeEvent evt) {
			if(evt.getPropertyName().equals(GraphicCache.THUMBNAIL_PROPERTY)) {
				...
				SwingUtilities.invokeLater( new RepaintLocationRunnable( loc, true ) );
			} else if(evt.getPropertyName().equals(GraphicCache.ICON_PROPERTY)) {
				IOLocation loc = (IOLocation)evt.getSource();
				SwingUtilities.invokeLater( new RepaintLocationRunnable( loc, false) );
			}
		}
	};
	...
	protected MouseListener commitMouseListener = new MouseAdapter() {
		...
		public void mouseClicked(MouseEvent e) {
			/* Give other listeners a chance to react
			 * to this mouse event by making this
			 * an invoke-later runnable:
			 */
			class ClickRunnable implements Runnable {
				...
				public void run() {
					int[] indices = IOLocationTileList.this.getSelectedIndices();
					
					int index = getUI().locationToIndex(IOLocationTileList.this, mouseEvent.getPoint());
					if(index==-1) return;
					...
					if(mouseEvent.getClickCount()==1) {
						fireIOLocationListListeners( NavigationListener.ListSelectionType.SINGLE_CLICK, getSelectedLocations() );
					} else if(mouseEvent.getClickCount()==2) {
						fireIOLocationListListeners( NavigationListener.ListSelectionType.DOUBLE_CLICK, getSelectedLocations() );
					}
				}
			}
	...
	/** This filter may be used by renderers to show unsupported
	 * locations as disabled. (For example, see the BasicTileCellRenderer.)
	 * <p>Note this does not actually filter elements out of this list,
	 * for that you need to call: <code>IOLocationFilter.filter(srcList)</code>
	 * 
	 * @param filter
	 */
	...
	public void addIOLocationListListener(NavigationListener<IOLocation> listener) {
		if(listeners.contains(listener))
			return;
		listeners.add(listener);
	}
	
	public void removeIOLocationListListener(NavigationListener<IOLocation> listener) {
		listeners.remove(listener);
	}
	
	protected void fireIOLocationListListeners(NavigationListener.ListSelectionType type, IOLocation... locations ) {
		for(NavigationListener<IOLocation> listener : listeners) {
			try {
				if(listener.elementsSelected( type, Arrays.copyOf(locations, locations.length) ))
					return;
			} catch(Exception e) {
				e.printStackTrace();
			}
	...
	public static class BasicTileCellRenderer implements ListCellRenderer {
		...
		public BasicTileCellRenderer(GraphicCache graphicCache,JLabel thumbnail) {
			if(graphicCache==null) graphicCache = new GraphicCache();
		...
		/** This lets subclasses format this label. The default implementation does nothing,
		 * but subclasses can override this to meddle with the text/icon/etc before the
		 * renderer returns this label.
		 */
	...
	/** This calculates the number of required rows
	 * based on a width and the fixed cell width.
	 * Then this calls <code>setVisibleRowCount(rows)</code>
	 * 
	 * @param width
	 */
	public void updateVisibleRowCount(int width) {
		...
		if(lastRows!=rows || lastColumns!=columns) {
			setPreferredSize( new Dimension( columns*getFixedCellWidth(), rows*getFixedCellHeight()) );
			setVisibleRowCount(-1);
		}
		lastRows = rows;
		lastColumns = columns;
