 ...
 * <p>This should replicate the icon observed on Mac OS X 10.7.4 for audio
 * files.
 * <p>Also this icon contains a <code>PlayToggleIcon</code> (which is set to
 * zero opacity initially) that can be used to emulated the playback controls
...
@ResourceSample ( sample = {
		"new com.bric.swing.resources.MusicIcon(24)",
		"new com.bric.swing.resources.MusicIcon(32)",
		"new com.bric.swing.resources.MusicIcon(64)",
		"new com.bric.swing.resources.MusicIcon(128)" },
		names = {
		"24x24", "32x32", "64x64", "128x128" } )
public class MusicIcon implements Icon {
	
	/** This is a circular play/pause icon, typically displayed in the
	 * center of a <code>MusicIcon</code>. This also supports an optional
	 * circular track that depicts sound completion from 0% to 100% by
	 * painting an arc that completes clockwise from the 12:00 position.
	 * <p>The is associated with the <code>MusicIcon</code> by convention,
	 * but this could be applied to other settings, too.
	 */
	public static class PlayToggleIcon implements Icon {
		...
		final static Icon playIcon = new TriangleIcon(SwingConstants.EAST, 12, 12, Color.white);
		final static Icon pressedPauseIcon = new PauseIcon(12, 12, new Color(160, 160, 160));
		final static Icon pressedPlayIcon = new TriangleIcon(SwingConstants.EAST, 12, 12, new Color(160, 160, 160));
		 ...
		 * @param width the width and height.
		 */
		 ...
		 * @param f the completion (from [0,1])
		 */
		public void setTrackCompletion(float f) {
			if(f<0 || f>1) throw new IllegalArgumentException("The track completion ("+f+") must be between zero and one.");
			completion = f;
		}
		 ...
		 * @param f the new track size (from [0,1])
		 */
		public void setTrackSize(float f) {
			if(f<0 || f>1) throw new IllegalArgumentException("The track size ("+f+") must be between zero and one.");
			trackSize = f;
		}
		 ...
		 * @param f the new opacity (from [0,1])
		 */
		public void setOpacity(float f) {
			if(f<0 || f>1) throw new IllegalArgumentException("The opacity ("+f+") must be between zero and one.");
			opacity = f;
		}
		 ...
		 * @param b whether the pause icon is visible
		 */
		public void setPauseIconVisible(boolean b) {
			pauseVisible = b;
		}
		
		/** @return true if the play/pause icon is rendered as pressed.
		 * (When pressed, those icons are a little bit darker.)
		 */
		public boolean isPressed() {
			return pressed;
		}

		/** Controls whether the play/pause icon is rendered as pressed.
		 * (When pressed, those icons are a little bit darker.)
		...
		private void paintIcon(Graphics2D g) {
			...
			//the shadow:
			ellipse.setFrameFromCenter(centerX, centerY, centerX + radiusOuter + 3, centerY + radiusOuter + 3 );
			g.setStroke(new BasicStroke(1));
			g.setColor(new Color(0,0,0,20));
			g.draw(ellipse);

			ellipse.setFrameFromCenter(centerX, centerY, centerX + radiusOuter + 2, centerY + radiusOuter + 2 );
			g.setColor(new Color(0,0,0,50));
			g.draw(ellipse);
			
			ellipse.setFrameFromCenter(centerX, centerY, centerX + radiusOuter + 1, centerY + radiusOuter + 1 );
			g.setColor(new Color(0,0,0,90));
			g.draw(ellipse);
			
			//the fill
			ellipse.setFrameFromCenter(centerX, centerY, centerX + radiusInner, centerY + radiusInner);
			...
			arc.setArc(centerX - radiusMiddle, centerY - radiusMiddle, 2*radiusMiddle, 2*radiusMiddle, 90, -completion * 360, Arc2D.OPEN);
			g.setColor(new Color(128, 128, 128, 200));
			g.setStroke(new BasicStroke(diff, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER));
			g.draw(arc);
			...
			ellipse.setFrameFromCenter(centerX, centerY, centerX + radiusOuter, centerY + radiusOuter );
	...
	/** A simple minimal app to demo the <code>MusicIcon</code>, and the <code>PlayToggleIcon</code>.
	...
	public static void main(String[] args) {
		...
		pauseCheckbox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				musicIcon.getPlayToggleIcon().setPauseIconVisible( pauseCheckbox.isSelected() );
				label.repaint();
			}
		});
		...
		pressedCheckbox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				musicIcon.getPlayToggleIcon().setPressed( pressedCheckbox.isSelected() );
				label.repaint();
			}
		});
		...
		pauseCheckbox.setToolTipText("Toggle the icon between \"play\" and \"pause\".");
		pressedCheckbox.setToolTipText("Toggle the state between pressed and default.");
		completionSlider.setToolTipText("Adjust the fraction of track completion shown.");
	...
	protected static Shape createEighthNotesShape() {
		GeneralPath circle1 = ShapeStringUtils.createGeneralPath("m 111.113 175.531 c 111.173 184.2 102.183 191.65 91.034 192.171 c 79.885 192.692 70.798 186.087 70.738 177.419 c 70.678 168.75 79.668 161.3 90.817 160.779 c 101.966 160.257 111.053 166.862 111.113 175.531 z");
		GeneralPath circle2 = ShapeStringUtils.createGeneralPath("m 173.579 164.139 c 173.638 172.808 165.467 180.258 154.318 180.779 c 143.169 181.3 134.082 174.695 134.022 166.027 c 133.962 157.358 142.952 149.908 154.101 149.387 c 165.25 148.865 173.519 155.47 173.579 164.139 z");
		GeneralPath polygon = ShapeStringUtils.createGeneralPath("m 99.31 73.824 l 173.592 62.328 l 173.592 163.322 l 164.175 174.311 l 164.175 95.261 l 111.188 103.947 l 111.188 175.11 l 99.31 183.286 l 99.31 73.824 z");
	...
	/** Create a <code>MusicIcon</code> that is 256x256, with a <code>PlayToggleIcon</code> that is 48x48. */
	public MusicIcon() {
		this(DEFAULT_WIDTH, DEFAULT_WIDTH, PlayToggleIcon.DEFAULT_WIDTH, PlayToggleIcon.DEFAULT_WIDTH);
	}

	/** Create a square <code>MusicIcon</code>, with a <code>PlayToggleIcon</code> that is 48x48. 
	 * @param width the width and height of this icon.
	 */
	 ...
	 * @param playButtonWidth the width of the <code>PlayToggleIcon</code>.
	 * @param playButtonHeight the height of the <code>PlayToggleIcon</code>.
	 */
	public MusicIcon(int width,int height,int playButtonWidth,int playButtonHeight) {
		this.width = width;
		this.height = height;
	...
	/** @return a <code>PlayToggleIcon</code> that is overlayed on this <code>MusicIcon</code>.
	 * By default this icon control has an opacity of zero, so it is never painted.
	 */
	...
	public void paintIcon(Component c, Graphics g0, int x, int y) {
		...
		g.setPaint(new RadialGradientPaint(
			new Point2D.Float(128,-20), 270, new float[] {0, .3f, .6f, .9f},
			new Color[] {
		...
		//the shape
		g.setPaint(new GradientPaint( 80, 190,
				new Color(105, 122, 141),
				170, 65,
