...
import com.bric.plaf.UIEffect;
import com.bric.swing.resources.TriangleIcon;

public class InspectorLayout {
	
	private static final String ROTATION = InspectorLayout.class.getName()+".rotation";
	private static final String EFFECT = InspectorLayout.class.getName()+".effect";
			...
			addChangeListener(new ChangeListener() {
				public void stateChanged(ChangeEvent e) {
					...
					if(AnimateTriangleEffect.this.expanded) {
						getComponent().putClientProperty(ROTATION, new Double( progress*(Math.PI/2) ) );
					} else {
						getComponent().putClientProperty(ROTATION, new Double( (1-progress)*(Math.PI/2) ) );
					}
				}
			});
		}
	...
	private class HeaderUI extends BasicButtonUI {
		...
		boolean clickable;
		TriangleIcon triangleIcon = new TriangleIcon( SwingConstants.EAST, 10, 10 );
		...
		public void paint(Graphics g0, JComponent c) {
			...
			if(c.isOpaque()) {
				if(clickable && c instanceof AbstractButton && ((AbstractButton)c).getModel().isArmed()) {
					g.setPaint(new GradientPaint( new Point(0, 0), new Color(230, 230, 230),
							new Point(0, c.getHeight()), new Color(180, 180, 180) ));
				} else {
					g.setPaint(new GradientPaint( new Point(0, 0), Color.white,
							new Point(0, c.getHeight()), new Color(216, 216, 216) ));
				}
				g.fillRect(0, 0, c.getWidth(), c.getHeight());
				
				Rectangle border = new Rectangle(0, 0, c.getWidth()-1, c.getHeight()-1);
				...
				if(c.hasFocus()) {
					Shape rect = new RoundRectangle2D.Float(border.x, border.y, border.width, border.height, 8, 8);
					com.bric.plaf.PlafPaintUtils.paintFocus(g, rect, 2);
				}
			...
			if(clickable) {
				...
				triangleIcon.paintIcon(c, g2, 
						15 - triangleIcon.getIconWidth()/2, 
						c.getHeight()/2 - triangleIcon.getIconHeight()/2);
				g2.dispose();
			}
		}
	...
	public class Header extends JButton {
		...
		private int[] getRange() {
			...
			int endIndex = -1;
			for(int a = startIndex; a<rowLayout.getRowCount() && endIndex==-1; a++) {
				JComponent[] jc = rowLayout.getRow(a);
			...
			if(startIndex==-1) {
				throw new IllegalStateException("this header ("+getText()+") was not found");
			}
	...
	public InspectorLayout() {
		rowLayout.addRowType(TWO_COLUMN_INSPECTOR, new Cell[] {
				new Cell(0, .5f, columnInsets),
				new Cell(1, .5f, columnInsets )
		});
		rowLayout.addRowType(SINGLE_COLUMN_NO_INSETS, new Cell[] {
				new Cell(1, .5f, new Insets(0,0,0,0) )
		});
	}
	 ...
	 * Note this component is stretched to fill the entire row; so
	 * the insets parameter can be used to help provide a more balanced
	 * appearance.
	 * 
	 * @param component the component to add.
	 * @param alignment one of the alignment enums: LEFT, CENTER, RIGHT, or STRETCH.
	 * @param insets and optional set of additional insets to apply to the component.
	 */
	public void addRow(JComponent component,HorizontalAlignment alignment,Insets insets,int verticalPriority) {
		ComponentCluster cluster = createAlignedCluster(component, alignment, insets, verticalPriority);
		rowLayout.addRow( cluster , SINGLE_COLUMN_NO_INSETS);
	}
	
	private ComponentCluster createAlignedCluster(JComponent component, HorizontalAlignment alignment,Insets insets,int verticalPriority) {
		if(HorizontalAlignment.STRETCH.equals(alignment)) {
			return new ComponentCluster( new JComponent[] { component }, 
					new ComponentConstraints[] { new ComponentConstraints(1, ComponentConstraints.HorizontalAlignment.LEFT, insets, verticalPriority) });
		} else if(HorizontalAlignment.LEFT.equals(alignment)) {
			return new ComponentCluster( new JComponent[] { component, null }, 
					new ComponentConstraints[] { 
						new ComponentConstraints(0, ComponentConstraints.HorizontalAlignment.LEFT, insets, verticalPriority),
						new ComponentConstraints(1, ComponentConstraints.HorizontalAlignment.LEFT, null, 0) });
		} else if(HorizontalAlignment.RIGHT.equals(alignment)) {
			return new ComponentCluster( new JComponent[] { null, component }, 
					new ComponentConstraints[] { 
						new ComponentConstraints(1, ComponentConstraints.HorizontalAlignment.RIGHT, null, 0),
						new ComponentConstraints(0, ComponentConstraints.HorizontalAlignment.RIGHT, insets, verticalPriority) });
		} else if(HorizontalAlignment.CENTER.equals(alignment)) {
			return new ComponentCluster( new JComponent[] { null, component, null } , 
					new ComponentConstraints[] { new ComponentConstraints(1, null, null, 0),
					new ComponentConstraints(0, null, insets, verticalPriority),
	 ...
	 * 
	 * The identifier is right-aligned, and the control's alignment
	 * is defined by the <code>alignment</code> argument.
	 * 
	 ...
	 * @param alignment how to position the control.
	 * @param insets an optional set of insets for the control.
	 */
	public void addRow(JComponent identifier,JComponent control,HorizontalAlignment alignment,Insets insets) {
		rowLayout.addRow(ComponentCluster.createRightAligned(identifier), createAlignedCluster(control, alignment, insets, 0), currentSectionID);
	}
	 ...
	 * @param identifier the control on the left.  This should usually contain text.
	 * A <code>JLabel</code> or a <code>JCheckBox</code> is recommended.
	 * @param controls the controls to add to this row.
	 */
	public void addRow(JComponent identifier,ComponentCluster controls) {
		rowLayout.addRow(ComponentCluster.createRightAligned(identifier), controls, currentSectionID);
	}
	
	/** Appends a new row containing 3 objects to this inspector.
	 * 
	 * The identifier is right-aligned.  The leftControl is
	 * left-aligned and occupies the remaining width, and 
	 * the rightControl is right-aligned against the far right
	 * margin and is as small as possible.
	 * 
	 * @param identifier the control on the left.  This should usually contain text.
	 * A <code>JLabel</code> or a <code>JCheckBox</code> is recommended.
	...
	public void addRow(JComponent identifier,JComponent leftControl,boolean stretchToFill,JComponent rightControl) {
		...
		JComponent[] rightClusterComps = new JComponent[] { leftControl, rightControl };
		if(stretchToFill) {
			rightCluster = new ComponentCluster(rightClusterComps, 
					new ComponentConstraints[] { new ComponentConstraints( 1, ComponentConstraints.HorizontalAlignment.LEFT, new Insets(0,0,0,2), 0 ),
					new ComponentConstraints(0, ComponentConstraints.HorizontalAlignment.RIGHT, null, 0 ) });
		} else {
			rightCluster = new ComponentCluster(rightClusterComps,
					new ComponentConstraints[] { new ComponentConstraints( 0, ComponentConstraints.HorizontalAlignment.LEFT, new Insets(0,0,0,2), 0 ),
					new ComponentConstraints(0, ComponentConstraints.HorizontalAlignment.RIGHT, null, 0 ) });
		}
		
		rowLayout.addRow(ComponentCluster.createRightAligned(identifier), rightCluster, currentSectionID);
