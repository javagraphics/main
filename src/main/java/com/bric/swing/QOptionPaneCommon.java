...
import com.bric.plaf.BasicQOptionPaneUI;
...
public class QOptionPaneCommon {
	...
	private static QOptionPane createSaveDialog(String appName,String documentName,String filePath,boolean saveProducesDialog,int fileState) {
		...
		if(fileState==FILE_EXTERNAL_CHANGES) {
			mainMessage = Text.replace(strings.getString("dialogMacSaveExternalChangesMessage"), "^0", documentName);
			...
			if(JVM.isMac) {
				mainMessage = Text.replace(strings.getString("dialogMacSavePromptMessage"), "^0", documentName);
				...
				mainMessage = Text.replace(mainMessage, "^0", documentString);
				comment = null;
			}
			if(JVM.isMac)
	...
	private static QOptionPane createReviewChangesDialog(String appName,int documentCount) {
		...
		mainMessage = Text.replace(mainMessage, "^0", Integer.toString(documentCount));
		mainMessage = Text.replace(mainMessage, "^1", appName);
