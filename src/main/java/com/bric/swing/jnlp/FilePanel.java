...
import com.bric.swing.FileIcon;
import com.bric.util.JVM;

/** A panel that helps select a file.
 * <p>If launched in a JNLP, then this uses the local FileOpenService to select
 * a File. Otherwise this uses the standard FileDialog (which requires this
...
public class FilePanel extends JPanel {
	 ...
	 * <p>This may be backed by a java.io.File object, or a FileContents object
	 * in JNLP sessions.
	 */
	...
	public static class FileContentsWrapper extends FileData {
		...
		public FileContentsWrapper(FileContents contents) {
	...
	/** Creates a FilePanel.
	 * 
	 * @param fileID the text in the identifying label on the left. Usually this will read "File:",
	 * or something similar.
	 * @param extensions the file extensions this panel supports.
	 * @param otherComponents additional components to put next to the "Browse..." button (such as a throbber, or other buttons)
	 */
	public FilePanel(String fileID,String[] extensions,JComponent... otherComponents) {
	 ...
	 * 
	 * @param ext may be null, in which case no file filter is applied.
	 */
	public void setExtensions(String... ext) {
		putClientProperty(EXTENSIONS_KEY, clone(ext));
	}

	/** Return the file extensions accepted in file dialogs. This array
	 * may be null if not file filter is applied.
	 */
	...
	/** Return the right panel, containing the file label, text field, and browse button. */
	public JPanel getRightPanel() {
		return rightPanel;
	}
	
	/** Return the identifying label on the left that labels this file. This should
	 * contain text like "File:" or "Source:", but is not going to mention the file name.
	 */
	public JLabel getIDLabel() {
		return idLabel;
	}
	
	/** Return the label representing the currently selected file. This will
	 * be visible if <code>isEditable()</code> returns false.
	 */
	public JLabel getFileLabel() {
		return fileLabel;
	}
	
	/** Returns the "Browse..." button. This will be enabled if it is attached
	 *  to a Frame.
	 */
