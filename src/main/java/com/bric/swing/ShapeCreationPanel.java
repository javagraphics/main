 ...
 * <p>There are a few different <code>ShapeCreationUIs</code> to choose
 * from. Once the appropriate UI is installed: just call
 * <code>shapePanel.setMode( MODE_CREATE )</code> to the user to be
 * able to draw shapes in this panel.
...
public class ShapeCreationPanel extends JComponent {
	...
	public static enum Active { 
		 ...
		 * @param scp the panel we're checking against
		 * @param shapeIndex the index of the shape we're checking
		 */
	...
	public static final String SELECTION_MODEL_KEY = ShapeCreationPanel.class.getName()+".selection-model";

	/** The client property key used to identify the <code>DataModel</code>.
	 * 
	 * @see #setDataModel(DataModel) */
	public static final String DATA_MODEL_KEY = ShapeCreationPanel.class.getName()+".data-model";

	/** The client property key used to identify the <code>AffineTransform</code>.
	 * 
	 * @see #setTransform(AffineTransform) */
	public static final String TRANSFORM_KEY = ShapeCreationPanel.class.getName()+".transform";

	/** The client property key used to identify the handle size.
	 * 
	 * @see #setHandleSize(int) */
    public static final String HANDLE_SIZE_KEY = ShapeCreationPanel.class.getName()+".handle-size-key";

	/** The client property key used to identify which handles are active.
	 * 
	 * @see #setHandlesActive(Active) */
    public static final String HANDLES_ACTIVE_KEY = ShapeCreationPanel.class.getName()+".handle-active";
    

	/** The client property key used to identify the mode.
	 * 
	 * @see #setMode(String) */
    public static final String MODE_KEY = ShapeCreationPanel.class.getName()+".mode";
    
    /** This is default mode for this panel. In this mode shapes can usually be selected,
     * translated, and deleted: but not created.
     * 
     * @see #getMode()
     */
    public static final String MODE_DEFAULT = "default";
    
    /** This is the mode used exclusively for creating shapes. In this mode when the
     * user clicks in this panel: new shapes are added to the data model.
     * 
     * @see #getMode()
     */
    public static final String MODE_CREATE = "create";
	
    /** A small on-screen handle. Note each <code>ShapePanelUI</code>
     * renders and interacts with these controls differently. (Some UIs
     * may not support any handle at all, others may only support the
     * PRIMARY handle.)
     * @see #getHandleSize()
     */
	public static enum Handle {
		...
		PRIMARY, 
		/** If this node represents cubic bezier data,
		 * this optional handle represents the previous control point. 
		 */
		 PREVIOUS_CONTROL, 
		/** If this node represents cubic bezier data,
		 * this optional handle represents the next control point. */
		 NEXT_CONTROL;
	}
	...
	public static interface SelectionModelListener {
		 ...
		 * @param shapePanel the shape panel whose selection changed.
		 * @param oldSelection the previous selection
		 * @param newSelection the new selection
		 */
		public void selectionChanged(ShapeCreationPanel shapePanel,Selection oldSelection,Selection newSelection);

		/** Notification that the indication has changed. 
		 * @param shapePanel the shape panel whose selection changed.
		 * @param oldIndication the previous indication
		 * @param newIndication the new indication
		 */
		public void indicationChanged(ShapeCreationPanel shapePanel,Selection oldIndication,Selection newIndication);
	}
	
	/** Data used to articulate exactly what the user has selected (or indicated).
	 * Not all UIs will support identifying all the information available in this class.
	 */
	public static class Selection {
		 ...
		 * This is equivalent to <code>new Selection(-1, -1, null)</code>.
		 */
		public Selection() {
			this(-1, -1, null);
		}
		 ...
		 * 
		 * @param shapeIndex the shape index being interacted with, or -1 if no
		 * shape is involved.
		 * @param nodeIndex the node index being interacted with, or -1 if no
		 * node is involved.
		 * @param handle the handle being interacted with. If null this is reset
		 * to PRIMARY, although it is not consulted if the nodeIndex is -1.
		 */
		public Selection(int shapeIndex,int nodeIndex,Handle handle) {
			if(handle==null) handle = Handle.PRIMARY;
		...
		/** @return the node index (relative to the shape index, which is relative to the {@link DataModel}).
		 */
		public int getNodeIndex() {
			return nodeIndex;
		}
		
		/** @return the handle (relative to the node index, which is relative to the shape index).
		 */
		...
		public String toString() {
			return "Selection[ shapeIndex="+shapeIndex+", nodeIndex="+nodeIndex+", handle="+handle+"]";
		}
	}
	
	/** This manages the selection and indication of a {@link ShapeCreationPanel}.
	 * <p>The "selection" probably needs to explanation, but "indication" is
	 * a less universal concept. Also known as "rollover" or other possible names,
	 * Jef Raskin defines indication as, "highlighting the single object pointed 
	 * to as the cursor is moved, without any other user action such as clicking."
	 *
	 */
	public static class SelectionModel {
		 ...
		 * @param shapePanel the panel this model relates to. 
		 */
		 ...
		 * <p>The "selection" probably needs no explanation, but "indication" is
		 * a less universal concept. Also known as "rollover" or other possible names,
		 * Jef Raskin defines indication as, "highlighting the single object pointed 
		 * to as the cursor is moved, without any other user action such as clicking."
		 */
		...
		/** Change the current selection, and notify listeners if a change occurred.
		 * @param newSelection the new selection
		 */
		public void select(Selection newSelection) {
			if(newSelection==null) throw new NullPointerException();
			if(! newSelection.equals(selection)) {
				Selection oldSelection = selection;
				selection = newSelection;
				for(SelectionModelListener l : listeners) {
					try {
						l.selectionChanged(shapePanel, oldSelection, newSelection);
					} catch(Exception e) {
						e.printStackTrace();
					}
				}
		...
		/** Change the current indication, and notify listeners if a change occurred.
		 * @param newIndication the new indication
		 */
		public void indicate(Selection newIndication) {
			if(newIndication==null) throw new NullPointerException();
			if(! newIndication.equals(indication)) {
				Selection oldIndication = indication;
				indication = newIndication;
				for(SelectionModelListener l : listeners) {
					try {
						l.indicationChanged(shapePanel, oldIndication, newIndication);
					} catch(Exception e) {
						e.printStackTrace();
					}
				}
		...
		/** Change the current selection, and notify listeners if a change occurred.
		 * @param shapeIndex the index of the shape to select
		 * @param nodeIndex the index of the node within the shape to select
		 * @param handle the handle (of the node of the shape) to select.
		 */
		public void select(int shapeIndex,int nodeIndex,Handle handle) {
			Selection newSelection = new Selection(shapeIndex, nodeIndex, handle);
			select(newSelection);
		}

		/** Change the current indication, and notify listeners if a change occurred.
		 * @param shapeIndex the index of the shape to indicate
		 * @param nodeIndex the index of the node within the shape to indicate
		 * @param handle the handle (of the node of the shape) to indicate.
		 */
		public void indicate(int shapeIndex,int nodeIndex,Handle handle) {
			Selection newIndication = new Selection(shapeIndex, nodeIndex, handle);
			indicate(newIndication);
		}
	}
	
	/** A simple wrapper for a shape that makes it immutable.
	 * <p>(That is: if we simply stored data as a GeneralPath then
	 * other entities may invoke <code>gpath.reset()</code>, or other
	...
	public interface DataModelListener {
		 ...
		 * @param shapePanel the affected panel.
		 * @param shapeIndex the index of the shape that was added.
		 * @param shape the shape that was added.
		 */
		public void shapeAdded(ShapeCreationPanel shapePanel,int shapeIndex,Shape shape);
		/** Notification that a shape was removed. 
		 * @param shapePanel the affected panel.
		 * @param shapeIndex the index of the shape that was removed.
		 * @param shape the shape that was removed.
		 */
		public void shapeRemoved(ShapeCreationPanel shapePanel,int shapeIndex,Shape shape);
		/** Notification that a shape was changed. 
		 * @param shapePanel the affected panel.
		 * @param shapeIndex the index of the shape that was changed.
		 * @param shape the shape that was changed.
		 */
		public void shapeChanged(ShapeCreationPanel shapePanel,int shapeIndex,Shape shape);
	}
	
	/** The data a {@link ShapeCreationPanel} represents. Specifically this
	 * is a list of shapes. The shapes should generally be considered
	 * immutable. Instead of changing a shape: simply replace it.
	 */
	public static class DataModel {
		 ...
		 * @param shapePanel the related panel.
		 */
		 ...
		 * @param shapeIndex the index of the shape to replace
	...
	/** @return the {@link SelectionModel} used to monitor the
	 * selection and indication of this panel.
	 * 
	 * @see #setSelectionModel(SelectionModel)
	 */
	 ...
	 * 
	 * @param selectionModel the new selection model for this panel.
	...
	/** @return the {@link DataModel} that stores all the shape
	 * data in this panel.
	 * 
	 * @see #setDataModel(DataModel)
	 */
	 ...
	 * 
	 * @param dataModel the new DataModel for this panel.
	 * @see #getDataModel()
	 */
    ...
    public AffineTransform getTransform() {
    	...
    	if(t==null) return new AffineTransform();
    	return new AffineTransform(t);
    }

    /** Assign the transform these shapes are rendered through.
     * 
     * @param transform the new transform these shapes are rendered through.
     * @see #getTransform()
     */
    ...
    /** @return the size of handles (in pixels). This is not affected
     * by the transform.
     * 
     * @see #setHandleSize(int)
     */
     ...
     * 
     * @param i the new handle size.
     * 
     * @see #getHandleSize()
     */
	public void setHandleSize(int i) {
		//if your goal is to turn off handles, use: setHandlesActive(Active.OFF)
		if(i<=0) throw new IllegalArgumentException("handleSize must be positive");
	...
	public Active getHandlesActive() {
		...
		if(value==null) return Active.SELECTED;
		return value;
	}

	/** Control which handles should be active.
	 * 
	 * @param a the new state of the handles
	...
	public String getMode() {
		...
		if(value==null) return MODE_DEFAULT;
		return value;
	}

	/** Assign the current mode of this panel.
	 * @param mode the new mode of this panel
	 * @see #MODE_DEFAULT
	 * @see #MODE_CREATE
	 */
	...
	public void updateUI() {
    	if(UIManager.getDefaults().get(uiClassID)==null) {
    		UIManager.getDefaults().put(uiClassID, "com.bric.plaf.CubicPathCreationUI");
    	}
