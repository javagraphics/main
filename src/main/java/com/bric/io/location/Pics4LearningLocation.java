...
import com.bric.swing.io.DefaultSearchResults;
...
public class Pics4LearningLocation extends CachedLocation {
	...
	private Pics4LearningLocation(IOLocation parent,String name,String url,String thumbnailURL) {
		this.parent = parent;
		this.name = name;
		if(url!=null)
			this.urlText = Text.replace(url, " ", "%20");
		if(thumbnailURL!=null)
			this.thumbnailURLText = Text.replace(thumbnailURL," ", "%20");
	}
			...
			if(children==null) {
				...
				if(parent==null && urlText.equals(ROOT) ) {
					...
					if(cancellable.isCancelled()==false)
						children = destination.toArray(new IOLocation[destination.size()]);
				} else {
				...
				if(s.equals("<image>")) {
					...
					readProperties : while( (s = br.readLine()) != null) {
						...
						if(s.startsWith("<name>")) {
							...
							s = s.substring(0,s.length()-"<name>".length()-1);
							name = s;
						} else if(s.startsWith("<src>")) {
							s = s.substring("<src>".length());
							s = s.substring(0,s.length()-"<src>".length()-1);
							src = s;
						} else if(s.startsWith("<thumbsrc>")) {
							s = s.substring("<thumbsrc>".length());
							s = s.substring(0,s.length()-"<thumbsrc>".length()-1);
							thumbnail = s;
					...
					Pics4LearningLocation imageNode = new Pics4LearningLocation(this, name, src, thumbnail);
	...
	private void loadRoot(List<IOLocation> categoryNodes,Cancellable cancellable) {
		InputStream in = null;
		try {
			URL url = new URL(urlText);
			in = url.openStream();
			...
			while( (s = br.readLine()) != null) {
				...
				if(s.indexOf("<category name=")!=-1) {
					...
					Pics4LearningLocation categoryNode = new Pics4LearningLocation(this,categoryName,
							("http://www.pics4learning.com/query.xml?category="+categoryName));
					
					Vector<Pics4LearningLocation> subcategoryNodes = null;
					readSubcategories : while( (s = br.readLine()) != null) {
						if(s.indexOf("<subcategory name=")!=-1) {
							if(subcategoryNodes==null)
								subcategoryNodes = new Vector<Pics4LearningLocation>();
							String subname = parseString(s);
							Pics4LearningLocation subcategoryNode = new Pics4LearningLocation(categoryNode,subname,
									("http://www.pics4learning.com/query.xml?category="+categoryName+"&subcategory="+subname));
					...
					if(subcategoryNodes!=null) {
						IOLocation[] subcategoryNodeArray = subcategoryNodes.toArray(new IOLocation[subcategoryNodes.size()]);
						categoryNode.children = subcategoryNodeArray;
					}
					
					//don't add until the subcategories (if any) are added,
					//since this may be added to a WatchableVector and go live immediately:
	...
	static class Pics4LearningLocationSearchResults extends Pics4LearningLocation implements SearchResults {
		IOLocation caller;
		String searchText;
		
		public Pics4LearningLocationSearchResults(IOLocation caller,String searchText) {
			super(null, "Search", "http://www.pics4learning.com/query.xml?query="+searchText);
