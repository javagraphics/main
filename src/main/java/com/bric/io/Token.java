 ...
 * <p>
 * For example if you split the String "3 + 15 = 18", you would
 * probably end up with these tokens:
 * <br>The number 3
 * <br>The plus operator
 * <br>The number 15
 * <br>The equals sign (or operator, depending on the language you're parsing)
 * <br>The number 18
 * <p>
...
public class Token implements Comparable<Token> {
	...
	public static String toString(Token... line)
	{
		...
		for (int a = 0; a < line.length; a++)
		{
			...
			if(lastToken!=null) {
				for(int b = lastToken.getDocumentEndIndex(); b<line[a].getDocumentStartIndex(); b++) {
					sb.append(' ');
				}
			}
			Token t = line[a];
	 ...
	 * 
	 * @param tokenText
	 * @param tokenStart the starting position of this token relative to this line.
	 * @param lineNumber
	 * @param documentStart the starting position of this token relative to the document. This should
	 * be equal to tokenStart when lineNumber is zero
	 */
	public Token(String tokenText,int tokenStart,int lineNumber,int documentStart) {
		text = tokenText;
		start = tokenStart;
	 ...
	 * This is different from {@link #getStartIndex()}, which is relative
	 * to the line number. This is relative to the entire document.
	 * 
	 * @return  the index in the document where this token starts.
	 */
	...
	/** Return the exception associated with this Token, or null if this isn't associated with an error.
	 * 
	 * @return the exception associated with this Token, or null if this isn't associated with an error.
	 */
	 ...
	 * 
	 * @param ex the exception to associate with this Token.
	 */
	...
	public String toString() {
		return getClass().getSimpleName()+"[ \""+text+"\", at "+getLineNumber()+":"+getStartIndex()+" ("+getDocumentStartIndex()+")]";
	}
