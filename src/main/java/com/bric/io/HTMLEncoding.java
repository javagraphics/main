 ...
 * 
 * <p>There are apache classes that do the same thing, but these libraries
 * are intended to be isolated and light, so I didn't want to introduce
 * those dependencies.
 * 
 * <p>These were originally based on a list from www.w3schools.com, but that link is dead now.
 * 
 * @see com.bric.io.java.JavaEncoding
 */
public class HTMLEncoding {
	static class EscapeChar {
		...
		String entityName;
		String description;
		EscapeChar(char ch,int entityNumber,String entityName,String description) {
			this.ch = ch;
		...
		public String toString() {
			return "EscapeChar[ ch=\'"+ch+"\', entityNumber="+entityNumber+", entityName="+entityName+", description="+description+" ]";
		}
	...
	private static final Map<String, EscapeChar> entityNameMap = new Hashtable<String, EscapeChar>();
	private static final Map<Character, EscapeChar> charMap = new Hashtable<Character, EscapeChar>();
	static {
		Set<EscapeChar> set = new HashSet<EscapeChar>();
		set.add( new EscapeChar( '\u00c0', 192, "&Agrave;", "capital a, grave accent" ) );
		...
		set.add( new EscapeChar( '\u00c1', 193, "&Aacute;", "capital a, acute accent" ) );
		set.add( new EscapeChar( '\u00c2', 194, "&Acirc;", "capital a, circumflex accent" ) );
		set.add( new EscapeChar( '\u00c3', 195, "&Atilde;", "capital a, tilde" ) );
		set.add( new EscapeChar( '\u00c4', 196, "&Auml;", "capital a, umlaut mark" ) );
		...
		set.add( new EscapeChar( '\u00c8', 200, "&Egrave;", "capital e, grave accent" ) );
		set.add( new EscapeChar( '\u00c9', 201, "&Eacute;", "capital e, acute accent" ) );
		set.add( new EscapeChar( '\u00ca', 202, "&Ecirc;", "capital e, circumflex accent" ) );
		set.add( new EscapeChar( '\u00cb', 203, "&Euml;", "capital e, umlaut mark" ) );
		set.add( new EscapeChar( '\u00cc', 204, "&Igrave;", "capital i, grave accent" ) );
		set.add( new EscapeChar( '\u00cd', 205, "&Iacute;", "capital i, acute accent" ) );
		set.add( new EscapeChar( '\u00ce', 206, "&Icirc;", "capital i, circumflex accent" ) );
		set.add( new EscapeChar( '\u00cf', 207, "&Iuml;", "capital i, umlaut mark" ) );
		set.add( new EscapeChar( '\u00d0', 208, "&ETH;", "capital eth, Icelandic" ) );
		set.add( new EscapeChar( '\u00d1', 209, "&Ntilde;", "capital n, tilde" ) );
		set.add( new EscapeChar( '\u00d2', 210, "&Ograve;", "capital o, grave accent" ) );
		set.add( new EscapeChar( '\u00d3', 211, "&Oacute;", "capital o, acute accent" ) );
		set.add( new EscapeChar( '\u00d4', 212, "&Ocirc;", "capital o, circumflex accent" ) );
		set.add( new EscapeChar( '\u00d5', 213, "&Otilde;", "capital o, tilde" ) );
		set.add( new EscapeChar( '\u00d6', 214, "&Ouml;", "capital o, umlaut mark" ) );
		set.add( new EscapeChar( '\u00d8', 216, "&Oslash;", "capital o, slash" ) );
		set.add( new EscapeChar( '\u00d9', 217, "&Ugrave;", "capital u, grave accent" ) );
		set.add( new EscapeChar( '\u00da', 218, "&Uacute;", "capital u, acute accent" ) );
		set.add( new EscapeChar( '\u00db', 219, "&Ucirc;", "capital u, circumflex accent" ) );
		set.add( new EscapeChar( '\u00dc', 220, "&Uuml;", "capital u, umlaut mark" ) );
		set.add( new EscapeChar( '\u00dd', 221, "&Yacute;", "capital y, acute accent" ) );
		set.add( new EscapeChar( '\u00de', 222, "&THORN;", "capital THORN, Icelandic" ) );
		set.add( new EscapeChar( '\u00df', 223, "&szlig;", "small sharp s, German" ) );
		set.add( new EscapeChar( '\u00e0', 224, "&agrave;", "small a, grave accent" ) );
		set.add( new EscapeChar( '\u00e1', 225, "&aacute;", "small a, acute accent" ) );
		set.add( new EscapeChar( '\u00e2', 226, "&acirc;", "small a, circumflex accent" ) );
		...
		set.add( new EscapeChar( '\u00e8', 232, "&egrave;", "small e, grave accent" ) );
		set.add( new EscapeChar( '\u00e9', 233, "&eacute;", "small e, acute accent" ) );
		set.add( new EscapeChar( '\u00ea', 234, "&ecirc;", "small e, circumflex accent" ) );
		set.add( new EscapeChar( '\u00eb', 235, "&euml;", "small e, umlaut mark" ) );
		set.add( new EscapeChar( '\u00ec', 236, "&igrave;", "small i, grave accent" ) );
		set.add( new EscapeChar( '\u00ed', 237, "&iacute;", "small i, acute accent" ) );
		set.add( new EscapeChar( '\u00ee', 238, "&icirc;", "small i, circumflex accent" ) );
		...
		set.add( new EscapeChar( '\u00f2', 242, "&ograve;", "small o, grave accent" ) );
		set.add( new EscapeChar( '\u00f3', 243, "&oacute;", "small o, acute accent" ) );
		set.add( new EscapeChar( '\u00f4', 244, "&ocirc;", "small o, circumflex accent" ) );
		...
		set.add( new EscapeChar( '\u00f9', 249, "&ugrave;", "small u, grave accent" ) );
		set.add( new EscapeChar( '\u00fa', 250, "&uacute;", "small u, acute accent" ) );
		set.add( new EscapeChar( '\u00fb', 251, "&ucirc;", "small u, circumflex accent" ) );
		set.add( new EscapeChar( '\u00fc', 252, "&uuml;", "small u, umlaut mark" ) );
		set.add( new EscapeChar( '\u00fd', 253, "&yacute;", "small y, acute accent" ) );
		set.add( new EscapeChar( '\u00fe', 254, "&thorn;", "small thorn, Icelandic" ) );
		set.add( new EscapeChar( '\u00ff', 255, "&yuml;", "small y, umlaut mark" ) );
		set.add( new EscapeChar( '\u0020', 160, "&nbsp;", "non-breaking space" ) );
		set.add( new EscapeChar( '\u00a1', 161, "&iexcl;", "inverted exclamation mark" ) );
		...
		set.add( new EscapeChar( '\u00aa', 170, "&ordf;", "feminine ordinal indicator" ) );
		set.add( new EscapeChar( '\u00ab', 171, "&laquo;", "angle quotation mark (left)" ) );
		...
		set.add( new EscapeChar( '\u00ba', 186, "&ordm;", "masculine ordinal indicator" ) );
		set.add( new EscapeChar( '\u00bb', 187, "&raquo;", "angle quotation mark (right)" ) );
		...
		set.add( new EscapeChar( '\u00bf', 191, "&iquest;", "inverted question mark" ) );
		...
		set.add( new EscapeChar( '\u0160', 352, "&Scaron;", "capital S with caron" ) );
		set.add( new EscapeChar( '\u0161', 353, "&scaron;", "small S with caron" ) );
		set.add( new EscapeChar( '\u0178', 376, "&Yuml;", "capital Y with diaeres" ) );
		set.add( new EscapeChar( '\u0192', 402, "&fnof;", "f with hook" ) );
		set.add( new EscapeChar( '\u02c6', 710, "&circ;", "modifier letter circumflex accent" ) );
		...
		set.add( new EscapeChar( '\u200c', 8204, "&zwnj;", "zero width non-joiner" ) );
		...
		set.add( new EscapeChar( '\u2018', 8216, "&lsquo;", "left single quotation mark" ) );
		set.add( new EscapeChar( '\u2019', 8217, "&rsquo;", "right single quotation mark" ) );
		set.add( new EscapeChar( '\u201a', 8218, "&sbquo;", "single low-9 quotation mark" ) );
		set.add( new EscapeChar( '\u201c', 8220, "&ldquo;", "left double quotation mark" ) );
		set.add( new EscapeChar( '\u201d', 8221, "&rdquo;", "right double quotation mark" ) );
		set.add( new EscapeChar( '\u201e', 8222, "&bdquo;", "double low-9 quotation mark" ) );
		...
		set.add( new EscapeChar( '\u2026', 8230, "&hellip;", "horizontal ellipsis" ) );
		...
		set.add( new EscapeChar( '\u2039', 8249, "&lsaquo;", "single left angle quotation" ) );
		set.add( new EscapeChar( '\u203a', 8250, "&rsaquo;", "single right angle quotation" ) );
		...
		set.add( new EscapeChar( '\u21b5', 8629, "&crarr;", "carriage return arrow" ) );
		...
		for(int a = 0; a<length; a++) {
			...
			if(c=='&') {
				...
				if(c=='#') {
					...
					int i;
					if(c=='x') {
						i = Integer.parseInt( encodedString.substring(a+3,endIndex), 16);
					} else {
						i = Integer.parseInt( encodedString.substring(a+2,endIndex), 10);
					}
					sb.append( (char)i );
				} else {
					EscapeChar ch = entityNameMap.get( encodedString.substring(a, endIndex+1) );
					if(ch==null) {
						sb.append("?");
					} else {
						sb.append(ch.ch);
