 ...
 * @(#)JobManager.java
 *
 * $Date: 2015-03-12 19:15:42 +0100 (Do, 12 Mär 2015) $
 *
 ...
 * <p>TODO: This does not resolve circular dependencies.
 * <p>This relies on the <code>java.util.concurrent.Executor</code> and related
 * classes, but it offers more useful feedback to help keep UIs up-to-date.
 *
 */
public class JobManager {
	/** This offers a model for simple notification of what this JobManager
	 * is currently doing.
	 */
	public static interface Listener {
		...
		/** This is called when a job finished executing. When this
		 * is invoked: the active job count of the JobManager will
		 * have changed.
		 * <p>This is probably invoked on a thread that executes
		 * jobs, so this should not perform EDT work and should
		...
		/** This is called when a job is skipped over, either
		 * because it was cancelled or because it was removed
		 * from the pending job queue in this manager.
		 * When this is invoked:
		 * the pending count of the JobManager will have changed.
		 * <p>This is probably invoked on a thread that executes
		 * jobs, so this should not perform EDT work and should
		 * probably be very light (triggering other threads if necessary).
		 */
		...
		/** This is called when a job is added to the queue of pending jobs. When this
		 * is invoked: the pending job count of the JobManager will
		 * have changed.
		 * <p>This is probably invoked on the thread that added the job.
		 */
	...
	/** Return true if a job is eligible to run.
	 * If there are any dependencies declared in this
	 * Job, they must be finished before this job can start.
	 */
	private boolean isEligibleToStart(Job job) {
		Job[] dependencies = job.getDependencies();
		for(Job d : dependencies) {
			/* Let cancelled jobs run. The Job.run() method checks
			 * for this condition and will throw an exception. This way:
			 * the JobManager.Listener will receive feedback that
			 * a job started, even if it was destined to crash and fail.
			 */
		...
		if(l instanceof ChangeListenerWrapper) {
			...
			for(int a = 0; a<listeners.size(); a++) {
				if(listeners.get(a) instanceof ChangeListenerWrapper) {
					ChangeListenerWrapper existing = (ChangeListenerWrapper)listeners.get(a);
					if(existing.changeListener==incoming.changeListener) {
						listeners.remove(a);
						return true;
					}
	...
	public void addJob(Job... jobs) {
		synchronized (queue) {
			for(Job job : jobs) {
				if(job.isFinished()) {
					System.err.println("JobManager warning: the job \""+job.getName()+"\" was classified as finished, but it is being resubmitted. Resetting status to unfinished.");
					job.properties.set(Job.FINISHED, false);
				}
				if(job.isCancelled()) {
					System.err.println("JobManager warning: the job \""+job.getName()+"\" was classified as cancelled, but it is being resubmitted. Resetting status to uncancelled.");
	...
	/** Adds a ChangeListener to this JobManager. This is notified of
	 * the same events as a {@link Listener}; it's just more generic.
	 * 
	 * @param l this listener will be notified when that state
	 * of this manager changes (active/inactive) and when jobs
	 * finish (or stop) running.
	 */
	 ...
	 * 
	 * @param includeActiveJobs if true then this also
	 * includes the number of active jobs. If false then this
	 * only includes pending jobs that are not currently
	 * being processed. (This is no way accounts for completed jobs.)
	 * @return if false then this includes only the pending jobs
	 * that have not started.  If true then this returns the
	 * pending jobs + the current job.
	 */
	 ...
	 * @return true if the job was removed/skipped, false if the job was not found
	 * in the pending queue. (It might already be executing, or have otherwise finished.)
	 */
	 ...
	 * 
	 * @param cancelActiveJobs if true then this additionally
