 ...
 * @(#)Job.java
 *
 * $Date: 2015-03-12 19:12:51 +0100 (Do, 12 Mär 2015) $
 *
...
/** This is an abstract task usually intended to be delegated to
 * a {@link JobManager}.
 *
 */
public abstract class Job implements Runnable, Cancellable {
	...
	public static final Key<Job[]> DEPENDENCIES = new Key<Job[]>("dependencies", Job[].class );
	public static final Key<Boolean> CANCELLED = new Key<Boolean>("cancelled", Boolean.class);
	public static final Key<Boolean> FINISHED = new Key<Boolean>("finished", Boolean.class);
	public static final Key<String> DESCRIPTION = new Key<String>("description", String.class);
	...
	public static final Key<Integer> PRIORITY = new Key<Integer>("priority", Integer.class);
	...
	/** Returns a very short user-friendly description of this job.
	 * The description generally should not change, but the note should.
	 * This may be an empty string if no description is available.
	 * 
	...
	/** Assign a very short user-friendly description of this job.
	 * The description generally should not change, but the note should.
	 * This may be an empty string if no description is available, but it may not be null.
	 * 
	 * @see #getDescription()
	 * @see #setNote(String)
	 */
	...
	/** Assign the name of this Job. This should be a short identifier
	 * (even shorting than the description).
	 */
	...
	/** Returns a very short user-friendly summary of the current state
	 * of this job.
	 * This may be an empty string if no note is available.
	 * 
	 * @see #setNote(String)
	 * @see #getDescription()
	 */
	...
	/** Assign a very short user-friendly summary of the current state
	 * of this job.
	 * This may be an empty string if no note is available, but it may not be null.
	 * 
	 * @see #getNote()
	 * @see #setDescription(String)
	 */
	 ...
	 * @param l this listener will be notified if this job's note changes.
	 */
	 ...
	 * @param l this listener will be notified if this job is cancelled.
	 */
	 ...
	 * @param l this listener will be notified when this job is finished.
	 */
	...
	/** This method is responsible for doing the real work in this Job.
	 * This method should constantly poll <code>isCancelled()</code>: if
	 * this job has been cancelled, this method should return immediately
	 * or throw a UserCancelledException.
	 * <P>Subclasses are also encouraged to call <code>setDescription()</code>
	 * when during this method to give the user cues about what is happening.
	 * <P>This method is not responsible for interacting with the <code>finish</code>
	 * property: that flag will be changed (and listeners notified) when this
	 * method returns.
	 * 
	 */
		...
		try {
			for(Job dependency : getDependencies()) {
				if(dependency.isCancelled())
					throw new UserCancelledException("the job \""+getName()+"\" was cancelled because it relied on the job \""+dependency.getName()+"\", which was previously cancelled");
				if(!dependency.isFinished())
					throw new IllegalStateException("the job \""+getName()+"\" should not be initiated until the job \""+dependency.getName()+"\" is finished.");
			}
			runJob();
			...
			System.err.println("Job: Exception executing \""+getName()+"\"");
	...
	/** Define a list of other Jobs this Job is dependent on.
	 * This Job should not be executed until all of these jobs
	 * are finished.
	 */
	...
	/** Return a list of jobs that must be completed before this
	 * job executes.
	 */
	public Job[] getDependencies() {
		Job[] r = properties.get(DEPENDENCIES);
		if(r==null) return new Job[] {};
		return r;
	}
}
