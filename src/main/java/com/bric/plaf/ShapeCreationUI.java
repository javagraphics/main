...
import com.bric.geom.CubicPath;
import com.bric.geom.ShapeUtils;
import com.bric.swing.ShapeCreationPanel;
import com.bric.swing.ShapeCreationPanel.DataModel;
import com.bric.swing.ShapeCreationPanel.DataModelListener;
import com.bric.swing.ShapeCreationPanel.Selection;
import com.bric.swing.ShapeCreationPanel.SelectionModel;
import com.bric.swing.ShapeCreationPanel.SelectionModelListener;

/** A UI for a {@link ShapeCreationPanel}.
 * <p>This abstract superclass offers a few common functions, but
 * subclasses most implement specific <code>MouseListeners</code>
 * and implementation of each handle.
 * 
 */
public abstract class ShapeCreationUI extends ComponentUI {
	...
	protected void updateCursor(ShapeCreationPanel shapePanel) {
		if( ShapeCreationPanel.MODE_DEFAULT.equals(shapePanel.getMode()) ) {
			Selection indication = shapePanel.getSelectionModel().getIndication();
			if(indication.getShapeIndex()==-1) {
				shapePanel.setCursor( Cursor.getDefaultCursor() );
			} else if(indication.getNodeIndex()==-1) {
				shapePanel.setCursor( Cursor.getPredefinedCursor(Cursor.HAND_CURSOR) );
			} else {
				shapePanel.setCursor( Cursor.getPredefinedCursor(Cursor.CROSSHAIR_CURSOR) );
			}
		} else {
			shapePanel.setCursor( Cursor.getPredefinedCursor(Cursor.CROSSHAIR_CURSOR) );
		}
	...
	PropertyChangeListener propertyListener = new PropertyChangeListener() {
		public void propertyChange(PropertyChangeEvent evt) {
			...
			if(ShapeCreationPanel.TRANSFORM_KEY.equals(evt.getPropertyName())) {
				scp.repaint();
			} else if(ShapeCreationPanel.HANDLE_SIZE_KEY.equals(evt.getPropertyName())) {
				scp.repaint();
			} else if(ShapeCreationPanel.HANDLES_ACTIVE_KEY.equals(evt.getPropertyName())) {
				scp.repaint();
				...
				if(oldModel!=null) oldModel.removeListener(dataModelListener);
				if(newModel!=null) newModel.addListener(dataModelListener);
			} else if(ShapeCreationPanel.SELECTION_MODEL_KEY.equals(evt.getPropertyName())) {
				SelectionModel oldModel = (SelectionModel)evt.getOldValue();
				SelectionModel newModel = (SelectionModel)evt.getNewValue();
				if(oldModel!=null) oldModel.removeListener(selectionModelListener);
				if(newModel!=null) newModel.addListener(selectionModelListener);
			}
			updateCursor(scp);
		}
	};
	 ...
	 * <p>For example: the CurvedPolylineCreationUI only wants to
	 * work with CurvedPolylines. This may necessarily lose some shape data
	 * in making it conform to the expected format.
	...
	/** Return a Selection for a shape. This takes into account <code>getStroke(scp, shapeIndex)</code>
	 * and should be consulted after possible matches for handles have been ruled out.
	 * 
	 * @param mouseLoc an un-transformed mouse location (that is: this comes directly from a MouseEvent
	 * and is relative to the ShapeCreationPanel's coordinates).
	 */
	protected Selection getSelectedShape(ShapeCreationPanel scp,Point2D mouseLoc) {
		...
		for(int shapeIndex = shapes.length-1; shapeIndex>=0; shapeIndex--) {
			...
			if(strokedShape.intersects(mouseLoc.getX()-.5, mouseLoc.getY()-.5, 1, 1)) {
	...
	public void installUI(JComponent c) {
		...
		scp.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, 0), "delete");
		scp.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_BACK_SPACE, 0), "delete");
		scp.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, 0), "nudgeLeft");
		scp.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, 0), "nudgeRight");
		scp.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0), "nudgeUp");
		scp.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, 0), "nudgeDown");
	...
	/** Invokes {@link #paintShapes(Graphics2D, ShapeCreationPanel)} and
	 * then {@link #paintControls(Graphics2D, ShapeCreationPanel)}
	 */
	@Override
	...
	/** Return the stroke used to paint a WritingStroke. This stroke is independent
	 * of the panel's AffineTransform. (So if the stroke is expected to scale with the
	 * transform, then this method needs to consult the AffineTransform and change accordingly.)
	 * 
	...
	/** This paints an individual shape.
	 * Subclasses may override this to control the color/stroke of shapes.
	 * <p>The default implementation here paints a 2-pixel focus ring around
	 * a shape if it is the currently selected shape.
	 * 
	 * @see #getStroke(ShapeCreationPanel, int)
	 */
	protected void paintShape(Graphics2D g,ShapeCreationPanel panel,int shapeIndex,Shape transformedShape) {
	 ...
	 * <p>When implementing this method: note that this is always invoked,
	 * and you need to consult the ShapeCreationPanel to determine when
	 * handles are supposed to be active. 
	 */
