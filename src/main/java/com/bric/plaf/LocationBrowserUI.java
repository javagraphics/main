...
public abstract class LocationBrowserUI extends ComponentUI {
	 ...
	 * the GraphicsCache has indicated the loc has a new graphic
	 * available.
	 * @param loc
	 * @param thumbnail if true then a thumbnail is available, if false then an icon is available.
	 */
	protected abstract void repaint(IOLocation loc,boolean thumbnail);

	/** This is called during the <code>installUI</code> method to set up
	 * the controls visible in this <code>LocationBrowserUI</code>.
	 * 
	 * @param comp the panel to install the GUI in.
	 */
	...
	public void installUI(JComponent c) {
		if(c!=browser)
			throw new IllegalArgumentException("this ui can only be used for the LocationBrowser it was constructed with");
	...
	/** This is called by <code>typingListener</code> when the user
	 * has typed a string.  By default it will select the first item
	 * in the directory that starts with the same text.
	 */
	...
	/** Returns the number of currently visible locations.
	 * If this is zero, then an indeterminant progress bar
	 * may appear while this location is still loading.
	 */
	public abstract int getVisibleLocationSize();
	
	/** Call this method to update/monitor the visibility of the loadingProgressBar.
	 * 
	 * @return true if we need to continue to monitor this
	 */
	private boolean updateProgressBar() {
		/**
		 * This is surprisingly tricky because we don't want to display it *immediately*
		 * if the list is empty.  This could result in annoying flickering if the user
		 * is still getting near-instant feedback.  So instead we want to wait a short
		 * delay before showing it.
		 */
		boolean needFeedback = browser.isLoading() && getVisibleLocationSize()==0;
