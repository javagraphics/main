...
public abstract class ListLocationBrowserUI extends LocationBrowserUI {
		...
		public void run() {
			...
			try {
				...
				synchronized(listModel) {
					...
					//seriously?  there isn't a way to set the selection all at once?
					table.getSelectionModel().clearSelection();
					for(int a = 0; a<indices.length; a++) {
						table.getSelectionModel().addSelectionInterval(indices[a], indices[a]);
					}
	...
	protected void synchronizeDirectoryContents() {
		...
		/** The likely cases here are:
		 * 1.  The table and array exactly match, because redundant
		 * runnables have piled up in the queue.
		 * 2.  New elements were added to list that the table needs to add.
		 * 3.  The list was emptied.
		 * 
		 * We should also plan for the unintended "other" scenario:
		 * 4.  The list changed in unpredicted ways, and the table needs to adjust.
		 * 
		 */
		...
		if(array.length==rowCount) { //scenario #1
			...
			for(int a = 0; a<array.length && equals; a++) {
				IOLocation tableValue = (IOLocation)tableModel.getValueAt(a, 0);
			...
			Vector<Integer> arrayElementsToAdd = new Vector<Integer>();
			for(int a = 0; a<array.length; a++) {
			...
			if(rowIndex==rowCount) {
				//excellent!  This means the table is an ordered subset of the array.
				//new elements were inserted: that's all.  This really is scenario #2
				for(int a = 0; a<arrayElementsToAdd.size(); a++) {
					...
					tableModel.insertRow(index, new Object[] { loc, new Date(date)});
				}
				
				/* used for debugging to validate the results:
				if(array.length!=tableModel.getRowCount())
					throw new RuntimeException();
				
				for(int a = 0; a<array.length; a++ ){
					rowValue = (IOLocation)tableModel.getValueAt(a, 0);
					if(array[a].equals(rowValue)==false)
						throw new RuntimeException("a = "+a+", "+array[a]+", "+rowValue);
				}
				*/
				return;
			}
	...
	public ListLocationBrowserUI(LocationBrowser b) {
		...
		table = new JTable(tableModel) {
			...
			@Override
			public boolean editCellAt(int row,int col,java.util.EventObject obj) {
				//TODO: edit this to let the user double-click to edit file names
				//(if we want that.)
				return false;
			}
		};
