...
import com.bric.geom.MasterPathWriter;
import com.bric.geom.NullPathWriter;
...
public class ButtonShape implements PositionConstants {
	...
	public GeneralPath getShape(GeneralPath path,
		...
		return getShape(path, null, width, height, POS_ONLY, POS_ONLY, true, null);
	}
	 ...
	 * 
	 * @param fillPath an optional destination for the button shape.
	 * @param strokePath an optional destination for the border.
	 * Often the border and fill will be the same, but in some
	 * cases where line segments are meant to be missing: they will
	 * be unique.
	 * @param width the width to stretch this fill to.
	 * @param height the height to stretch this fill to.
	 ...
	 */
	public GeneralPath getShape(GeneralPath fillPath,
			GeneralPath strokePath,
			int width,int height,
			int horizontalPosition,int verticalPosition,
			boolean includeStrokePartitions,
			Shape customShape) {
		
		//if this is the case: we'll still return this object
		if(fillPath==null) fillPath = new GeneralPath();
		...
		PathWriter strokeWriter = strokePath==null ? new NullPathWriter() : new GeneralPathWriter(strokePath);
		
		//prevents errors with 0x0 buttons:
		fillWriter.setEliminateRedundantLines(false);
		
		PathWriter master = new MasterPathWriter(
				fillWriter, strokeWriter
		);
		...
		if(customShape!=null) {
			...
			if(verticalPosition==POS_TOP && 
				...
				if(topLeftRadius==0) {
					...
					master.curveTo(topLeftRadius-topLeftRadius*k, 0, 
				...
				PathWriter w = includeStrokePartitions ? master : fillWriter;
				w.lineTo(width, height);
			} else if(verticalPosition==POS_MIDDLE && horizontalPosition==POS_RIGHT) {
				master.moveTo(width,height);
				master.lineTo(width, 0);
				PathWriter w = includeStrokePartitions ? master : fillWriter;
				w.lineTo(0, 0);
				w.lineTo(0, height);
				w.lineTo(width, height);
			} else if(verticalPosition==POS_MIDDLE && horizontalPosition==POS_ONLY) {
				...
				if(bottomLeftRadius==0) {
					...
					master.curveTo(0, height-bottomLeftRadius+bottomLeftRadius*k,
				...
				w.lineTo(width, 0);
				w.lineTo(0, 0);
			} else if(horizontalPosition==POS_MIDDLE && verticalPosition==POS_BOTTOM) {
				...
				w.lineTo(0, 0);
				w.lineTo(0, height);
			} else if(horizontalPosition==POS_MIDDLE && verticalPosition==POS_ONLY) {
				if(includeStrokePartitions) {
					master.moveTo(width, 0);
					master.lineTo(0, 0);
					master.lineTo(0, height);
				...
				if(bottomRightRadius==0) {
					...
					master.curveTo(width-bottomRightRadius+bottomRightRadius*k, height, 
