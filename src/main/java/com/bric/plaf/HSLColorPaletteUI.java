...
public class HSLColorPaletteUI extends ScrollableColorPaletteUI {
	...
	protected ColorSet getColorSet(ColorPalette cp,boolean grid) {
		...
		return new HLColors(hues, grid, rows, columns, saturation, alpha, (String)cp.getClientProperty( HSBColorPaletteUI.PALETTE_PADDING_PROPERTY ));
	}
	...
	protected void updateColor(ColorPalette cp) {
		...
		HLColors colorSet = new HLColors(hues, true, rows, columns, hsl[1], 1, (String)cp.getClientProperty( HSBColorPaletteUI.PALETTE_PADDING_PROPERTY ));
	...
	public void installUI(JComponent c) {
		...
		cp.addPropertyChangeListener(HSBColorPaletteUI.PALETTE_PADDING_PROPERTY, repaintPropertyListener);
		cp.addPropertyChangeListener(HSBColorPaletteUI.PALETTE_PADDING_PROPERTY, updateColorPropertyListener);
	}
	...
	public void uninstallUI(JComponent c) {
		...
		ColorPalette cp = (ColorPalette)c;
		cp.removePropertyChangeListener(HSBColorPaletteUI.PALETTE_PADDING_PROPERTY, repaintPropertyListener);
	}
	...
	static class HLColors extends ColorSet {
		 ...
		 */
		public HLColors(HueDistribution hueDistribution,boolean grid,int rows,int columns,float saturation,float alpha,String padding) {
			...
			} else if(HSBColorPaletteUI.PALETTE_PADDING_SATURATED.equals(padding)) {
				saturatedPadding = true;
			} else if(HSBColorPaletteUI.PALETTE_PADDING_NONSATURATED.equals(padding)) {
				nonsaturatedPadding = true;
			}
