 ...
 */
package com.bric.desktop.error;
...
public class ErrorDialog extends QDialog {
	...
	public ErrorDialog(Frame frame) {
		...
		DialogFooter myFooter = DialogFooter.createDialogFooter(DialogFooter.OK_OPTION, DialogFooter.EscapeKeyBehavior.TRIGGERS_DEFAULT);
		setFooter(myFooter);
		myFooter.getButton(DialogFooter.OK_OPTION).addActionListener(new ActionListener() {
	...
	protected void refreshContents() {
		LinkedHashSet<JComponent> panels = new LinkedHashSet<>(throwableMap.values());
	...
	public String toString() {
		return "ErrorDialog-"+id+" frame=\""+((Frame)getOwner()).getTitle()+"\"";
	}
	...
	class ThrowablePanel extends JPanel {
		...
		protected ThrowablePanel(ThrowableDescriptor t) {
			...
			c.gridx = 0; c.gridy = 0; c.weightx = 1; c.weighty = 1;
			c.fill = GridBagConstraints.BOTH;
			add( QDialog.createContentPanel( msg, t.throwable.getClass().getName(), null, true), c);
			c.gridx++; c.weightx = 0;
			c.insets = new Insets(3,3,3,3);
			add(multiplierLabel, c);
			
			multiplierLabel.setFont(multiplierLabel.getFont().deriveFont(Font.BOLD));
			multiplierLabel.setForeground(Color.lightGray);
		}
		
		protected void addThrowable(ThrowableDescriptor t) {
			throwables.add(t);
			multiplierLabel.setText(throwables.size()>=2 ? "x"+throwables.size() : " ");
		}
