 ...
 * <P>If you trace two new paths (one on either side of the original path)
 * and then join those two new paths together: you've just created an outline/stroke
 * of your original path.
 * <P>To compare this with the BasicStroke class: this class uses a JOIN_BEVEL
 * and does not address caps (however if you just draw a line from one
 * inset path to another: that is a CAP_BUTT.)
 * 
 */
public class InsetPathWriter extends PathWriter {
	 ...
	 * 
	 * @param w the destination to write the new path data to.
	 * @param inset the number of pixels to inset
	 * @param clockwise where to draw the inset path.
	 * If this is true, then the path is drawn at 90 degrees clockwise
	 * to the original path.  Otherwise this is drawn at 90 degrees
	 * counterclockwise.
	 */
	public InsetPathWriter(PathWriter w,float inset,boolean clockwise) {
		this(w,inset, clockwise ? (float)(Math.PI/2.0) : (float)(-Math.PI/2.0));
	}
	 ...
	 * @param w the destination to write the new path data to.
	 * @param inset the number of pixels to inset
	 * @param theta the angle the path will be written at.
	 * That is: if you draw a line tangent to the original path, and then
	 * move <code>(+inset*cos(theta), +inset*sin(theta))</code>, that is
	...
	public void curveTo(float cx1, float cy1, float cx2, float cy2, float x,
         ...
         * and t=[a value near one].  This way if this "curve" really 
         * represents a line (where the first control point is the last point,
         * and the second control point in the end point), we don't get wacky
         * inappropriate angles.
         * 
         */
		//double theta0 = Math.atan2(cy,cx);
        double t = .00001;
		double theta0 = Math.atan2(
				(3*ay*t+2*by)*t+cy,
				(3*ax*t+2*bx)*t+cx);
		...
		t = .9999;
		double theta1 = Math.atan2(
				(3*ay*t+2*by)*t+cy,
				(3*ax*t+2*bx)*t+cx);
		
		float x2 = (float)( ax+bx+cx+dx+inset*Math.cos(theta1+theta) );
		float y2 = (float)( ay+by+cy+dy+inset*Math.sin(theta1+theta) );
		
		theta0 = Math.atan2(3.0/9.0*ay+2.0/3.0*by+cy,3.0/9.0*ax+2.0/3.0*bx+cx);
		theta1 = Math.atan2(3.0*4.0/9.0*ay+2.0*2.0/3.0*by+cy,3.0*4.0/9.0*ax+2.0*2/3.0*bx+cx);
	...
	public void quadTo(float ctrlX, float ctrlY, float x, float y) {
		...
		writer.quadTo( newCtrlX, newCtrlY,
				x2, y2 );
