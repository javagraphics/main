 ...
 */
// This file is originally based on sun.awt.geom.Curve (c) 2006 by
...
public abstract class CurveX {
	...
	public static void insertLine(AreaXBody curves,
			double x0, double y0,
			double x1, double y1)
	{
		if (y0 < y1) {
			curves.add(new Order1X(x0, y0,
					x1, y1,
					INCREASING));
		} else if (y0 > y1) {
			curves.add(new Order1X(x1, y1,
					x0, y0,
					DECREASING));
		} else {
//			Do not add horizontal lines
	...
	public static void insertQuad(CurveList curves,
			double x0, double y0,
			double coords[])
	{
		double y1 = coords[3];
		if (y0 > y1) {
			Order2X.insert(curves, coords,
					coords[2], y1,
					coords[0], coords[1],
					x0, y0,
					DECREASING);
...
//			Do not add horizontal lines
	...
	public static void insertCubic(CurveList curves,
			double x0, double y0,
			double coords[])
	{
		double y1 = coords[5];
		if (y0 > y1) {
			Order3X.insert(curves, coords,
					coords[4], y1,
					coords[2], coords[3],
					coords[0], coords[1],
					x0, y0,
