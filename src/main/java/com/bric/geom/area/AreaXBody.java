 ...
 * @(#)AreaXBody.java
 *
 * $Date: 2014-11-09 00:02:48 +0100 (So, 09 Nov 2014) $
 *
...
import com.bric.geom.Intersections;
import com.bric.geom.ShapeBounds;
import com.bric.math.MathG;

/** This is a list of <code>CurveX</code> segments that makes up the shape
 * of an <code>AreaX</code>.
 * <p>This is based on the <code>Vector</code> named "curves" in the
 * original <code>java.awt.geom.Area</code> class.  However this class
 * has a few key modifications:
 * <ul><li>The bounds are internally updated constantly.</li>
 * <li>The methods to add curves are protected.  So while this list can
 * be iterated over via public methods, it is immutable to classes outside
 * this package.  This is a deliberate choice to facilitate the
 * <code>AreaXRules</code> model.
 * <li>This object adds the concept of validation (more on this below).</li>
 * <li>This object is not synchronized.  The <code>AreaX</code> class is,
 * but this class is not.  This is for internal behind-the-scenes work:
 * it is the caller's responsibility to synchronize method calls safely.</li></ul>
 * <h3>Validation</h3>
 * This concept is new to the <code>AreaX</code> package, but the code is
 * not new.  The original <code>Area</code> class automatically validated everything
 * all the time.  Here I separated this process because -- as we start
 * dealing with complex <code>AreaXRules</code> -- we can get some
 * performance improvements if we don't validate all shapes.
 * <p>A "valid" body is one that has been primed for geometric operations.  An
 * "invalid" body is one that has not yet been primed -- it is a raw dump
 * of another shape.
 * <p>In the end: all shapes that are involved in computing an add, xor,
 * intersect or subtract are validated.  But we don't have to perform
 * an expensive validation in order to see whether Shape A contains Shape B.
 */
public class AreaXBody extends CurveList {
	 ...
	 *
	 * @param   validationOperator  if this body is not yet valid, this is
	 * the operation that will validate it.  If this is null then this
	 * body is considered valid.
	...
	/** Studies this object (the LHS) and the argument (the RHS)
	 * to determine the relationship.
	 * 
	 * @param rhs a shape to iterate over.
	 * @param at an optional AffineTransform to apply to the rhs operand.
	 ...
	 * to determine the relationship.
	 * 
	 * @param lhsTransform an optional AffineTransform to apply to this operand.
	 * @param rhs a shape to iterate over.
	 * @param at an optional AffineTransform to apply to the rhs operand.
	 * @return one of the RELATIONSHIP constants.
	 */
	public int getRelationship(AffineTransform lhsTransform,Shape rhs,AffineTransform at) {
		...
		if(relationship==AreaX.RELATIONSHIP_NONE) {
			//at this point we know the LHS doesn't contain the RHS, and that they don't intersect.
			
			//this could be RHS_CONTAINS, too?  Or Complex?
			if(elementCount==0) return AreaX.RELATIONSHIP_NONE;
	...
	private boolean intersectsQuad(double x0,double y0,double cx,double cy,double x1,double y1,AreaXIterator axi,AffineTransform axiTx) {
		...
		while(axi.isDone()==false) {
			...
			if(k==PathIterator.SEG_MOVETO) {
				...
				if(Intersections.quadQuad(x0,y0,cx,cy,x1,y1,
						lastX,lastY,coords[0],coords[1],coords[2],coords[3]))
					return true;
				lastX = coords[2];
				lastY = coords[3];
			} else if(k==PathIterator.SEG_CUBICTO) {
				if(Intersections.quadCubic(x0,y0,cx,cy,x1,y1,
						lastX,lastY,coords[0],coords[1],coords[2],coords[3],coords[4],coords[5]))
					return true;
				lastX = coords[4];
				lastY = coords[5];
			}
	...
	private boolean intersectsCubic(double x0,double y0,double cx0,double cy0,double cx1,double cy1,double x1,double y1,AreaXIterator axi,AffineTransform axiTx) {
		...
		while(axi.isDone()==false) {
			...
			//ignore SEG_CLOSE instructions, because the AreaXIterator will always
			...
			if(k==PathIterator.SEG_MOVETO) {
				...
				if(Intersections.quadCubic(lastX,lastY,coords[0],coords[1],coords[2],coords[3],
						x0,y0,cx0,cy0,cx1,cy1,x1,y1))
					return true;
				lastX = coords[2];
				lastY = coords[3];
			} else if(k==PathIterator.SEG_CUBICTO) {
				if(Intersections.cubicCubic(x0,y0,cx0,cy0,cx1,cy1,x1,y1,
						lastX,lastY,coords[0],coords[1],coords[2],coords[3],coords[4],coords[5]))
					return true;
				lastX = coords[4];
				lastY = coords[5];
	 ...
	 * <p>Validation is an expensive process of restructuring a primitive <code>AreaXBody</code>
	 * so it is primed for geometric operations.
	 */
	 ...
	 * <p>If this object is invalid but this method has already been performed,
	 * then the previously created object is returned.
	 * <p>Otherwise: this body is validated.  This is expensive but required
	 * for other operations.
	 */
	public AreaXBody validate() {
		if(validationOperator==null) return this;
		if(validatedBody!=null) return validatedBody;
	...
	/** Returns a hashcode based on the bounds of this body.
	 * This way a validated and unvalidated body will nearly
...
//			3 parametric equation derivative coefficients
	...
	public Rectangle getBounds(Rectangle dest) {
		if(dest==null) dest = new Rectangle();
	...
	public AreaXBody transform(AffineTransform t) {
		if(Math.abs(t.getShearX())<.0000001 && Math.abs(t.getShearY())<.0000001) {
			...
			for(int a = 0; failed==false && a<size; a++) {
				...
				if( (newCurve.getY0()==newCurve.getY1()) !=
					(curve.getY0()==curve.getY1())) {
					/** This can happen as a rare result
					 * of computer error when multiplying.
					 * 
					 * Unfortunately this condition
					 * will lead to an InternalError
					 * in subsequent AreaOpX calls,
					 * so we need to abandon ship here.
					 */
					failed = true;
				}
	...
	/**
	 * Calculates the addition of the specified <code>AreaXBody</code>
	 * with this <code>AreaXBody</code>.
	 * The resulting shape will include
	 ...
	 * </pre>
	 * 
	 * @param   rhs  the <code>AreaXBody</code> to be added to the
	 *          this body.
	 * @return  the new <code>AreaXBody</code> of this operation.
	 *          This object and the operator are not modified.
	 * @throws NullPointerException if <code>rhs</code> is null
	 */
	public AreaXBody add(AreaXBody rhs) {
		if(this.isEmpty()) return rhs;
		if(rhs.isEmpty()) return this;
		return new AreaOpX.AddOp().calculate(this, rhs);
	}

	/** This simply merges two bodies together.
	 * <p>WARNING: use this only when it is guaranteed that the LHS
	 * and RHS do not overlap in any way.  This is much more efficient
	 * than calling <code>add()</code>, but it will be very dangerous
	 * if misused.
	 */
	 ...
	 * shape of this <code>AreaXBody</code>.
	 * The resulting <code>AreaXBody</code> will include
	 * areas that were contained only in this <code>AreaXBody</code>
	 * and not in the argument <code>AreaXBody</code>.
	 * <pre>
	 *     // Example:
	 *     Area a1 = new Area([triangle 0,0 =&gt; 8,0 =&gt; 0,8]);
	 *     Area a2 = new Area([triangle 0,0 =&gt; 8,0 =&gt; 8,8]);
	 ...
	 *     ##                                 ##     ##
	 * </pre>
	 * @param   rhs  the <code>AreaXBody</code> to be subtracted from the
	 *		current shape
	 * @return  the new <code>AreaXBody</code> of this operation.
	 *          This object and the operator are not modified.
	 * @throws NullPointerException if <code>rhs</code> is null
	...
	/**
	 * Calculate the <code>AreaXBody</code> of the intersection of
	 * its current shape and the shape of the specified <code>AreaXBody</code>.
	 * The resulting shape will include
	 * only areas that were contained in both this <code>AreaXBody</code>
	 * and also in the specified <code>AreaXBody</code>.
	 * <pre>
	 *     // Example:
	 *     Area a1 = new Area([triangle 0,0 =&gt; 8,0 =&gt; 0,8]);
	 *     Area a2 = new Area([triangle 0,0 =&gt; 8,0 =&gt; 8,8]);
	 ...
	 *     ##                                 ##
	 * </pre>
	 * @param   rhs  the <code>AreaXBody</code> to be intersected with this
	 *		<code>AreaXBody</code>
	 * @return  the new <code>AreaXBody</code> of this operation.
	 *          This object and the operator are not modified.
	 * @throws NullPointerException if <code>rhs</code> is null
	...
	/**
	 * The resulting shape of this operation will include
	 * only areas that were contained in either this <code>AreaXBody</code>
	 * or in the specified <code>AreaXBody</code>, but not in both.
	 * <pre>
	 *     // Example:
	 *     Area a1 = new Area([triangle 0,0 =&gt; 8,0 =&gt; 0,8]);
	 *     Area a2 = new Area([triangle 0,0 =&gt; 8,0 =&gt; 8,8]);
	 ...
	 * @param   rhs  the <code>AreaXBody</code> to be exclusive ORed with this
	 *		<code>AreaXBody</code>.
	 * @return  the new <code>AreaXBody</code> of this operation.
	 *          This object and the operator are not modified.
	 * @throws NullPointerException if <code>rhs</code> is null
...
class AreaXIterator implements  PathIterator {
	...
	/** This creates an empty iterator.  This is generally useless
	 * unless you call <code>reset()</code> to define a body to
	 * iterate over.
	 */
	protected AreaXIterator() {}
	
	/** Creates a <code>AreaXIterator</code> for a given
	 * <code>AreaXBody</code>.
	 * 
	 */
