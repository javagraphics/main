 ...
 * Each node has "previous" and "next" control point.
 * <p>The scale factor is used to limit the magnitude of the
 * the control points.
 * <p>Several methods have two sets of arguments:
 * <ul><li>One only accepting the <code>nodeIndex</code>.  This looks at <i>all</i>
...
public final class CubicPath extends AbstractShape {
	private Vector<WeakReference<CubicPathIterator>> iterators = new Vector<WeakReference<CubicPathIterator>>();
	private int windingRule = PathIterator.WIND_EVEN_ODD;
	private final Vector<Vector<CubicNode>> paths = new Vector<Vector<CubicNode>>();
	...
	private CubicPath(int windingRule,Vector<Vector<CubicNode>> in_paths) {
		this(windingRule);
		
		paths.ensureCapacity(in_paths.size());
		for(int a = 0; a<in_paths.size(); a++) {
			Vector<CubicNode> in_nodes = in_paths.get(a);
			Vector<CubicNode> nodes = new Vector<CubicNode>(in_nodes.size());
	...
	/** This sets the scale factor for this shape.  The default
	 * value is 1.0.
	 * 
	 * @param sf new the scale factor.  This must be greater than zero.
	 * @return true if this changed the scale factor.
	 */
	public boolean setScaleFactor(double sf) {
		if(sf<=0) throw new IllegalArgumentException("scaleFactor ("+sf+") must be greater than zero.");
		if(sf==scaleFactor) return false;
		scaleFactor = sf;
		return true;
	}
	...
	private synchronized void releaseIterators() {
		while(iterators.size()>0) {
			WeakReference<CubicPathIterator> ref = iterators.remove(iterators.size()-1);
	...
	public synchronized boolean isNodeSharp(int nodeIndex) {
		...
		throw new IllegalArgumentException("no node found at index "+nodeIndex);
	}
	...
	/** A "sharp" node is a node that doesn't have a control points.
	 * 
	 * @param pathIndex  the path to examine.
	 * @param nodeIndex the node to examine.
	 * @return true if the node is sharp.
	 */
	 ...
	 * 
	 * @param nodeIndex the node to examine.
	 * @param equalDistance if this is true then the control points have to be collinear
	 * <i>and</i> equidistant to the node point.
	 * @return true if the node is smooth.
	 */
	public synchronized boolean isNodeSmooth(int nodeIndex,boolean equalDistance) {
		for(int pathIndex = 0; pathIndex<paths.size(); pathIndex++) {
			Vector<CubicNode> nodes = paths.get(pathIndex);
			int s = nodes.size();
			if(nodeIndex<s) {
	 ...
	 * @return true if the node is smooth.
	 */
	public synchronized boolean isNodeSmooth(int pathIndex,int nodeIndex,boolean equalDistance) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
		CubicNode node = nodes.get(nodeIndex);
	 ...
	 * 
	 * @param nodeIndex the node to change.
	 * @param p the new coordinates of the node.
	 * @param translateControlPoints whether the control points should translate similarly.
	 * @return true if data was modified by this call.
	 */
	public synchronized boolean setNode(int nodeIndex,Point2D p,boolean translateControlPoints) {
	 ...
	 * @return true if data was modified by this call.
	 */
	public synchronized boolean setNode(int nodeIndex,double x,double y,boolean translateControlPoints) {
		for(int pathIndex = 0; pathIndex<paths.size(); pathIndex++) {
			Vector<CubicNode> nodes = paths.get(pathIndex);
			int s = nodes.size();
			if(nodeIndex<s) {
				return setNode(pathIndex, nodeIndex, x, y, translateControlPoints);
			}
			nodeIndex -= s;
		}
		System.err.println(this);
	...
	/** Sets a node's location.
	 * 
	 * @param pathIndex the path to change.
	 * @param nodeIndex the node to change.
	 * @param p the new coordinates of the node.
	 * @param translateControlPoints whether the control points should translate similarly.
	 * @return true if data was modified by this call.
	 */
	public synchronized boolean setNode(int pathIndex,int nodeIndex,Point2D p,boolean translateControlPoints) {
		return setNode(pathIndex, nodeIndex, p.getX(), p.getY(),translateControlPoints);
	}
	 ...
	 * @return true if data was modified by this call.
	 */
	public synchronized boolean setNode(int pathIndex,int nodeIndex,double x,double y,boolean translateControlPoints) {
		releaseIterators();
	 ...
	 * 
	 * @param pathIndex the path to get the node count of.
	 * @return the number of nodes in a given path.
	 */
	...
	public synchronized int getNodeCount() {
		...
		for(int a = 0; a<paths.size(); a++) {
			Vector<CubicNode> nodes = paths.get(a);
			sum += nodes.size();
		}
		return sum;
	}
	 ...
	 * @param dest the object to store the coordinates in.  May be null.
	 * @return this will return the <code>dest</code> argument if it wasn't null.
	 */
	 ...
	 * @return this will return the <code>dest</code> argument if it wasn't null.
	 */
	public synchronized Point2D getNode(int pathIndex,int nodeIndex,Point2D dest) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
		CubicNode node = nodes.get(nodeIndex);
		
		if(dest==null) dest = new Point2D.Double();
		node.getPoint(dest);
		return dest;
	}
	 ...
	 * 
	 * @param nodeIndex the node index.
	 * @param p the object to store the coordinates in.  May be null.
	 * @return this will return <code>null</code> if there is no previous control point
	 * for this node.  Or this will return <code>p</code> if it wasn't null.
	 */
	...
	/** Sets the coordinates of the next control point for a node based on
	 * the previous control point.
	 * 
	 * @param pathIndex the path index.
	 * @param nodeIndex the node index.
	 * @param includeDistance whether the distances between control points and the node should be equal.
	 * @return true if this changed the shape.
	 */
	public synchronized boolean setNextControlForNodeFromPrev(int pathIndex,int nodeIndex,boolean includeDistance) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
		CubicNode node = nodes.get(nodeIndex);
		...
		if(includeDistance) {
			return setNextControlForNode(pathIndex, nodeIndex, node.x+distance*Math.cos(angle), node.y+distance*Math.sin(angle));
		} else {
			if(node.nextControl==null) node.nextControl = new Point2D.Double(node.x, node.y);
			distance = Point2D.distance(node.x, node.y, node.nextControl.getX(), node.nextControl.getY())*scaleFactor;
			return setNextControlForNode(pathIndex, nodeIndex, node.x+distance*Math.cos(angle), node.y+distance*Math.sin(angle));
		}
	}
	 ...
	 * @return true if this changed the shape.
	 */
	public synchronized boolean setNextControlForNodeFromPrev(int nodeIndex,boolean includeDistance) {
		for(int pathIndex = 0; pathIndex<paths.size(); pathIndex++) {
			Vector<CubicNode> nodes = paths.get(pathIndex);
			int s = nodes.size();
			if(nodeIndex<s) {
				return setNextControlForNodeFromPrev(pathIndex, nodeIndex, includeDistance);
			}
			nodeIndex -= s;
		}
		System.err.println(this);
	 ...
	 * @return true if this changed the shape.
	 */
	public synchronized boolean setPrevControlForNodeFromNext(int pathIndex,int nodeIndex,boolean includeDistance) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
		CubicNode node = nodes.get(nodeIndex);
		...
		double distance = Math.sqrt(dy*dy+dx*dx)*scaleFactor;
		if(includeDistance) {
			return setPrevControlForNode(pathIndex, nodeIndex, node.x+distance*Math.cos(angle), node.y+distance*Math.sin(angle));
		} else {
			if(node.prevControl==null) node.prevControl = new Point2D.Double(node.x, node.y);
			distance = Point2D.distance(node.x, node.y, node.prevControl.getX(), node.prevControl.getY())*scaleFactor;
			return setPrevControlForNode(pathIndex, nodeIndex, node.x+distance*Math.cos(angle), node.y+distance*Math.sin(angle));
		}
	}
	 ...
	 * @return true if this changed the shape.
	 */
	public synchronized boolean setPrevControlForNodeFromNext(int nodeIndex,boolean includeDistance) {
		for(int pathIndex = 0; pathIndex<paths.size(); pathIndex++) {
			Vector<CubicNode> nodes = paths.get(pathIndex);
			int s = nodes.size();
			if(nodeIndex<s) {
				return setPrevControlForNodeFromNext(pathIndex, nodeIndex, includeDistance);
			}
			nodeIndex -= s;
		}
		System.err.println(this);
	 ...
	 */
	public synchronized boolean setPrevControlForNode(int nodeIndex,double x,double y) {
		for(int pathIndex = 0; pathIndex<paths.size(); pathIndex++) {
			Vector<CubicNode> nodes = paths.get(pathIndex);
			int s = nodes.size();
			if(nodeIndex<s) {
	 ...
	 * 
	 * @param nodeIndex the node index.
	 * @param p the new coordinates of the control point.
	 * @return true if this changed this shape.
	 */
	 ...
	 * @return true if this changed this shape.
	 */
	public synchronized boolean setNextControlForNode(int nodeIndex,double x,double y) {
		for(int pathIndex = 0; pathIndex<paths.size(); pathIndex++) {
			Vector<CubicNode> nodes = paths.get(pathIndex);
			int s = nodes.size();
			if(nodeIndex<s) {
	 ...
	 * This may be useful if <code>nodeIndex</code> is the last node of a path
	 * and the "next" node is the zeroth node in this path.
	 * 
	 * @param nodeIndex the node index.
	 * @param dest the optional destination for the coordinates.
	 ...
	 * @return the coordinates of the next node.
	 */
	public synchronized Point2D getNextNode(int pathIndex,int nodeIndex,Point2D dest) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
	 ...
	 * This may be useful if <code>nodeIndex</code> is the zeroth node of a path
	 * and the "previous" node is the last node in this path.
	 * 
	 * @param nodeIndex the node index.
	 * @param dest the optional destination for the coordinates.
	 ...
	 * @return the coordinates of the previous node.
	 */
	public synchronized Point2D getPrevNode(int pathIndex,int nodeIndex,Point2D dest) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
	 ...
	 * @return true if this changed this shape.
	 */
	public synchronized boolean setPrevControlForNode(int pathIndex,int nodeIndex,Point2D p) {
	 ...
	 * @return true if this changed this shape.
	 */
	public synchronized boolean setPrevControlForNode(int pathIndex,int nodeIndex,double x,double y) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
		CubicNode node = nodes.get(nodeIndex);
	 ...
	 * @return true if this changed this shape.
	 */
	public synchronized boolean setNextControlForNode(int pathIndex,int nodeIndex,Point2D p) {
	 ...
	 * @return true if this changed this shape.
	 */
	public synchronized boolean setNextControlForNode(int pathIndex,int nodeIndex,double x,double y) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
		CubicNode node = nodes.get(nodeIndex);
	 ...
	 * 
	 * @param nodeIndex the node index.
	 * @param dest the optional destination to store the control point in.
	 * @return may return <code>null</code> if there is no next control point for a node.  Otherwise
	 * this will return <code>dest</code> if it is non-null.
	 */
	public synchronized Point2D getNextControlForNode(int nodeIndex,Point2D dest) {
		for(int pathIndex = 0; pathIndex<paths.size(); pathIndex++) {
			Vector<CubicNode> nodes = paths.get(pathIndex);
			int s = nodes.size();
			if(nodeIndex<s) {
	 ...
	 * @return whether a node has a next control point.
	 */
	public synchronized boolean hasNextControlForNode(int pathIndex,int nodeIndex) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
		CubicNode node = nodes.get(nodeIndex);
	 ...
	 * @return whether a node has a next control point.
	 */
	public synchronized boolean hasPrevControlForNode(int pathIndex,int nodeIndex) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
		CubicNode node = nodes.get(nodeIndex);
	 ...
	 * this will return <code>dest</code> if it is non-null.
	 */	
	public synchronized Point2D getPrevControlForNode(int nodeIndex,Point2D dest) {
		for(int pathIndex = 0; pathIndex<paths.size(); pathIndex++) {
			Vector<CubicNode> nodes = paths.get(pathIndex);
			int s = nodes.size();
			if(nodeIndex<s) {
	 ...
	 * this will return <code>dest</code> if it is non-null.
	 */
	public synchronized Point2D getPrevControlForNode(int pathIndex,int nodeIndex,Point2D dest) {
	 ...
	 * this will return <code>dest</code> if it is non-null.
	 */
	private synchronized Point2D getPrevControlForNode(int pathIndex,int nodeIndex,Point2D dest,double scaleFactor) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
		CubicNode node = nodes.get(nodeIndex);
	 ...
	 * this will return <code>dest</code> if it is non-null.
	 */
	public synchronized Point2D getNextControlForNode(int pathIndex,int nodeIndex,Point2D dest) {
		return getNextControlForNode(pathIndex, nodeIndex, dest, scaleFactor);
	}
	
	private synchronized Point2D getNextControlForNode(int pathIndex,int nodeIndex,Point2D dest,double scaleFactor) {
		Vector<CubicNode> nodes = paths.get(pathIndex);
		CubicNode node = nodes.get(nodeIndex);
	...
	public synchronized int moveTo(float x,float y) {
		releaseIterators();
		Vector<CubicNode> newPath = new Vector<CubicNode>();
		paths.add(newPath);
	 ...
	 */
	public synchronized void curveTo(float cx1,float cy1,float cx2,float cy2,float x2,float y2) {
		curveTo(paths.size()-1,cx1,cy1,cx2,cy2,x2,y2);
	}

	/** This is analogous to <code>GeneralPath.curveTo()</code>.
	 * 
	 * @param pathIndex the path to append this curve to.
	 * @param cx1 the x-coordinate of the first control point.
	 * @param cy1 the y-coordinate of the first control point.
	 * @param cx2 the x-coordinate of the second control point.
	 * @param cy2 the y-coordinate of the second control point.
	 ...
	 * @param y2 the y-coordinate of the end point.
	 */
	public synchronized void curveTo(int pathIndex,float cx1,float cy1,float cx2,float cy2,float x2,float y2) {
		releaseIterators();
		Vector<CubicNode> path = paths.get(pathIndex);
	...
	public final synchronized void append(Shape s) {
		...
		while(i.isDone()==false) {
			...
			if(k==PathIterator.SEG_MOVETO) {
				...
				lastSegment.nextControl = new Point2D.Double(lastSegment.x, lastSegment.y);
				newSegment.prevControl = new Point2D.Double(newSegment.x, newSegment.y );
				...
				lastSegment.nextControl = new Point2D.Double(coords[0], coords[1]);
				newSegment.prevControl = new Point2D.Double(coords[2], coords[3]);
				...
				if(lastSegment.pointEquals(moveSegment)==false) {
					...
					moveSegment.prevControl = new Point2D.Double(
							moveSegment.x, moveSegment.y);
					lastSegment.nextControl = new Point2D.Double(
							lastSegment.x, lastSegment.y );
				} else {
					//our last segment redundantly overlaps with the moveSegment:
	 ...
	 * @return this will be <code>WIND_NON_ZERO</code> or <code>WIND_EVEN_ODD</code>.
	 */
	@Override
	 ...
	 * 
	 * @param newRule the new winding rule.  This must be <code>WIND_NON_ZERO</code> or <code>WIND_EVEN_ODD</code>.
	 */
	public void setWindingRule(int newRule) {
		if(!(newRule==PathIterator.WIND_EVEN_ODD || newRule==PathIterator.WIND_NON_ZERO))
			throw new IllegalArgumentException("the rule ("+newRule+") must be WIND_EVEN_ODD or WIND_NON_ZERO");
	...
	static class CubicNode {
		...
		@Override
		public String toString() {
			return "("+toString(prevControl)+", ("+x+", "+y+"), "+toString(nextControl)+")";
		}
		...
		public boolean isSmooth(boolean equalDistance) {
			if(nextControl==null || prevControl==null)
				return false;
			double angle1 = Math.atan2( nextControl.getY()-y, nextControl.getX()-x );
			double angle2 = Math.atan2( y-prevControl.getY(), x-prevControl.getX() );
			//TODO: does this work?  Do we need to account for border cases?
			boolean anglesEqual = Math.atan(angle1-angle2)<TOLERANCE;
			
			if(!anglesEqual) return false;
			
			if(equalDistance==false) return anglesEqual;
		...
		public boolean setPoint(double x,double y,boolean translateControls) {
			...
			if(translateControls && prevControl!=null) {
				prevControl.setLocation(prevControl.getX()+dx, prevControl.getY()+dy);
			}
			if(translateControls && nextControl!=null) {
				nextControl.setLocation(nextControl.getX()+dx, nextControl.getY()+dy);
			}
		...
		public boolean pointEquals(CubicNode s) {
			return Math.abs(s.x - x)<TOLERANCE && Math.abs(s.y - y)<TOLERANCE;
		}
		...
		public void getPrevControl(Point2D dest,double distanceScale) {
			if(distanceScale==1) {
				...
				dest.setLocation( x+distance*distanceScale*Math.cos(theta), y+distance*distanceScale*Math.sin(theta) );
			}
		}
		...
		public boolean setPrevControl(double x,double y,double distanceScale) {
			...
			if(Math.abs(prevControl.getX()-newX)<TOLERANCE && Math.abs(prevControl.getY()-newY)<TOLERANCE)
				return false;
		...
		public boolean setNextControl(double x,double y,double distanceScale) {
			...
			if(Math.abs(nextControl.getX()-newX)<TOLERANCE && Math.abs(nextControl.getY()-newY)<TOLERANCE)
				return false;
	...
	static class CubicPathIterator implements PathIterator {
		...
		public int currentSegment(double[] coords) {
			...
			if(nodePtr<nodeCount) {
				...
				return SimplifiedPathIterator.simplify(PathIterator.SEG_CUBICTO, 
						point.getX(), point.getY(), coords);
			} else if(nodePtr==nodeCount) {
				//this is the special case where we're trying to close the shape
				//so this cubic curve will arc from node (nodeCount-1) to node 0
			...
			System.err.println("pathPtr = "+pathPtr+", nodePtr = "+nodePtr);
		...
		public void next() {
			...
			if(nodePtr==nodeTotal && src.getNextControlForNode(pathPtr, nodeTotal-1, point, 1)!=null) {
				//we have 2 more segments to write, so this is OK.
			} else if(nodePtr==nodeTotal+1 && src.getNextControlForNode(pathPtr, nodeTotal-1, point, 1)!=null) {
