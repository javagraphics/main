...
/** A collection of static methods to identify if specific curves intersect
 * each other.  All curves are defined in terms of their original bezier
 * control points.
 */
public class Intersections {
	...
	public static boolean lineLine(double x0a,double y0a,double x1a,double y1a,
	...
	public static boolean lineQuad(double x0a,double y0a,double x1a,double y1a,
	...
	/** Adds a value to a sorted list.
	 * If the value already exists: it is ignored.
	 * 
	 * @param array the array the list is stored in
	 * @param size the number of elements in the list
	 * @param newValue the new value to insert into the list.
	 * @return the size of the list.  This may not change if you asked
	...
	public static boolean quadQuad(double x0a,double y0a,double cxa,double cya,double x1a,double y1a,
			double x0b,double y0b,double cxb,double cyb,double x1b,double y1b) {
		...
		try {
			...
			for(int i1 = 0; i1<size1-1; i1++) {
				for(int i2 = 0; i2<size2-1; i2++) {
					if(binary_search(0, ax0, bx0, cx0, 0, ay0, by0, cy0, array1[i1], array1[i1+1], 
							0, ax1, bx1, cx1, 0, ay1, by1, cy1, array2[i2], array2[i2+1] ))
						return true;
				}
			}
			return false;
	 ...
	 * return a list of 4 items (2 times will always be zero and one).
	 * 
	 ...
	 * return a list of 6 items (2 times will always be zero and one).
	 * 
	 * @param dest the array the list is stored in.
	 * @return the number of items in this list.
	 */
	 ...
	 * by defining the a terms as zero.
	 * 
	 * @param ax0 the coefficient of the t^3 term in the first x parametric equation.
	 * @param bx0 the coefficient of the t^2 term in the first x parametric equation.
	 * @param cx0 the coefficient of the t^1 term in the first x parametric equation.
	 * @param dx0 the coefficient of the t^0 term in the first x parametric equation.
	 * @param ay0 the coefficient of the t^3 term in the first y parametric equation.
	 * @param by0 the coefficient of the t^2 term in the first y parametric equation.
	 * @param cy0 the coefficient of the t^1 term in the first y parametric equation.
	 * @param dy0 the coefficient of the t^0 term in the first y parametric equation.
	 * @param startT0 the min t-value to use in the first parametric curve.
	 * @param endT0 the max t-value to use in the first parametric curve.
	 * @param ax1 the coefficient of the t^3 term in the second x parametric equation.
	 * @param bx1 the coefficient of the t^2 term in the second x parametric equation.
	 * @param cx1 the coefficient of the t^1 term in the second x parametric equation.
	 * @param dx1 the coefficient of the t^0 term in the second x parametric equation.
	 * @param ay1 the coefficient of the t^3 term in the second y parametric equation.
	 * @param by1 the coefficient of the t^2 term in the second y parametric equation.
	 * @param cy1 the coefficient of the t^1 term in the second y parametric equation.
	 * @param dy1 the coefficient of the t^0 term in the second y parametric equation.
	 * @param startT1 the min t-value to use in the second parametric curve.
	 * @param endT1 the max t-value to use in the second parametric curve.
	 * @return whether the two curves intersect.
	 */
	private static boolean binary_search(double ax0, double bx0, double cx0, double dx0,
			double ay0, double by0, double cy0, double dy0, double startT0,double endT0,
		...
		boolean intersects = intersects( startX0, startY0, endX0, endY0, startX1, startY1, endX1, endY1);
		...
		while(true) {
			...
			if( (!split0) && (!split1) ) {
				...
				double midX0 = ((ax0*midT0+bx0)*midT0+cx0)*midT0+dx0;
				double midY0 = ((ay0*midT0+by0)*midT0+cy0)*midT0+dy0;

				double midT1 = (startT1+endT1)/2;
				double midX1 = ((ax1*midT1+bx1)*midT1+cx1)*midT1+dx1;
				double midY1 = ((ay1*midT1+by1)*midT1+cy1)*midT1+dy1;

				boolean intersects1 = intersects(startX0, startY0, midX0, midY0, startX1, startY1, midX1, midY1);
				boolean intersects2 = intersects(midX0, midY0, endX0, endY0, startX1, startY1, midX1, midY1);
				boolean intersects3 = intersects(startX0, startY0, midX0, midY0, midX1, midY1, endX1, endY1);
				boolean intersects4 = intersects(midX0, midY0, endX0, endY0, midX1, midY1, endX1, endY1);
				...
				if( (intersects1) && (!intersects2) && (!intersects3) && (!intersects4)) {
					...
					if(intersects1 && binary_search(ax0, bx0, cx0, dx0,
							ay0, by0, cy0, dy0, startT0, midT0,
							ax1, bx1, cx1, dx1,
							ay1, by1, cy1, dy1, startT1, midT1 ) )
						return true;
					
					if(intersects2 && binary_search(ax0, bx0, cx0, dx0,
							ay0, by0, cy0, dy0, midT0, endT0,
							ax1, bx1, cx1, dx1, 
							ay1, by1, cy1, dy1, startT1, midT1 ) )
						return true;

					if(intersects3 && binary_search(ax0, bx0, cx0, dx0,
							ay0, by0, cy0, dy0, startT0, midT0,
							ax1, bx1, cx1, dx1,
							ay1, by1, cy1, dy1, midT1, endT1 ) )
						return true;
					
					if(intersects4 && binary_search(ax0, bx0, cx0, dx0,
							ay0, by0, cy0, dy0, midT0, endT0,
							ax1, bx1, cx1, dx1, 
							ay1, by1, cy1, dy1, midT1, endT1 ) )
						return true;
				...
				boolean intersects1 = intersects(startX0, startY0, midX0, midY0, startX1, startY1, endX1, endY1);
				boolean intersects2 = intersects(midX0, midY0, endX0, endY0, startX1, startY1, endX1, endY1);
				...
				boolean intersects1 = intersects(startX0, startY0, endX0, endY0, startX1, startY1, midX1, midY1);
				boolean intersects2 = intersects(startX0, startY0, endX0, endY0, midX1, midY1, endX1, endY1);
	...
	public static boolean quadCubic(double x0a,double y0a,double cxa,double cya,double x1a,double y1a,
		...
		try {
			int polyASize = definePolygon(array1, x0a, y0a, cxa, cya, x1a, y1a);
			int polyBSize = definePolygon(array2, x0b, y0b, cx0b, cy0b, cx1b, cy1b, x1b, y1b);
			if(polygonPolygon(array1,polyASize,array2,polyBSize)==false)
				return false;
			...
			for(int i1 = 0; i1<size1-1; i1++) {
				for(int i2 = 0; i2<size2-1; i2++) {
					if(binary_search(0, ax0, bx0, cx0, 0, ay0, by0, cy0, array1[i1], array1[i1+1], 
							ax1, bx1, cx1, dx1, ay1, by1, cy1, dy1, array2[i2], array2[i2+1] ))
						return true;
				}
			}
			return false;
	...
	public static boolean cubicCubic(double x0a,double y0a,double cx0a,double cy0a,double cx1a,double cy1a,double x1a,double y1a,
			double x0b,double y0b,double cx0b,double cy0b,double cx1b,double cy1b,double x1b,double y1b) {
		...
		try {
			int polyASize = definePolygon(array1, x0a, y0a, cx0a, cy0a, cx1a, cy1a, x1a, y1a);
			int polyBSize = definePolygon(array2, x0b, y0b, cx0b, cy0b, cx1b, cy1b, x1b, y1b);
			if(polygonPolygon(array1,polyASize,array2,polyBSize)==false)
				return false;
			...
			for(int i1 = 0; i1<size1-1; i1++) {
				for(int i2 = 0; i2<size2-1; i2++) {
					if(binary_search(ax0, bx0, cx0, dx0, ay0, by0, cy0, dy0, array1[i1], array1[i1+1], 
							ax1, bx1, cx1, dx1, ay1, by1, cy1, dy1, array2[i2], array2[i2+1] ))
						return true;
				}
			}
	...
	protected static boolean linePolygon(double x0,double y0,double x1,double y1,double[] coords,int coordCount) {
		...
		//OK, we confirmed there isn't an intersection.  But the polygon might still contain the original line:
	...
	protected static boolean polygonPolygon(double[] coords1,int coordCount1,double[] coords2,int coordCount2) {
		for(int a = 0; a<coordCount1; a++) {
			...
			for(int b = 0; b<coordCount2; b++) {
				...
				if(Line2D.linesIntersect(coords1[index1a*2], coords1[index1a*2+1], coords1[index2a*2], coords1[index2a*2+1], 
						coords2[index1b*2], coords2[index1b*2+1], coords2[index2b*2], coords2[index2b*2+1]))
					return true;
			}
		}
     ...
     * <i>insideness</i>, see the class comments of <code>Shape</code>.
     * <p>Copied form java.awt.Polygon.java.
	...
	public static boolean lineCubic(double x0a,double y0a,double x1a,double y1a,
			double x0b,double y0b,double cx0b,double cy0b,double cx1b,double cy1b,double x1b,double y1b) {
		double[] array;
		synchronized(doubleArrays) {
			if(doubleArrays.size()==0) {
		...
		try {
			int polySize = definePolygon(array,x0b,y0b,cx0b,cy0b,cx1b,cy1b,x1b,y1b);
			...
			for(int a = 0; a<results; a++) {
				if(array[a]>=0 && array[a]<=1) {
					double x = ((ax*array[a]+bx)*array[a]+cx)*array[a]+dx;
					if(minX <= x && x <= maxX)
						return true;
				}
			}
