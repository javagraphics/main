 ...
 * <p>TODO: this is loosely based on my perception of how the Google Docs
...
public class CurvedPolyline extends AbstractShape {
	...
	public static class CurvedPolylineIterator implements PathIterator {
		...
		/**
		 * 
		 * @param array an array of points to create this shape from.
		 * @param windingRule one of the winding constants (PathIterator.WIND_NONZERO or PathIterator.WIND_EVEN_ODD)
		 * @param tx an optional AffineTransform to transform this data (may be null).
		 */
		public CurvedPolylineIterator(Point2D[] array,int windingRule,AffineTransform tx) {
			...
			for(int a = 0; a<array.length; a++) {
				if(a==0 || a==array.length-1) {
					points[a] = new Point2D.Double(array[a].getX(), array[a].getY());
				} else {
					Point2D prev = array[a-1];
					Point2D next = array[a+1];
					double theta = Math.atan2(next.getY()-prev.getY(), next.getX()-prev.getX());
					points[a] = new SplinePoint2D( array[a].getX(), array[a].getY(), theta);
				}
			}
			this.tx = tx;
			this.windingRule = windingRule;
			if(!(windingRule==PathIterator.WIND_NON_ZERO || windingRule==PathIterator.WIND_EVEN_ODD))
				throw new IllegalArgumentException("windingRule = "+windingRule);
		}
		...
		public int currentSegment(double[] coords) {
			try {
				...
				if(points[index-1] instanceof SplinePoint2D && points[index] instanceof SplinePoint2D) {
	...
	public synchronized Point2D getPoint(int index) {
		return new Point2D.Double( points.get(index).getX(), points.get(index).getY() );
	}
	
	/** Redefine an existing point.
	 * 
	 * @param index the index of the point to redefine.
	...
	/** This expresses this shape as a series of points, 
	 * written as "m [X] [Y] l [X] [Y] l [X] [Y] ..."
	 * Note that although this shape includes cubic data,
	 * it is expressed simply as points (linear data).
	 */
	@Override
	...
	public synchronized PathIterator getPathIterator(AffineTransform at) {
		...
		for(int a = 0; a<pointArray.length; a++) {
			pointArray[a] = new Point2D.Double( pointArray[a].getX(), pointArray[a].getY() );
		}
	...
	/** Set a winding rule.
	 * 
	 * @param newWindingRule <code>PathIterator.WIND_EVEN_ODD</code> or
	 * <code>PathIterator.WIND_NON_ZERO</code>.
	 * @return true if a change occurred.
	 */
	public synchronized boolean setWindingRule(int newWindingRule) {
		if(newWindingRule==PathIterator.WIND_EVEN_ODD || newWindingRule==PathIterator.WIND_NON_ZERO) {
