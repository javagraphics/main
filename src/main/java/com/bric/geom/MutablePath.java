 ...
 *<P>This shape has the notion of several distinct
 *paths where each path begins with a separator <code>moveoTo()</code>
 *instruction.  There are some methods that treat this object
 *as one long string of segments, and some methods that recognize
 *that you can access each path separately.
 *<P>Several methods here match the signatures of <code>GeneralPath</code>
 *methods for convenience.  Internally field and methods are made
 *private/final because of the thorough verification that happens as you
 *manipulate this object: subclasses shouldn't let you position anything
 *other than a SEG_MOVETO after a SEG_CLOSE, for example.
 */
public class MutablePath extends AbstractShape {

	private Vector<Vector<MutablePathSegment>> paths = new Vector<Vector<MutablePathSegment>>();
	int windingRule = PathIterator.WIND_NON_ZERO;
	private Vector<WeakReference<MutablePathIterator>> iterators = new Vector<WeakReference<MutablePathIterator>>();
	...
	protected synchronized MutablePathSegment peek(int pathIndex) {
		...
		Vector<MutablePathSegment> segments = paths.get(pathIndex);
	...
	public synchronized int moveTo(double x,double y) {
		...
		Vector<MutablePathSegment> newSegments = new Vector<MutablePathSegment>();
	...
	public synchronized void quadTo(int pathIndex,double cx,double cy,double x,double y) {
		releaseIterators();
	...
	public synchronized void curveTo(double cx0,double cy0,double cx1,double cy1,double x,double y) {
		int i = paths.size()-1;
		if(i<0)
			throw new RuntimeException("all paths must begin with a moveTo statement");
	...
	public synchronized void delete(int pathIndex,int segmentIndex) {
		...
		if(segmentIndex==0 && segments.size()>1) {
			...
			if(newHead.type==PathIterator.SEG_CLOSE) {
				...
				throw new RuntimeException("unexpected segment type: "+newHead.type);
			}
		}
	...
	protected synchronized void releaseIterators() {
		while(iterators.size()>0) {
			WeakReference<MutablePathIterator> ref = iterators.remove(iterators.size()-1);
	...
	public synchronized int getSegmentCount() {
		...
		for(int a = 0; a<paths.size(); a++) {
			Vector<MutablePathSegment> segments = paths.get(a);
			sum += segments.size();
		}
		return sum;
	}
	
	public synchronized int getSegment(int pathIndex,int segmentIndex,double[] coords) {
		Vector<MutablePathSegment> segments = paths.get(pathIndex);
	...
	public synchronized int getSegment(int pathIndex,int segmentIndex,float[] coords) {
		Vector<MutablePathSegment> segments = paths.get(pathIndex);
		MutablePathSegment s = segments.get(segmentIndex);
		for(int b = 0; b<s.data.length; b++) {
	...
	public synchronized int getSegment(int index,double[] coords) {
		...
		throw new IllegalArgumentException("no segment available at index "+originalIndex);
	}
	...
	public synchronized boolean setSegment(int pathIndex,int segmentIndex,int type,double[] coords) {
		Vector<MutablePathSegment> segments = paths.get(pathIndex);
		MutablePathSegment s = segments.get(segmentIndex);
		...
		releaseIterators();
		int oldType = s.type;
		if( oldType==PathIterator.SEG_MOVETO && type!=PathIterator.SEG_MOVETO) {
			...
			if(pathIndex==0) {
				...
				Vector<MutablePathSegment> previousSegments = paths.get(pathIndex-1);
				MutablePathSegment tail = previousSegments.get(previousSegments.size()-1);
				if(tail.type==PathIterator.SEG_CLOSE) {
		...
		} else if(oldType!=PathIterator.SEG_MOVETO && type==PathIterator.SEG_MOVETO) {
			//this used to be a regular segment, but not it needs to mark the beginning
		...
		} else if(oldType!=PathIterator.SEG_CLOSE && type==PathIterator.SEG_CLOSE) {
	...
	/** Redefine a segment.  This lets you do anything without an exception:
	 * you could technically make a path begin a SEG_CLOSE, or place ten
	 * SEG_MOVETOs next to each other.
	 * 
	 * @param index
	 * @param type
	 * @param coords
	...
	public synchronized PathIterator getPathIterator(AffineTransform at) {
		MutablePathIterator i = new MutablePathIterator(paths,windingRule,at,this);
...
class MutablePathSegment {
	...
	private static boolean equals(double[] array1,double[] array2) {
		...
		for(int a = 0; a<array1.length; a++) {
			if(array1[a]!=array2[a]) return false;
		}
		return true;
	}
	...
	public boolean setData(int type,double[] data) {
		if(!(type==PathIterator.SEG_MOVETO || type==PathIterator.SEG_CLOSE ||
				type==PathIterator.SEG_CUBICTO || type==PathIterator.SEG_LINETO ||
		...
		if(type==PathIterator.SEG_CLOSE) {
			if(data.length>0)
				throw new IllegalArgumentException("close segments cannot have data ("+toString(data)+")");
		} else if(type==PathIterator.SEG_MOVETO) {
			if(data.length<2)
				throw new IllegalArgumentException("moveTo segments must have 2 coordinates ("+toString(data)+")");
		} else if(type==PathIterator.SEG_LINETO) {
			if(data.length<2)
				throw new IllegalArgumentException("lineTo segments must have 2 coordinates ("+toString(data)+")");
		} else if(type==PathIterator.SEG_QUADTO) {
			if(data.length<4)
				throw new IllegalArgumentException("quadTo segments must have 4 coordinates ("+toString(data)+")");
		} else if(type==PathIterator.SEG_CUBICTO) {
			if(data.length<6)
				throw new IllegalArgumentException("cubicTo segments must have 6 coordinates ("+toString(data)+")");
		}
		return true;
	}
	...
	public MutablePathSegment(double cx0, double cy0,double cx1,double cy1,double  
			x,double y) {
		setData(PathIterator.SEG_CUBICTO,new double[] {cx0, cy0, cx1, cy1,  
				x, y});
	}
 ...
 * <P>This uses a peculiar approach to accessing the path data:
 * the data is not cloned unless <code>release()</code> is called.
 * Until then this points to the data in the <code>MutablePath</code>
 * it was created from.  This is designed to avoid large memory
 * demands unless absolutely necessary.
 * <P>(<code>release()</code> is called when the parent <code>MutablePath</code>
 * is modified.  So as long as the parent remains unchanged there is no need
 * to clone all its segment data.)
 */
class MutablePathIterator implements PathIterator {
	...
	public MutablePathIterator(Vector<Vector<MutablePathSegment>> pathList,int windingRule,AffineTransform transform,Object syncLock) {
		winding = windingRule;
		if(transform!=null) {
	...
	public synchronized void release() {
		...
		synchronized(synchronizeAgainst) {
			...
			for(int p = pathPtr; p<paths.size(); p++) {
				...
				Vector<MutablePathSegment> segments = paths.get(p);
	...
	public int currentSegment(float[] coords) {
		...
		if(synchronizeAgainst!=null) {
			synchronized(synchronizeAgainst) {
				Vector<MutablePathSegment> segments = paths.get(pathPtr);
