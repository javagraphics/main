...
/** This is a class that implements all <code>Shape</code> methods
 * except for <code>getPathIterator(AffineTransform)</code>.
 * Because all the other methods consult <code>getPathIterator()</code>
 * it is essential that when subclasses implement that method it be
 * very light.  See <code>MutablePath</code> as a possible example.
 */
public abstract class AbstractShape implements Shape {
	...
	public boolean contains(Rectangle2D r,AffineTransform transform) {
		return contains(r.getX(), r.getY(), r.getWidth(), r.getHeight(), transform);
	}
	 ...
	 * This total is used in combination with the winding rule to determine
		...
		while(iter.isDone()==false) {
			...
			if(type==PathIterator.SEG_MOVETO) {
				...
				if(type==PathIterator.SEG_LINETO) {
					inside = y>=Math.min(data[1],lastY) && y<=Math.max(data[1],lastY);
					if(inside) {
						if (lastY > data[1]) {
					...
					if(!inside) {
						//not inside yet?  Then let's do the slightly harder test:
						double focusT = -by/(2*ay);
						if(focusT>=0 && focusT<=1) {
							double focusY = ay*focusT*focusT+by*focusT+cy;
					...
					if(!inside) {
						...
						if(det<0) {
							...
							double t = -2*by/(6*ay);
							if(t>0 && t<1) {
								double newY = ay*t*t*t+by*t*t+cy*t+dy;
	 ...
	 * This method returns immediately when/if it finds proof of an intersection.
	 * 
	 */
	protected boolean identifyCrossings(double x,double y,double w,double h,AffineTransform transform) {
	...
	public boolean intersects(double x, double y, double w, double h, AffineTransform transform) {
	...
	public boolean contains(double x, double y, double w, double h,AffineTransform transform) {
		if(identifyCrossings(x,y,w,h,transform)) {
			return false;
		}
