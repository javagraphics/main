...
public class MouseTracker {
	...
	private static Hashtable<Integer, Boolean> pressedButtons = new Hashtable<Integer, Boolean>();
	static {
			...
			public void eventDispatched(AWTEvent e) {
				if(e instanceof MouseEvent) {
					...
					if(k.getID()==MouseEvent.MOUSE_PRESSED) {
						 ...
						 */
						Enumeration<Boolean> values = pressedButtons.elements();
						while(values.hasMoreElements()) {
							if(Boolean.TRUE.equals(values.nextElement()))
								change = true;
						}
	...
	public synchronized static boolean isButtonPressed() {
		Enumeration<Integer> e = pressedButtons.keys();
		while(e.hasMoreElements()) {
			Boolean b = pressedButtons.get(e.nextElement());
