 ...
 * <p>This goes to extra lengths to reduce color banding more than Java's
 * default gradient implementation, and is generally comparable in performance.
...
public class GradientTexturePaint implements Paint {
	...
	public static enum Cycle { LOOP, NONE, TILE };
	...
	static class DiffusedTextureSource extends TextureSource {
		DiffusedTextureSource(Color[] colors,float[] fractions,double distance) {
			...
			/** The width (in pixels) of the image we used to render this gradient.
			 * This isn't an exact science, but a small value (of less than 10)
			 * can easily reveal to the human eye where the tiling repeats, so I want
			 * to try to make that less visible
			 */
			int depth = 16;
					...
					search : {
						for(int a = 0; a<fractions.length-1; a++) {
							...
							if(f>=fractions[a] && f<=fractions[a+1]) {
								float rel = (f-fractions[a])/(fractions[a+1]-fractions[a]);
			...
			//seed the array:
			int[][] kernel = new int[][] {{1, 3, 5, 3, 1}, {3, 5, 7, 5, 3}, {5, 7, 0, 7, 5}, {3, 5, 7, 5, 3}, {1, 3, 5, 3, 1}};
					...
					if( isSeed(x, y) ) {
						...
						for(int i = 0; i<kernel.length; i++) {
							for(int j = 0; j<kernel[i].length; j++) {
								int y2 = (y+i-kernel.length/2+size)%size;
								int x2 = (x+j-kernel[0].length/2+depth)%depth;
								if( !isSeed(x2, y2) ) {
									localSum += kernel[i][j];
								}
							}
						...
						for(int i = 0; i<kernel.length; i++) {
							for(int j = 0; j<kernel[i].length; j++) {
								...
								int x2 = (x+j-kernel[0].length/2+depth)%depth;
								if( !isSeed(x2, y2) ) {
									redE[ y2 ][ x2 ] -= newRedError*kernel[i][j]/localSum;
									greenE[ y2 ][ x2 ] -= newGreenError*kernel[i][j]/localSum;
									blueE[ y2 ][ x2 ] -= newBlueError*kernel[i][j]/localSum;
									alphaE[ y2 ][ x2 ] -= newAlphaError*kernel[i][j]/localSum;
								}
							}
						}
					}
			 ...
			 *  [ 1, 3, 5, 3, 1] ]</pre>
			 */
			kernel = new int[][] {{0, 0, 0, 7, 5}, {3, 5, 7, 5, 3}, {1, 3, 5, 3, 1}};
			...
			for(int y = 0; y<size; y++) {
				for(int x = 0; x<depth; x++) {
					if( !isSeed(x,y) ) {
						...
						for(int i = 0; i<kernel.length; i++) {
							for(int j = 0; j<kernel[i].length; j++) {
								redE[ (y+i)%size ][ (x+j-kernel[0].length/2+depth)%depth] -= newRedError*kernel[i][j]/kernelSum;
								greenE[ (y+i)%size ][ (x+j-kernel[0].length/2+depth)%depth] -= newGreenError*kernel[i][j]/kernelSum;
								blueE[ (y+i)%size ][ (x+j-kernel[0].length/2+depth)%depth] -= newBlueError*kernel[i][j]/kernelSum;
								alphaE[ (y+i)%size ][ (x+j-kernel[0].length/2+depth)%depth] -= newAlphaError*kernel[i][j]/kernelSum;
							}
						}
					}
				}
			...
			BufferedImage image = new BufferedImage(size,depth,BufferedImage.TYPE_INT_ARGB);
			for(int x = 0; x<size; x++) {
				image.getRaster().setDataElements(x, 0, 1, depth, argbQ[x]);
			}
			texturePaint = new TexturePaint(image, new Rectangle(0,0,size,depth));
		}
		
		private static boolean isSeed(int x,int y) {
			if(!seedingEnabled) return false;
			return x%2==0 && y%2==0;
		}
	...
	/** This is the simplest texture source, but it may
	 * introduce color banding if similar colors
	 * are stretched over a long distance.
	 */
	...
	private static void boundsCheck(Color[] colors, float[] fractions) {
		if(colors.length!=fractions.length)
			throw new IllegalArgumentException("The size of the colors array ("+colors.length+") must equal the size of the fractions array ("+fractions.length+")");
		for(int a = 0; a<fractions.length; a++) {
			if(fractions[a]<0 || fractions[a]>1)
				throw new IllegalArgumentException("At least one fractional value was not between [0,1]. (fractions["+a+"] = "+fractions[a]+")");
			if(a!=0) {
				if(fractions[a-1]>fractions[a])
					throw new IllegalArgumentException("At least one fractional value was less than the previous value. (fractions["+(a-1)+"] = "+fractions[a-1]+", fractions["+(a)+"] = "+fractions[a]+")");
			}
		}
	}
	 ...
	 * @param colors the colors to check for alpha components
	 * @return true if any of the colors in the argument are
	 * not fully opaque.
	 */
	 ...
	 * 
	 * @param colors the colors to use in this gradient.
	 * @param fractions the fractions to use in this gradient.  These
	 * must be ascending fractions from [0,1].  Each fraction
	 ...
	 */
	public GradientTexturePaint(float x1,float y1,Color c1,float x2,float y2,Color c2) {
		this(new Color[] {c1, c2}, new float[] {0, 1}, x1, y1, x2, y2, Cycle.NONE);
	}
	 ...
	 * @param x2 the x-value of the second coordinate.
	 * @param y2 the y-value of the second coordinate.
	 * @param cycle TILE is recommended for best performance.
	 */
	public GradientTexturePaint(Color[] colors,float[] fractions,float x1,float y1,float x2,float y2,Cycle cycle) {
		boundsCheck(colors, fractions);
		...
		if(cycle==Cycle.LOOP) {
			...
			int newArraySize = colors.length*2 - (fractions[fractions.length-1]<1 ? 0 : 1);
	 ...
	 * @param cycle TILE is recommended for best performance.
	 */
	public GradientTexturePaint(Color[] colors, float[] fractions,Point2D p1, Point2D p2,Cycle cycle) {
	...
	public PaintContext createContext(ColorModel cm, Rectangle deviceBounds,
		...
		if(hints==null)
			hints = new RenderingHints(new Hashtable<RenderingHints.Key, Object>());
		
		boolean useDiffusion = 
			RenderingHints.VALUE_DITHER_ENABLE.equals( hints.get(RenderingHints.KEY_DITHERING) ) ||
			RenderingHints.VALUE_DITHER_DEFAULT.equals( hints.get(RenderingHints.KEY_DITHERING) ) ||
			RenderingHints.VALUE_COLOR_RENDER_QUALITY.equals( hints.get(RenderingHints.KEY_COLOR_RENDERING) ) ||
			RenderingHints.VALUE_RENDER_QUALITY.equals( hints.get(RenderingHints.KEY_RENDERING) );
		
		TextureSource source;
		if(useDiffusion && needsDiffusion) {
			if(diffusedSource==null) diffusedSource = new DiffusedTextureSource(colors, fractions, Point2D.distance(x1, y1, x2, y2));
			source = diffusedSource;
		} else {
			if(simpleSource==null) simpleSource = new SimpleTextureSource(colors, fractions, Point2D.distance(x1, y1, x2, y2));
			source = simpleSource;
		}
		 ...
		 * The edges will appear much less pixelated if the interpolation
		 * is set to BILINEAR or BICUBIC.  Because this is probably
		 * what the developer has in mind when (s)he turns on antialiasing,
		 * we'll make that switch here for them.
		 */
		if(RenderingHints.VALUE_ANTIALIAS_ON.equals(hints.get(RenderingHints.KEY_ANTIALIASING))) {
			//I'm not sure if "hints" is a shared object or if we should make a clone.
			//To be safe, clone it:
			RenderingHints copy = new RenderingHints(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
			copy.putAll(hints);
			hints = copy;
			hints.put(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
		}
		
		PaintContext context =  source.texturePaint.createContext(cm, deviceBounds, userBounds, newTransform, hints);
		if(cycle==Cycle.NONE) {
			context = new CoveredContext(context, x1, y1, colors[0].getRGB(), 
					x2, y2, colors[colors.length-1].getRGB(), 
					xform);
		}
		return context;
	}
	
	/** Return the colors in this gradient. Each color corresponds to
	 * a value in {link #getFractions()}.
	 * 
	...
	/** Return the fractions in this gradient. Each fraction corresponds to
	 * a value in {link #getColors()}.
	 * 
	 ...
	 * gradient are opaque, otherwise this returns <code>Transparency.TRANSLUCENT</code>.
	 * 
