...
import com.bric.awt.CalligraphyPathWriter;
import com.bric.geom.GeneralPathWriter;
import com.bric.geom.ShapeBounds;
import com.bric.geom.TransformUtils;

/** This is a basic 3D representation of a letter.
 * It has a fixed depth, so the dimensions of this effect
 * can be calculated as:
 * <br>double dx = getDepth()*Math.cos(angle);
 * <br>double dy = getDepth()*Math.sin(angle);
 */
public abstract class BlockLetter {
	...
	public static class Simple extends BlockLetter {
		...
		/** Return the 2 shapes required to render the depth of
		 * this BlockLetter. The first shape should correspond to
		 * <code>block1Paint</code> and the second shape should
		 * correspond to <code>block2Paint</code>.
		 * 
		 * @return the two shapes required to render the depth
		 * of this BlockLetter.
		 */
		protected Shape[] getDepthBlocks() {
			...
			CalligraphyPathWriter cpw = new CalligraphyPathWriter(angle, 0, depth, dest1, dest2);
			cpw.write(outline);
	...
	/** This BlockLetter supports two different types of gradients.
	 * This renders the gradient as a series of lines (or polygons)
	...
	public static class Gradient extends BlockLetter {
		...
		class SheetComparator implements Comparator<Sheet> {
			...
			public int compare(Sheet o1, Sheet o2) {
				...
				if(t==0) t = compare(o1.x2, o2.x2);
				if(t==0) t = compare(o1.y1, o2.y1);
				if(t==0) t = compare(o1.y2, o2.y2);
				if(t==0) t = compare(o1.theta, o2.theta);
				return t;
			}
		...
		public void paintDepth(Graphics2D g, float x, float y) {
			...
			for(Sheet sh : sheets) {
				...
				if(colors.length==1) {
					...
					final AffineTransform tx = TransformUtils.createAffineTransform(
						0, 0,
						1, 0,
						0, 1,
						sh.x1, sh.y1,
						sh.x1 + Math.cos(angle)*depth,
						sh.y1 + Math.sin(angle)*depth,
						sh.x2, sh.y2
					);
					GradientPaint paint = new GradientPaint( 0, 0, 
							...
							colors[1] ) {
								...
								public PaintContext createContext(ColorModel cm,
									...
									t.concatenate(xform);
									t.concatenate(tx);
									return super.createContext(cm, deviceBounds, userBounds, t, hints);
								}
					};
					g2.setPaint(paint);
		 ...
		 * <p>These colors might be a function of theta.
		 * <p>This may return 1 color, in which case the sheet will
		 * be a solid color. Or this might return 2 colors, in which case
		 * the first color will be the color touching the character glyph
		 * and the second color will be the color furthest away.
		 * 
		 * @param theta the angle (in radians) tangent to the glyph shape.
		 * @return the colors used to color this sheet.
		 */
	...
	protected Hashtable<String, Object> table = null;
	...
	/** Return the total depth of this BlockLetter. This is
	 * the hypotenuse of displacement; to identify the x and y
	 * displacement use:
	 * <br>double dx = getDepth()*Math.cos(angle);
	 * <br>double dy = getDepth()*Math.sin(angle);
	 * <p>If this is zero, then this BlockLetter should simply render
	 * as the original character glyph.
	 * 
	 ...
	 * @param x the x-coordinate indicating where to paint.
	 * @param y the y-coordinate indicating where to paint.
	 */
	 ...
	 * <P>If the method <code>paintDepth()</code> isn't invoked, then
	 * this method will still present normal-looking (non-3D) text.
	 * @param g the graphics to draw to
	 * @param x the x-coordinate indicating where to paint.
	 * @param y the y-coordinate indicating where to paint.
	 */
	...
	/** Prepares this Graphics2D to paint this BlockLetter at the given (x,y) coordinate.
	 * 
	 * @param g the Graphics2D to paint to
	 * @param x the x-coordinate to paint to
	 * @param y the y-coordinate to paint to
	 * @return a cloned Graphics2D to paint to. When complete you should dispose this Graphics2D.
	 */
