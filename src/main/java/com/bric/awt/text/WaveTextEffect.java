 ...
 * @(#)WaveTextEffect.java
 *
 * $Date: 2014-08-18 10:05:17 +0200 (Mo, 18 Aug 2014) $
 *
...
public class WaveTextEffect implements TextEffect {
	...
	Vector<BlockLetter.Simple> blockLetters = new Vector<BlockLetter.Simple>();
	float textWidth = 0;
	float textHeight = 0;
		...
		for(int a = 0; a<text.length(); a++) {
			...
			if(Character.isWhitespace(c)==false) {
				BlockLetter.Simple l = new BlockLetter.Simple(c, font, foreground) {
					...
					protected void paintOutline(Graphics2D g,float x,float y,Paint paint,Stroke stroke) {
						Graphics2D g2 = (Graphics2D)g.create();
						g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
						g2.translate(x - depth*Math.cos(angle), y - depth*Math.sin(angle));
						g2.setStroke(stroke);
						g2.setPaint(paint);
						g2.draw(outline);
						g2.dispose();
