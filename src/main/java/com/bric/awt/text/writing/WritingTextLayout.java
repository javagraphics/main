...
/** Analogous to a <code>java.awt.font.TextLayout</code>, this helps
 * lay out glyphs in a WritingFont.
 */
public class WritingTextLayout {

	public static final Key<String> TEXT = new Key<String>("text", String.class);	
	public static final Key<WritingFont> FONT = new Key<WritingFont>("font", WritingFont.class);
	public static final Key<Float> FONT_SIZE = new Key<Float>("font-size", Float.class);
	public static final Key<Float> BEATS_PER_SECOND_KEY = new Key<Float>("beats-per-second", Float.class);
	public static final Key<Float> PIXELS_PER_SECOND_KEY = new Key<Float>("pixels-per-second", Float.class);
	private static final Key<Object> GLYPHS_KEY = new Key<Object>("glyphs", Object.class );
	...
	public WritingTextLayout() {
		...
		addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				if(BEATS_PER_SECOND_KEY.matches(evt) || PIXELS_PER_SECOND_KEY.matches(evt))
					return;
				invalidate();
			}
		});
	 ...
	 * and outside callers may also choose to invoke this to flush data from memory
	 * if a layout is no longer visible.
	 */
	...
	public void paint(Graphics2D g, Rectangle rectangle, float time) {
		...
		try {
			...
			g2.clipRect(rectangle.x, rectangle.y, rectangle.width, rectangle.height);
			int y = rectangle.y;
	...
	class Glyph {
		...
		Glyph(char ch) {
			...
			ascent = totalHeight / 
					( 1f + getFont().getProperty(WritingFont.DESCENT) + getFont().getProperty(WritingFont.LEADING) );
		...
		protected WritingShape createEmptyGlyph(float widthToHeighRatio) {
			if(widthToHeighRatio<=0) throw new IllegalArgumentException();
	...
	class Line {
		 ...
		 * 
		 * @param g
		 * @param leftX the x-coordinate of the top-left corner of this line's bounds.
		 * @param topY the y-coordinate of the top-left corner of this line's bounds.
		 * @param time
			...
			try {
				...
				for(int a = 0; a<glyphs.size(); a++) {
					Rectangle2D r = new Rectangle2D.Double(
							x, 0, glyphs.get(a).width, glyphs.get(a).ascent);
					boolean finished = glyphs.get(a).shape.paint(g2, r, time, beatsPerSecond, pixelsPerSecond);
					if(finished) {
						time -= glyphs.get(a).shape.getDuration( beatsPerSecond, pixelsPerSecond );
					} else {
						return false;
					}
	...
	protected Glyph[] getGlyphs() {
		Glyph[] glyphs = (Glyph[])properties.get(ObservableProperties.TRANSIENT, GLYPHS_KEY);
		if(glyphs==null) {
			String text = getText();
	...
	public float getDuration() {
		...
		for(int a = 0; a<glyphs.length; a++) {
			sum += glyphs[a].shape.getDuration( getBeatsPerSecond(), getPixelsPerSecond() );
		}
		return sum;
	}
	...
	class WTLAnimationReader implements AnimationReader, Resettable {
		...
		int frame = 0;
		WTLAnimationReader(int w,boolean transparentBackground,int fps,boolean loop) {
			if(fps<=0) throw new IllegalArgumentException(fps+" must be a positive number");
			if(w<=0) throw new IllegalArgumentException(w+" must be a positive number");
		...
		public BufferedImage getNextFrame(boolean cloneImage)
			...
			/** This option paints the animation against a solid background, and then
			 * makes the remaining pixels transparent.
			 * 
			 */
	...
	/** This reader may rely on this WritingTextLayout, so if the layout
	 * is changed then the reader may become unreliable.
	 */
	public AnimationReader createAnimation(int width,boolean transparentBackground,int fps,boolean loop) {
