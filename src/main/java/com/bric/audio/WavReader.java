...
public class WavReader {
	...
	private static void main(String[] s) {
		try {
			...
			if(!isValidSizeHeader(wavFile)) {
				System.out.println("identified corrupt size headers: "+wavFile.getAbsolutePath());
	 ...
	 * <p>A <code>RandomAccessFile</code> is used to redefine the size
	 * of the "RIFF" and "data" chunks based on the real file size.
	...
	public static void fixCorruptSizeFields(File wavFile) throws IOException {
		...
		try {
			...
			if(!"RIFF".equals(chunkID))
				throw new IOException("the file did not begin with a RIFF header: "+wavFile.getAbsolutePath());
			...
			if(!"WAVE".equals(chunkID))
				throw new IOException("the file did not contain a WAVE identifier: "+wavFile.getAbsolutePath());
			...
			while(raf.getFilePointer()<realFileSize) {
				...
				if("data".equals(chunkID)) {
					long dataChunkSize = realFileSize - raf.getFilePointer() - 4;
	 ...
	 * <p>This method was written in response to QTJ's MovieExporter
	 * occasionally converting MP3s to a wave file incorrectly. The
	 * resulting wave file contained all the correct PCM data, but
	 * the chunk size of the "RIFF" and "data" chunks are zero.
	 * <p>This method only validates the first header. If this method
	 * returns false, the method <code>fixCorruptSizeFields()</code>
	 * should fix the problem.
	 * 
	 * @param wavFile
	 * @return true if the the "RIFF" size header is consistent with
	 * the real file size.
	 */
	...
	public static AudioInputStream createAudioInputStream(File file) throws IOException {
	 ...
	 * <p>This was mostly an exercise on my part, I do not have any reason
	 * to recommend this method over using AudioSystem's default wave file
	 * support.
	 * 
	 * @param in the input to create an AudioInputStream for.
	 * @return an AudioInputStream based on the argument.
	 * @throws IOException if an IO error occurs
	 */
	public static AudioInputStream createAudioInputStream(InputStream in) throws IOException {
	...
	public WavReader(InputStream in) throws IOException {
		...
		if(!chunkID.equals("RIFF")) {
			println(bytes, 4);
			throw new IOException("This stream began with \""+chunkID+"\" instead of \"RIFF\"");
		}
		size = readLong(4)+8;
		String riffType = readString(4);
		if(!riffType.equals("WAVE")) {
			println(bytes, 4);
			throw new IOException("This RIFF type was \""+riffType+"\" instead of \"WAVE\".");
		}
	}
	

	/** Returns an AudioFormat object based on the last WavFormatChunk read.
	 * Note this will be null until <code>read()</code> is called.
	 * 
	...
	public AudioFormat getAudioFormat() {
		...
		return new AudioFormat(
			(lastFormatChunk.sampleRate)/1000f,
			lastFormatChunk.sigBitsPerSample,
			lastFormatChunk.numChannels,
			lastFormatChunk.sigBitsPerSample!=8,
			false );
	}
	...
	public void read() throws IOException {
		while(this.in.getReadBytes()!=size) {
			...
			try {
				...
				/** T4L bug 15259:
				* Some encoders must write
				* a byte "0" to signify EOF?
				*/
				return;
			}
	...
	/** Returns the last WavFormatChunk read.
	 * Note this will be null until <code>read()</code> is called.
	 * 
	...
	protected synchronized void readChunk(String id,long size) throws IOException {
		if(id.equals("fmt ")) {
			...
			if(lastFormatChunk==null) {
				System.err.println("no format chunk available; unable to interpret data.");
				skip(size);
			} else if(lastFormatChunk.compressionCode!=WavFormatChunk.COMPRESSION_PCM) {
				System.err.println("unsupported compression code: "+lastFormatChunk.compressionCode+"; unable to interpret data.");
				skip(size);
			} else {
	...
	*/
	 ...
	 * 
	 * @param len the number of bytes this number requires.
	 * @return the number.
	 */
	...
	/** Reads a number.
	 * 
	 * @param bytes the array the number is stored in.
	 * @param len the number of bytes this number requires.
	 * @return the number.
	 */
	protected static int readInt(byte[] bytes,int offset,int len) throws IOException {
		if(len>4)
	 ...
	 * @return the number.
	 */
	protected static long readLong(byte[] bytes,int offset,int len) throws IOException {
		if(len>8)
	 ...
	 * 
	 * @param len the number of bytes this string requires.
	 * @return the string.
	 */
	...
	/** Reads a string.
	 * 
	 * @param bytes the array this string is stored in.
	 * @param len the number of bytes this string requires.
	 * @return the string.
	 */
	...
	private static synchronized void read(InputStream in,byte[] dest,int amt) throws IOException {
		int off = 0;
		while(off<amt) {
			...
			if(read==-1) {
				...
				if(off==0) //if we read nothing
					throw new EmptyReadException();
				throw new EOFException("expected "+amt+" bytes, but only "+off+" were read before the stream ended.");
			}
			off+=read;
		}
	}
	
	private static synchronized void read(RandomAccessFile raf,byte[] dest,int amt) throws IOException {
		int off = 0;
		while(off<amt) {
	...
	private synchronized void skip(long amt) throws IOException {
		...
		while(off<amt) {
			...
			if(read==-1) throw new IOException("expected "+amt+" bytes, but only "+off+" were skipped before the stream ended.");
			off+=read;
		}
	}
