/*
 * @(#)StretchedAudioInputStream.java
 *
...
/** This stretches an AudioInputStream so it plays back slower or faster
 * than normal.
 */
public class StretchedAudioInputStream extends AudioInputStream {
	...
	private StretchedAudioInputStream(InputStream input,AudioFormat format,long frames) {
		super( input, format, frames);
	}

	/** Create a StretchedAudioInputStream that distorts the incoming
	 * audio so it matches a fixed duration.
	 * 
	 * @param input the AudioInputStream to stretch.
	 * @param duration the duration in seconds the AudioInputStream should be stretched to.
	 * @throws IOException if an IO problem occurs.
	 */
	public static StretchedAudioInputStream create(AudioInputStream input,double duration) throws IOException {
	...
	private static class StretchThread extends Thread {
		...
		StretchThread(AudioInputStream in,AudioFormat format,long frameCount,OutputStream out) {
			super("Stretch Audio Thread");
			this.in = new BufferedInputStream(in);
			this.out = out;
			frameSize = format.getFrameSize();
			function = PolynomialFunction.createFit(0, 0, in.getFrameLength(), frameCount);
		}
			...
			try {
				...
				while( (read = in.read(data) )!= -1) {
					for(int t = 0; t<read; t+=frameSize) {
						long mappedFrame = (long)function.evaluate(inputFrameIndex);
				...
				if(e.getMessage().toLowerCase().contains("read end dead")) {
					//do nothing: someone closed the receiving input stream, so we're done
				} else {
					e.printStackTrace();
				}
			} finally {
