...
public class WavFormatChunk {
	...
	/** The number of bytes per sample slice. This value is not affected by the number of channels and can be calculated with the formula: BlockAlign = SignificantBitsPerSample / 8 * NumChannels */
	...
	/**
	 * 
	 * @param compressionCode Any value is acceptable, but COMPRESSION_PCM (0x0001) is the most common.
	 * @param numChannels The number of channels specifies how many separate audio signals that are encoded in the wave data chunk. A value of 1 means a mono signal, a value of 2 means a stereo signal, etc. 
	 * @param sampleRate The number of sample slices per second. This value is unaffected by the number of channels. 
	 * @param sigBitsPerSample This value specifies the number of bits used to define each sample. This value is unaffected by the number of channels.  This value is usually 8, 16, 24 or 32. If the number of bits is not byte aligned (a multiple of 8) then the number of bytes used per sample is rounded up to the nearest byte size and the unused bytes are set to 0 and ignored. 
	 */
	public WavFormatChunk(int compressionCode,int numChannels,long sampleRate,int sigBitsPerSample) {
	 ...
	 * @param numChannels The number of channels specifies how many separate audio signals that are encoded in the wave data chunk. A value of 1 means a mono signal, a value of 2 means a stereo signal, etc. 
	 * @param sampleRate The number of sample slices per second. This value is unaffected by the number of channels. 
	 * @param avgBytesPerSecond This value indicates how many bytes of wave data must be streamed to a D/A converter per second in order to play the wave file. This information is useful when determining if data can be streamed from the source fast enough to keep up with playback. This value can be easily calculated with the formula: AvgBytesPerSec = SampleRate * BlockAlign 
	 * @param blockAlign The number of bytes per sample slice. This value is not affected by the number of channels and can be calculated with the formula: BlockAlign = SignificantBitsPerSample / 8 * NumChannels 
	 * @param sigBitsPerSample This value specifies the number of bits used to define each sample. This value is usually 8, 16, 24 or 32. If the number of bits is not byte aligned (a multiple of 8) then the number of bytes used per sample is rounded up to the nearest byte size and the unused bytes are set to 0 and ignored. 
	 */
	public WavFormatChunk(int compressionCode, int numChannels,long sampleRate,long avgBytesPerSecond,int blockAlign,int sigBitsPerSample) {
		this.compressionCode = compressionCode;
		this.numChannels = numChannels;
		this.sampleRate = sampleRate;
	 ...
	 * @param includeHeader whether to include the chunk ID and chunk size or not.
	 * @throws IOException if an IO problem occurs.
	 */
	public void write(OutputStream out,boolean includeHeader) throws IOException {
	...
	public String toString() {
		...
		" sigBitsPerSample = "+sigBitsPerSample+" )";
	}
}
