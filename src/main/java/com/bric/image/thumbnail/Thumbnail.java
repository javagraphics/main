...
public abstract class Thumbnail {
	 ...
	 * This abstract class may rely on a BufferedImage, Image, File, or URL.
	 */
	protected abstract static class ImageSource {
		/** Scale the source image to a specific size
		 * 
		 * @param destinationSize the exact size to scale the image source to.
		 * The image may be stretched as needed to fit this size.
		 * <p>It is assumed (but not enforced) that the destinationSize
	...
	private static class URLImageSource extends ImageSource {
		...
		@Override
		public BufferedImage scale(Dimension destinationSize) {
			return Scaling.scale(url, BufferedImage.TYPE_INT_ARGB, destinationSize);
		}
	}
	...
	private static class FileImageSource extends ImageSource {
		...
		@Override
		public BufferedImage scale(Dimension destinationSize) {
			return Scaling.scale(file, BufferedImage.TYPE_INT_ARGB, destinationSize);
		}
	}

	/** Create a thumbnail from a URL.
	 * 
	 * @param source a url for a PNG, JPG, GIF, or BMP image.
	 * @param maxSize the maximum dimensions of the thumbnail. Note the
	 * returned thumbnail may be smaller than these bounds. For example:
	 * if you have a 4:3 landscape-oriented picture and the maximum size is
	 * 64x64 pixels, then the return image will be 64 pixels wide and
	 * approximately 48 pixels tall. (It may not be exactly 48 pixels tall,
	 * depending on the decorations this thumbnail format may add.)
	 * <p>If the source image is already smaller than the maxSize, then
	 ...
	 * 
	 * @param source an image to create a thumbnail for.
	 * @param maxSize the maximum dimensions of the thumbnail. Note the
	 * returned thumbnail may be smaller than these bounds. For example:
	 * if you have a 4:3 landscape-oriented picture and the maximum size is
	 * 64x64 pixels, then the return image will be 64 pixels wide and
	 ...
	 * 
	 * @param source a file for a PNG, JPG, GIF, or BMP image.
	 * @param maxSize the maximum dimensions of the thumbnail. Note the
	 * returned thumbnail may be smaller than these bounds. For example:
	 * if you have a 4:3 landscape-oriented picture and the maximum size is
	 * 64x64 pixels, then the return image will be 64 pixels wide and
	 ...
	 * @return an image that is at most maxSize dimensions.
	 */
	protected abstract BufferedImage create(ImageSource source,Dimension maxSize);
}
