/*
 * @(#)GenericImageSinglePassIterator.java
 *
...
public abstract class GenericImageSinglePassIterator implements PixelIterator {
	 ...
	 * By default this is 120,000.
	 */
	public static long TIMEOUT_FOR_CONSTRUCTION = 120000;
	
	/** This exception indicates that the source image did not provide
	...
	/** This object is the intermediate between the two threads controlling
	 * this iterator.  This has 4 different uses:
	 * 1. When the ImageProducer first receives this, it is a request (and an empty vessel)
	 * 2. Then this object is passed back to the iterator as pixel data
	 * 3. Then this object is passed back to the ImageProducer as an acknowledgment that
	 * everything went well.
	 * 4. At any of the previous 3 stages: if the "error" field is defined then an exception
	 * should be thrown using that string as the exception description.
	 */
	private static class PixelPackage {
		...
		synchronized void deliver(int x,int y,int w,int h,ColorModel colorModel,Object pixels,int offset,int scanSize) {
			error = null;
			this.x = x;
			this.y = y;
			this.w = w;
	...
	private static class GenericImageSinglePassByteIterator extends GenericImageSinglePassIterator 
		implements BytePixelIterator {

		public GenericImageSinglePassByteIterator(
		...
		private boolean isBGR(ColorModel model) {
			if(model.getTransferType()!=DataBuffer.TYPE_BYTE || model.getPixelSize()!=24)
				return false;
			//There has to be a better way to do this, but I don't know what that is...
		...
		private boolean isABGR(ColorModel model) {
			if(model.getTransferType()!=DataBuffer.TYPE_BYTE || model.getPixelSize()!=32)
				return false;
			//There has to be a better way to do this, but I don't know what that is...
		...
		private boolean isGray(ColorModel model) {
			if(model.getTransferType()!=DataBuffer.TYPE_BYTE || model.getPixelSize()!=8)
				return false;
			//There has to be a better way to do this, but I don't know what that is...
		...
		@Override
		void populate(Object dest,Object pixels,int offset,int x,int width,ColorModel colorModel) {
			byte[] destArray = (byte[])dest;
			try {
				...
				if(colorModel.getTransferType()==DataBuffer.TYPE_BYTE) {
					...
					if(scratchArray==null || scratchArray.length<bytesPerPixel) {
						scratchArray = new byte[bytesPerPixel];
					}
					
					if(type==BufferedImage.TYPE_3BYTE_BGR && bytesPerPixel==3 && isBGR(colorModel)) {
						System.arraycopy(pixelData, offset+3*x, destArray, 3*x, width*3);
					} else if( (type==BufferedImage.TYPE_4BYTE_ABGR_PRE ||
							type==BufferedImage.TYPE_4BYTE_ABGR) && bytesPerPixel==4 && isABGR(colorModel)) {
						System.arraycopy(pixelData, offset+4*x, destArray, 4*x, width*4);
					} else if(type==BufferedImage.TYPE_BYTE_GRAY && bytesPerPixel==1 && isGray(colorModel)) {
						System.arraycopy(pixelData, offset+x, destArray, x, width);
					} else {
						for(int myX = x; myX<x+width; myX++) {
							for(int k = 0; k<bytesPerPixel; k++) {
								scratchArray[k] = pixelData[offset + myX*bytesPerPixel + k];
							}
							...
							} else if(type==BufferedImage.TYPE_4BYTE_ABGR || type==BufferedImage.TYPE_4BYTE_ABGR) {
							...
							} else if(type==PixelIterator.TYPE_4BYTE_ARGB || type==PixelIterator.TYPE_4BYTE_ARGB_PRE) {
								destArray[ 4*myX+0 ] = (byte)( (rgb >> 24) & 0xff);
					...
					if(type==BufferedImage.TYPE_3BYTE_BGR) {
						for(int myX = x; myX<x+width; myX++) {
							int rgb = colorModel.getRGB( pixelData[myX+offset] );
							destArray[ 3*myX+0 ] = (byte)( (rgb >> 16) & 0xff);
							destArray[ 3*myX+1 ] = (byte)( (rgb >> 8) & 0xff);
							destArray[ 3*myX+2 ] = (byte)( (rgb >> 0) & 0xff);
						}
					...
					//Because this is all coming from setPixels(...) methods, which
		...
		/**
		 * 
		 * @throws NonSinglePassException if the source image did not
		 * deliver the image in a single pass.
		 */
	...
	private static class GenericImageSinglePassIntIterator extends GenericImageSinglePassIterator 
		implements IntPixelIterator {

		public GenericImageSinglePassIntIterator(
				int width, int height,
				int type, boolean topDown) {
			super(width, height, type, topDown);
		}
		...
		public int getPixelSize() {
			...
			throw new RuntimeException("unexpected iterator type: "+type+" "+
					Reflection.nameStaticField(BufferedImage.class, new Integer(type)));
		}
		
		/** Flip the bytes for int elements in an array.
		 * 
		 * @param array an array with elements to flip.
		 * @param offset the offset of the array to begin flipping
		 * @param length the number of array elements to flip
		 * @param constantMask a mask of the values that should not change
		 * @param byteIndex1 the index of the first byte to swap
		 * @param byteIndex2 the index of the second byte to swap
		 */
		static void flipBytes(int[] array,int offset,int length,int constantMask,int byteIndex1,int byteIndex2) {
				...
				if(colorModel.getTransferType()==DataBuffer.TYPE_BYTE) {
					...
					if(type==BufferedImage.TYPE_INT_RGB && 
						...
						for(int a = 0; a<width; a++) {
							destArray[x+a] = 0xff000000 + (pixelData[offset+x+a] & 0xffffff);
						}
	...
	private static class Consumer implements ImageConsumer {
		...
		/** This is used only once to define the GenericImageSinglePassIterator, or
		 * a String representing an error in creating it. */
		...
		public void setHints(int hintFlags) {
			/** In my experience these hints are unreliable.  A core part
			 * of developing this class involves testing JPEGs: they never
			 * appeared to support the flag for single-pass, but they were
			 * actually single-passes in the end.  So I don't trust this
			 * flag at all.
			 * 
		...
		private void processCachedPixels() {
			if(cachedPixelPackage!=null) {
				processPixels( cachedPixelPackage.x, cachedPixelPackage.y, 
						...
						cachedPixelPackage.pixels, cachedPixelPackage.offset, cachedPixelPackage.scanSize );
		...
		private void _setPixels(int x, int y, int w, int h, ColorModel model,
			...
			/** Sometimes images are decoded in multiple passes.
			 * Google PNG interlacing for an example.  In this case
			 * we would be within our rights to throw a NonSinglePassException,
			 * but since the ENTIRE block is going to be repeated a few more
			 * times, we can still make this work.
			 * 
			 */
			if(x==0 && y==0 && width.intValue()==w && height.intValue()==h) {
				cachedPixelPackage = new PixelPackage();
				cachedPixelPackage.deliver(x, y, w, h, model, pixels, offset, scanSize);
				return;
			}
		...
		private void processPixels(int x, int y, int w, int h, ColorModel model,
				Object pixels, int offset, int scanSize) {
			try {
				/** 1. First we receive an empty (or rewritable) package for a request.
				 *  2. Then we pass our contents along to the outgoing queue.
				 *  3. Then we wait for acknowledgment, indicating that the client
				 *  is done.
				 */
				...
				pixelPackage.deliver(x, y, w, h, model, pixels, offset, scanSize);
		...
		/** This is called when setPixels(...) is called, and if a GenericImageSinglePassIterator
		 * hasn't been defined yet: then we define it here.
		 * 
		 * @param topDown whether this data is readable as top-to-bottom data
		 * vs bottom-to-top.
		 */
		private void initIterator(boolean topDown) {
			if(definedIterator==false) {
				...
				switch(iteratorType) {
					...
					case PixelIterator.TYPE_4BYTE_ARGB_PRE:
					case BufferedImage.TYPE_BYTE_GRAY:
						iterator = new GenericImageSinglePassByteIterator(
								w, h, iteratorType, topDown);
						break;
					...
					case BufferedImage.TYPE_INT_BGR:
					case BufferedImage.TYPE_INT_RGB:
						iterator = new GenericImageSinglePassIntIterator(
								w, h, iteratorType, topDown);
						break;
					default:
						throw new RuntimeException("unsupported iterator type: "+iteratorType);
				}
		...
		public void imageComplete(int status) {
			...
			if(definedIterator==false) {
				String error = "imageComplete( "+status+" ) was called before setPixels(...)";
				incomingIteratorOrError.push(error);
				throw new RuntimeException(error);
			}
			if(listening) {
		...
		private GenericImageSinglePassIterator getPixelIterator() {
			Object incoming = incomingIteratorOrError.pull(TIMEOUT_FOR_CONSTRUCTION);
	 ...
	 * @param file <code>Toolkit.createImage(filePath)</code> is used to create the
	 * <code>java.awt.Image</code>, so the supported image types are JPG, PNG and GIF.
	 * @param iteratorType one of these 4 BufferedImage types:
	 * TYPE_INT_ARGB, TYPE_INT_ARGB_PRE, TYPE_INT_RGB, TYPE_INT_BGR.
	 * @return a <code>GenericImageSinglePassIntIterator</code> for the file provided.
	 */
	public static GenericImageSinglePassIntIterator getIntIterator(File file,int iteratorType) {
	 ...
	 * @return a <code>GenericImageSinglePassByteIterator</code> for the file provided.
	 */
	public static GenericImageSinglePassByteIterator getByteIterator(File file,int iteratorType) {
	 ...
	 * TYPE_INT_ARGB, TYPE_INT_ARGB_PRE, TYPE_INT_RGB, TYPE_INT_BGR,
	 * TYPE_3BYTE_BGR, TYPE_BYTE_GRAY, TYPE_4BYTE_ABGR, TYPE_4BYTE_ABGR_PRE.
	 * @return a <code>GenericImageSinglePassIterator</code> for the file provided.
	 */
	public static GenericImageSinglePassIterator get(File file,int iteratorType) {
		...
		if(image==null) throw new IllegalArgumentException("The toolkit could not create an image for "+file.getAbsolutePath());
	 ...
	 * request: then the graphic is returned at its original size.
	 * @param url the graphic to create a thumbnail of.
	 * @param maxSize the largest bounds of the thumbnail.  For example:
	...
	public static BufferedImage createScaledImage(URL url,Dimension maxSize) {
		...
		boolean isJPEG = url.toString().toLowerCase().endsWith(".jpg") || url.toString().toLowerCase().endsWith(".jpeg");
		int type = isJPEG ? BufferedImage.TYPE_INT_RGB : BufferedImage.TYPE_INT_ARGB;
		...
		Dimension newSize = Scaling.scaleDimensionsProportionally(currentSize, maxSize);
	...
	/** The number of rows we have processed.
	 * This value ranges from [0, height-1].  This
	 * is a counter, so the value of topDown
	 * does not affect this value.
	 */
	int rowCtr = 0;

	private GenericImageSinglePassIterator(int width, int height,int type,boolean topDown) {
		this.width = width;
		this.height = height;
		this.type = type;
	...
	/** Returns the pixel type of this iterator.  This will be one of these 8 BufferedImage types:
	 * TYPE_INT_ARGB, TYPE_INT_ARGB_PRE, TYPE_INT_RGB, TYPE_INT_BGR,
	 * TYPE_3BYTE_BGR, TYPE_BYTE_GRAY, TYPE_4BYTE_ABGR, TYPE_4BYTE_ABGR_PRE.
	 */
	public int getType() {
	...
	/** Returns true if the pixel type of this iterator can only
	 * express opaque images.
	 */
	public boolean isOpaque() {
		return PixelConverter.isOpaque(type);
	}

	/** If false then there is still pixel data to process
	 * by calling <code>next(...)</code>.
	 * 
	 */
	...
	/** The length to create arrays to pass to the
	...
	public void skip() {
		/** This method does nothing, because it's in the
		 * next() method that we assess what we've
		 * been handed and what goes in the destination
		 * array.
		 */
		rowCtr++;
		if(isDone()) {
	...
	/**
	 * 
	 * @param destArray will be either an int[] or byte[]
	 */
	synchronized void processNextRow(Object destArray) {
		while(true) {
			...
			PixelPackage pixelPackage = incoming.pull(TIMEOUT_IN_PROCESS);
			if(pixelPackage.error!=null) throw new RuntimeException(pixelPackage.error);
			...
			try {
				if(pixelPackage.finished){
					...
					String error = "The iterator needed to process row "+rowCtr+", but was given row "+pixelPackage.y;
					...
					populate(destArray, pixelPackage.pixels, 
							pixelPackage.offset, pixelPackage.x, pixelPackage.w, pixelPackage.colorModel);
					rowCtr++;
					pixelPackage.y++;
					pixelPackage.h--;
					pixelPackage.offset += pixelPackage.scanSize;
	...
	abstract void populate(Object destArray,Object pixels,
			int offset,int x,int width,ColorModel colorModel);
}
