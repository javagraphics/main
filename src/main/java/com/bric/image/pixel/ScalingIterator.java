 ...
 * <p>You cannot directly instantiate this class, but you can
 * use the static <code>get()</code> methods or instantiate the two
 * subclasses: <code>IntScalingIterator</code> or <code>ByteScalingIterator</code>.
 * 
 * @see com.bric.image.pixel.ScalingDemo
...
public abstract class ScalingIterator implements PixelIterator {
	 ...
	 * <p>If the ratio is 1.0: then this method returns the argument provided.
	 * Otherwise a new scaled iterator is returned.
	 * <p>If the image type of the source image is one of the four supported
	 * int-based types (ARGB, ARGB_PRE, RGB, or BGR) then the new scaled iterator
	 * will also use that type.  Otherwise the new iterator will use ARGB or RGB
	 * (depending on whether the source image is opaque or not).
	 * 
	 * @param i the incoming source image data.
	 * @param scaledRatio the ratio to scale to, where 1.0 is 100%, .5 is 50%, etc.
	 ...
	 * Otherwise a new scaled iterator is returned.
	 * <p>If the image type of the source image is one of the four supported
	 * byte-based types (ABGR, ABGR_PRE, BGR, or GRAY) then the new scaled iterator
	 * will also use that type.  Otherwise the new iterator will use ABGR or BGR
	 * (depending on whether the source image is opaque or not).
	 * 
	 * @param i the incoming source image data.
	 * @param scaledRatio the ratio to scale to, where 1.0 is 100%, .5 is 50%, etc.
	 ...
	 * <p>If the ratio is 1.0: then this method returns the argument provided.
	 * Otherwise a new scaled iterator is returned.
	 * <p>If the image type of the source image is one of the eight supported
	 * types: then the new scaled iterator will also use that type.  Otherwise 
	 * the new iterator will use ABGR / BGR (for bytes) or ARGB / RGB (for ints).
	 * 
	 * @param i the incoming source image data.
	 * @param scaledRatio the ratio to scale to, where 1.0 is 100%, .5 is 50%, etc.
	 * @return a scaled iterator.
	...
	/** Checks the incoming image type against the 4 int types this
	 * class supports.
	 */
	...
	/** Checks the incoming image type against the 4 byte types this
	 * class supports.
	 */
	 ...
	 * <p>If the source image is already the width and height specified: then this
	 ...
	 */
	public static IntPixelIterator get(IntPixelIterator i,int newWidth,int newHeight) {
	 ...
	 * @return an iterator that uses the width and height specified.
	 */
	public static BytePixelIterator get(BytePixelIterator i,int newWidth,int newHeight) {
		if(i.getWidth()==newWidth && i.getHeight()==newHeight)
			return i;
		int imageType = i.getType();
	...
	public static PixelIterator get(PixelIterator i,int newWidth,int newHeight) {
		...
		throw new IllegalArgumentException("Unsupported iterator: "+i.getClass().getName());
	}
	...
	public static class ByteScalingIterator extends ScalingIterator implements BytePixelIterator {
		 ...
		 * 
		 * @param i the incoming image data to scale.
		 * @param newImageType this must be one of the four byte-based pixel types
		 * this object supports: BGR, ABGR, ABGR_PRE, or GRAY.
		 * @param newWidth the width to scale to.
		 * @param newHeight the height to scale to.
		 */
		public ByteScalingIterator(PixelIterator i,int newImageType,int newWidth,int newHeight) {
			super(i, i.getWidth(), i.getHeight(), newWidth, newHeight, i.isTopDown(), i.isOpaque());
			this.imageType = newImageType;

			if(isSupportedByteType(imageType)==false)
				throw new IllegalArgumentException("the image type for this contructor must be TYPE_3BYTE_BGR, TYPE_4BYTE_ABGR, TYPE_4BYTE_ABGR_PRE or TYPE_BYTE_GRAY. (newImageType = "+newImageType+")");
		}
		...
		public int getMinimumArrayLength() {
			...
			if(srcIterator instanceof BytePixelIterator) {
				return Math.max( srcIterator.getMinimumArrayLength(), dstW*bytesPerPixel);
			}
		...
		public int getPixelSize() {
			...
			throw new RuntimeException("unexpected condition: imageType = "+imageType);
		}
	...
	public static class IntScalingIterator extends ScalingIterator implements IntPixelIterator {
		 ...
		 * 
		 * @param i the incoming image data to scale.
		 * @param newImageType this must be one of the four int-based pixel types
		 * this object supports: ARGB, ARGB_PRE, RGB or BGR.
		 * @param newWidth the width to scale to.
		 * @param newHeight the height to scale to.
		 */
		public IntScalingIterator(PixelIterator i,int newImageType,int newWidth,int newHeight) {
			super(i, i.getWidth(), i.getHeight(), newWidth, newHeight, i.isTopDown(), i.isOpaque());
			this.imageType = newImageType;
			
			if(isSupportedIntType(newImageType)==false)
				throw new IllegalArgumentException("the image type for this constructor must be TYPE_INT_ARGB, TYPE_INT_ARGB_PRE, TYPE_INT_BGR, or TYPE_INT_RGB. (newImageType = "+newImageType+")");
		}
	...
	static abstract class Row {
		/** This indicates the source row this Row object
		 * corresponds to.  This is only used for upsampling.
		 */
		int marker = -1;
		 ...
		 * @nextRow it is assumed this argument will be of the same class
		 * as this object.
		 * @fraction the amount of this row vs the argument to use.  When zero:
		 * only this object is used, when one: only the argument is used.
		 */
		abstract void writeColorComponents(Row nextRow, double fraction, int[] destArray,int type);
		 ...
		 * only this object is used, when one: only the argument is used.
		 */
		abstract void writeColorComponents(Row nextRow, double fraction, byte[] destArray,int type);
	...
	class IntRow extends Row {
			...
			for(int x = 0; x<width; x++) {
				...
				case 0:
					...
					if(x<sums.length) {
						...
						//erm, hopefully this isn't happening, but just in case.
						newAlpha = lastAlpha;
						newRed = lastRed;
						newGreen = lastGreen;
						newBlue = lastBlue;
		...
		void writeColorComponents(Row nextRow, double fraction,
			...
			if(fraction>1 || fraction<0)
				throw new IllegalArgumentException("fraction ("+fraction+") must be within [0,1]");
			IntRow next = (IntRow)nextRow;

			switch(type) {
				case BufferedImage.TYPE_INT_RGB:
					for(int x = 0; x<width; x++) {
						int r = (int)((reds[x] / sums[x])*(1-fraction)+(next.reds[x] / next.sums[x])*fraction);
						int g = (int)((greens[x] / sums[x])*(1-fraction)+(next.greens[x] / next.sums[x])*fraction);
						int b = (int)((blues[x] / sums[x])*(1-fraction)+(next.blues[x] / next.sums[x])*fraction);
				...
				case BufferedImage.TYPE_INT_ARGB_PRE:
					for(int x = 0; x<width; x++) {
						...
						int g = (int)((greens[x] / sums[x])*(1-fraction)+(next.greens[x] / next.sums[x])*fraction);
						int b = (int)((blues[x] / sums[x])*(1-fraction)+(next.blues[x] / next.sums[x])*fraction);
						int a = (int)((alphas[x] / sums[x])*(1-fraction)+(next.alphas[x] / next.sums[x])*fraction);
		...
		void interpolateXValues() {
			...
			for(int x = 0; x<width; x++) {
				...
				case 0:
					...
					for(int k = startX; k<x; k++) {
						int f = k-startX+1;
						alphas[k] = (short) ((lastAlpha*(span-f)+newAlpha*f)/span);
						reds[k] = (short) ((lastRed*(span-f)+newRed*f)/span);
						greens[k] = (short) ((lastGreen*(span-f)+newGreen*f)/span);
						blues[k] = (short) ((lastBlue*(span-f)+newBlue*f)/span);
						sums[k] = 1;
					}
					break;
				}
	...
	private ScalingIterator(PixelIterator srcIterator,int srcW,int srcH,int scaledWidth,int scaledHeight,boolean topDown,boolean isOpaque) {
		if(srcIterator instanceof BytePixelIterator && 
		...
		/** If the x and y scale factor are .1, then we'll dump
		 * 100 source pixels (separated into separate RGB channels)
		 * into 1 dest element.  When this value crosses a threshold:
		 * we should switch from shorts to ints.  If we don't
		 * then we'll suffer arithmetic overflows, and the color channels
		 * will look funky.
		 */
		int capacity = MathG.ceilInt(1.0/scaleX)*MathG.ceilInt(1.0/scaleY);
		if(capacity>0xff) {
		 ...
		 * This is a slight oversimplification: because in other places
		 * when a scale factor (x or y) is less than .25, then we start
		 * intentionally skipping source pixels.  The threshold *would*
		 * be 0xff if we didn't skip anything.  Since we're skipping data:
		 * we could afford to make this higher.  But I don't know exactly
		 * how high...
		 */
	}
	...
	/** This is called by both the int and byte subclasses, so exactly
	 * one of these arguments will always be null.  But so much of the
	 * logic is the same it made sense to put it all in one method.
	 * 
	...
	/** This is called when scaleY>1, and we have to read two rows from
	 * the source image and interpolate between them to write two or more
	 * rows in the destination image.
	 * <P>Exactly one of the arguments will be null, depending on whether this
	 * data should be written with ints or bytes.
	 */
	void nextUpsample(byte[] incomingByteArray,int[] incomingIntArray) {
		/** I took the path of least resistance in writing this upsampling method:
		 * We tween ALL rows (except the first row).  The result is a very smooth
		 * upward scaling, but the performance could be slightly improved if we
		 * introduced rounding.  But the rounding would actually be a little
		 * bit more work, and would result in a lower image quality... so I'm
		 * not going to implement it for now.
		 */
		...
		if(row.marker<srcY0) {
			...
			row.marker = srcY0;
			if(scaleX>1) row.interpolateXValues();
		} 
		...
		if(srcY0==srcY1) {
			...
			if(row2.marker<srcY1) {
				...
				row2.marker = srcY1;
				if(scaleX>1) row2.interpolateXValues();
			}
	
	
			if(incomingIntArray!=null) {
				row.writeColorComponents(row2, srcFraction, incomingIntArray, getType());
			} else {
				row.writeColorComponents(row2, srcFraction, incomingByteArray, getType());
			}
		}
	}
	...
	/** Reads one row of pixel data from the source into the Row
	 * argument provided.
	 * 
	 * @param row the row to collect the incoming pixel data.
	 * @param incomingByteArray an optional byte array.  If this is null and
	 * a byte array is needed: the <code>scratchByteArray</code> will be
	 * used.
	 * @param incomingIntArray an optional byte array.  If this is null and
	 * an int array is needed: the <code>scratchIntArray</code> will be
	 * used.
	 */
		...
		if(srcIterator instanceof IntPixelIterator) {
			...
			if(intArray==null) {
				...
				if(scratchIntArray==null)
					scratchIntArray = new int[srcIterator.getMinimumArrayLength()];
			...
			if(byteArray==null) {
				...
				if(scratchByteArray==null)
					scratchByteArray = new byte[srcIterator.getMinimumArrayLength()];
