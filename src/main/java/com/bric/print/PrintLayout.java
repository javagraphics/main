...
public class PrintLayout {
	...
	public Paintable[] createPaintables(Paintable[] paintables, int pgNumber,
		...
		String customHeaderText = Text.replace(header, "<PageNumber/>", Integer
				.toString(1));
		customHeaderText = Text.replace(customHeaderText, "<PageCount/>",
				Integer.toString(1));
		headerPaintable.define(customHeaderText, (int) (getPaperWidth()
				- insets.left - insets.right));
		String customFooterText = Text.replace(footer, "<PageNumber/>", Integer
				.toString(1));
		customFooterText = Text.replace(customFooterText, "<PageCount/>",
				Integer.toString(1));
		...
		for (int page = 0; page < pages.length; page++) {
			...
			String thisHeader = Text.replace(header, "<PageNumber/>", Integer
					.toString(page + pgNumber + 1));
			thisHeader = Text.replace(thisHeader, "<PageCount/>", Integer
					.toString(pgCount));
			...
			String thisFooter = Text.replace(footer, "<PageNumber/>", Integer
					.toString(page + pgNumber + 1));
			thisFooter = Text.replace(thisFooter, "<PageCount/>", Integer
					.toString(pgCount));
