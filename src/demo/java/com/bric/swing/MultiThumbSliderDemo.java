...
instructions = "This should be pretty self-explanatory: fiddle with the controls in the "
		+ "first tab to adjust the look and behavior of the 4 different MultiThumbSliders "
		+ "shown. Note in real deployments: these properties should be predefined and the "
		+ "user will have no way of changing them. It is possible in this demo app to "
		+ "achieve some paradoxical/buggy results (try overlapping two thumbs, then change "
		+ "the collision policy to avoid overlapping thumbs), but if these settings "
		+ "are immutable to the user this shouldn't be a major problem."
		+ "<p>The second tab uses regular JSliders for comparison. This was provided as a "
		+ "reference when developing the look and feel.",
link = "http://javagraphics.blogspot.com/2015/01/sliders-creating-slider-with-multiple.html",
sandboxDemo = true
)
