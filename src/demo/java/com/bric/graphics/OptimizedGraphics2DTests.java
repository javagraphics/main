...
/** A set of tests for the {@link OptimizedGraphics2D} class.
 * 
 */
@Blurb (
		filename = "OptimizedGraphics2D",
		title = "Graphics2D: Improving Performance",
		releaseDate = "February 2009",
		summary = "It turns out some <code>Graphics2D</code> operations are painfully inefficient -- "+
				"and we can easily intercept and divert those methods!\n"+
				"<p>This is becoming less of a problem now that Quartz is being phased out, but even without "+
				"Quartz we can still make some improvements...",
				link = "http://javagraphics.blogspot.com/2009/02/graphics2d-improving-performance.html",
				sandboxDemo = true
		)
public class OptimizedGraphics2DTests {
	...
	static class ClipBoundsTest extends Test {
		...
		public void run(BufferedImage dest, boolean optimize) {
			...
			if(optimize) {
				g = new OptimizedGraphics2D(g,OptimizedGraphics2D.Option.OPTIMIZE_CLIP_BOUNDS);
			}
	...
	static class GlyphVectorTest extends Test {
	...
	static class ClipTest extends Test {
	...
	static class ImageBackgroundTest extends Test {
	...
	static class FillClipTest extends Test {
		...
		public void run(BufferedImage dest, boolean optimize) {
			Graphics2D g = dest.createGraphics();
			if(optimize)
				g = new OptimizedGraphics2D(g,OptimizedGraphics2D.Option.OPTIMIZE_CLIPPED_SHAPES);
