 ...
 * <p>This v7 draft caches a product to reduce the
 * number of multiplications. This did not improve performance.
 * @see com.bric.image.BasicImageContextDemo
 */
public class BasicImageContext_v7 extends ImageContext {
	 ...
	 */
	public BasicImageContext_v7(BufferedImage bi) {
		int type = bi.getType();
						...
						if(newY>=0 && newY<oh && newX>=0 && newX<ow) {
							...
							if(srcA==255) {
								...
								if(resA!=0) {
									...
									data[yw+x] = (resA / 255 << 24) + (r << 16) + (g << 8) + b;
								}
							}
						}
					}
