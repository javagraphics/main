...
public class IntersectionsTest extends BasicTestElement {
	...
	private Shape getShape(int i) {
		...
		if(type.equals("Lines")) {
			return new Line2D.Double( random.nextDouble()*100, random.nextDouble()*100,
					random.nextDouble()*100, random.nextDouble()*100);
		} else if(type.equals("Quads")) {
			return new QuadCurve2D.Double( random.nextDouble()*100, random.nextDouble()*100,
					random.nextDouble()*100, random.nextDouble()*100,
					random.nextDouble()*100, random.nextDouble()*100 );
		} else if(type.equals("Cubics")) {
			return new CubicCurve2D.Double( random.nextDouble()*100, random.nextDouble()*100,
					random.nextDouble()*100, random.nextDouble()*100,
					random.nextDouble()*100, random.nextDouble()*100,
					random.nextDouble()*100, random.nextDouble()*100 );
		}
	...
	public void doTest() {
		...
		int failures = 0;
		int testCount = 1000;
		System.out.println("Beginning Intersections Accuracy Tests...");
		for(int a = 0; a<testCount; a++) {
			...
			for(int b = a; b<testCount; b++) {
				...
				boolean observedIntersection = isIntersectionObserved(shapeA, shapeB, image1, image2, sumBounds, 1);
				boolean theoreticalIntersection = getTheoreticalIntersection(shapeA, shapeB);
				...
				if(!passed) {					
					...
					System.out.println("failed.  lhs = "+a+", rhs = "+b+", theoretical = "+theoreticalIntersection+", observed = "+observedIntersection+", ctr = "+ctr+", bounds = "+sumBounds);
					failures++;
					ctr++;
	...
	protected static boolean isIntersectionObserved(Shape shapeA,Shape shapeB,BufferedImage bi1,BufferedImage bi2,Rectangle2D rect,int depth) {
		...
		Vector<Rectangle2D> rects = new Vector<Rectangle2D>();
	...
	public static final void draw(Shape shape,AffineTransform at,BufferedImage bi,boolean showControlPoints) {
		clear(bi);
		Graphics2D g = bi.createGraphics();
		g.setColor(Color.black);
		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		g.fill( new BasicStroke(1).createStrokedShape( at.createTransformedShape(shape) ) );
		if(showControlPoints) {
			...
			while(i.isDone()==false) {
				...
				if(k==PathIterator.SEG_QUADTO || k==PathIterator.SEG_CUBICTO) {
					g.setColor(Color.blue);
					g.fill(new Rectangle2D.Double(coords[0]-1,coords[1]-1,2,2));
				}
				if(k==PathIterator.SEG_CUBICTO) {
					g.setColor(Color.green);
					g.fill(new Rectangle2D.Double(coords[2]-1,coords[3]-1,2,2));
				}
				i.next();
			}
		}
	...
	public String getDescription() {
		//TODO: explore these 5 failures more.  If it is confirmed that it is the test that is failing,
