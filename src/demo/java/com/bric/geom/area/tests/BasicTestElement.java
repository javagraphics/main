...
public abstract class BasicTestElement extends TestElement {
	...
	/** This method is responsible for updating the progress
	 * bar and checking the cancelled boolean.
	 */
	...
	public synchronized static boolean equals(Shape s1,Shape s2) {
		...
		g.transform( RectangularTransform.create(sum, new Rectangle(0,0,image1.getWidth(),image1.getHeight())) );
		...
		return equals(image1, image2);
	}
	
	/** Check to see if two images are equal, give or take a 3x3
	 * cluster of pixels.  This assumes both images are black-on-transparent.
	 * @param bi1
	 * @param bi2
	 */
								...
								if(errors.contains(p2)) {
									...
									if(errors.contains(p2)) {
										...
										if(errors.contains(p2)) {
											...
											if(errors.contains(p2)) {
												try {
													File f1 = new File("problem"+(ctr++)+".png");
													File f2 = new File("problem"+(ctr++)+".png");
