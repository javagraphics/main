...
public class AccuracyTest extends BasicTestElement {
	...
	private GeneralPath getShape(int index) {
		...
		GeneralPath path = new GeneralPath( r.nextBoolean() ? 
	...
	public String getDescription () {
		return "This creates several random shapes " +
			...
			"graphically on an image that is "+image1.getWidth()+"x"+image1.getHeight()+" pixels.  The image is then scanned " +
	...
	public void doTest() {
		...
		System.out.println("Beginning Accuracy Tests...");
		for(int a = 0; a<operations.length; a++) {
			for(int b = 0; b<numShapes; b+=incr) {
				for(int c = 0; c<numShapes; c+=incr) {
					...
					m = m/(operations.length)+((float)a)/((float)operations.length);
					...
					if(shape.getSelectedIndex()==0) {
						...
						try {
							...
							failures++;
							System.err.println("lhs = "+b+" rhs = "+c);
							t.printStackTrace();
							continue;
						}
					}
					...
					RectangularTransform rt = new RectangularTransform(
							bounds,
					...
					synchronized(image1) {
						...
						boolean passed = equals(image1, image2);
						if(!passed) {
							...
							System.out.println("failed "+opName+".  lhs = "+b+", rhs = "+c);
							failures++;
						}
					}
				}
