	...
	public void doTest() {
		...
		try {
			...
			for(int a = 0; a<time.length; a++) {
				...
				if(equals(image1, image2)==false) {
					failures++;
					System.err.println("failed for a = "+a+" (repeat = "+repeat.getValue()+", additions = "+additions+")");
				}
				
			}
			Arrays.sort(time);
			System.out.println("Median time: "+time[time.length/2]+" ms");
		} finally {
			if(cancelled) {
