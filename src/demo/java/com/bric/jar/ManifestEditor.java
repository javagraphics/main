...
public class ManifestEditor extends JPanel {
	...
	JComboBox<String> permissionsComboBox = new JComboBox<String>(new String[] {"Undefined", "Sandbox", "All-Permissions"});
	...
	JComboBox<String> trustedOnlyComboBox = new JComboBox<String>(new String[] {"Undefined", "True", "False"});
	JComboBox<String> trustedLibraryComboBox = new JComboBox<String>(new String[] {"Undefined", "True", "False"});

	
	public ManifestEditor() {
		addManifestAttribute(layout, "Main-Class", mainClassField, "com.foo.Widget");
		addManifestAttribute(layout, "Created-By", createdByField, "1.7.0_06 (Oracle Corporation)");
		addManifestAttribute(layout, "Class-Path", classPathField, "jar1-name directory-name/jar2-name");
		addManifestAttribute(layout, "SplashScreen-Image", splashField, "splashdemo/splash.png");
		layout.addRow(JarWriterApp.createHeader("Security Manifest Attributes for Applets / Web Start"), SwingConstants.CENTER, true);
		layout.addRow(new JLabel("These attributes apply to signed applets and Java Web Start applications."), SwingConstants.CENTER, true);
		layout.addRow(new JLabel("These attributes are ignored for stand-alone Java applications."), SwingConstants.CENTER, true);
		addManifestAttribute(layout, "Permissions", permissionsComboBox);
		addManifestAttribute(layout, "Codebase", codebaseField, "https://*.example.com");
		addManifestAttribute(layout, "Application-Name", appNameField, "Signed RIAs use this in security prompts as a title to inform user choices.");
		addManifestAttribute(layout, "Entry-Point", entryPointField, "apps.test.TestUI apps.test.TestCLI");
		...
		InspectorGridBagLayout miniLayout = new InspectorGridBagLayout(miniPanel);
		addManifestAttribute(miniLayout, "Application-Library-Allowable-Codebase", appLibAllowCodebaseField, "https://*.example.com");
		addManifestAttribute(miniLayout, "Caller-Allowable-Codebase", callerAllowCodebaseField, "https://*.example.com");
	...
	class ComboBoxAttribute extends Attribute {
		...
		public void write(Manifest manifest) {
			...
			if(i==-1 || "Undefined".equals(comboBox.getModel().getElementAt(i))) {
				manifest.getMainAttributes().remove(attrName);
			} else {
				manifest.getMainAttributes().putValue(attrName, comboBox.getModel().getElementAt(i));
			}
		}
	}
	
	Set<Attribute> attributes = new HashSet<Attribute>();
	private void addManifestAttribute(InspectorGridBagLayout layout,String attrName,JTextField field,String prompt) {
	...
	private void addManifestAttribute(InspectorGridBagLayout layout,String attrName,JComboBox<String> comboBox) {
	 ...
	 * 
	 * @param frame the frame this dialog belongs to.
	 * @param existingManifest the Manifest to edit.
	...
	public static Manifest showDialog(JFrame frame,Manifest existingManifest) {
		...
		JScrollPane scrollPane = new JScrollPane(editor, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		scrollPane.setPreferredSize(new Dimension(600,500));
		int choice = QDialog.showDialog(frame, 
				"Manifest Editor", 
