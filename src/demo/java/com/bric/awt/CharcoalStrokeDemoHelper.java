...
public class CharcoalStrokeDemoHelper extends StrokeHelper {
	...
	public static BufferedImage createBlurbGraphic(Dimension preferredSize) throws Exception {
		...
		g.setStroke(new CharcoalStroke(10,.5f));
	...
	public static File createCharcoalSamples(File resourcesDir) throws IOException, NoSuchMethodException, SecurityException {
		File sampleDir = getSampleDir(resourcesDir);
		File linearSample = new File(sampleDir, "charcoalLines.png");
		File wavySample = new File(sampleDir, "charcoalWaves.gif");
		createWavySamples( wavySample, 
				CharcoalStroke.class.getConstructor( new Class[] {Float.TYPE, Float.TYPE} ),
				"Crack Size",
				new float[] {1, 3, 6, 10});
		createLinearSample( linearSample, 
				new CharcoalStroke(1, .5f),
				new CharcoalStroke(5, .65f),
				new CharcoalStroke(10, .8f)
				);
		return sampleDir;
	}
}
