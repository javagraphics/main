...
public class BristleStrokeDemoHelper extends StrokeHelper {
	...
	public static BufferedImage createBlurbGraphic(Dimension preferredSize) throws Exception {
		...
		Rectangle2D bounds = StrokeDemo.defineWiggle(wiggle);
		...
		g.setStroke(new BristleStroke(10,.5f));
	...
	public static File createBristleSamples(File resourcesDir) throws IOException, NoSuchMethodException, SecurityException {
		File sampleDir = getSampleDir(resourcesDir);
		File linearSample = new File(sampleDir, "bristleLines.png");
		File wavySample = new File(sampleDir, "bristleWaves.gif");
		createWavySamples( wavySample, 
				BristleStroke.class.getConstructor( new Class[] {Float.TYPE, Float.TYPE} ),
				"Thickness",
				new float[] {1, 3, 6, 10});
		createLinearSample( linearSample, 
				new BristleStroke(1, .5f),
				new BristleStroke(5, .65f),
				new BristleStroke(10, .8f)
				);
		return sampleDir;
	}
}
